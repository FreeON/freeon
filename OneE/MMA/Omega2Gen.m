...
omega[lambda_,l_,m_,a_,b_,c_]:=Module[{sum,SumA,SumB,KPartA,KPartB,APartA,APartB},
      sum=0;
      Do[
         KPart=Zxyz[lambda,mu]/.{X->Kx,Y->Ky,Z->Kz};              
         APartA=Expand[Apart[(Zxyz[l,m]*Zxyz[lambda,mu]*X^Eye Y^Jay Z^Kay)]];
         APartA=APartA/.FactorialList;
         APartA=APartA/.{Eye->a,Jay->b,Kay->c};
         sum=sum+KPart*APartA;
        ,{mu,-lambda,lambda}];
8 Pi^(3/2) sum]
...
Do[Do[
   ...
   Index=100*BFEll+ProjL;

   Print[" BFEll = ",BFEll," ProjL = ",ProjL];
   ...
   Do[Do[
     ...
     ,{Ehm,-ProjL,ProjL}],{Lambda,0,BFEll+ProjL}];
