...
!---------------------------------------------------------------------------------------
 SUBROUTINE AddDist(Rho,GM,Zeta,NumDist)
    TYPE(CRDS)                    :: GM
    TYPE(HGRho_new)               :: Rho,RhoTmp
    INTEGER                       :: NumDist,I,iq,ir
...
!
    DO I = 1,NumDist 
       ...
       Rho%Ell%I(Iq)   = 0
       Rho%Zeta%D(Iq)  = Zeta
       Rho%Qx%D(Iq)    = GM%Carts%D(1,I)
       Rho%Qy%D(Iq)    = GM%Carts%D(2,I)
       Rho%Qz%D(Iq)    = GM%Carts%D(3,I)
       Rho%Co%D(Ir)    =-GM%AtNum%D(I)*DDelta
    ENDDO
