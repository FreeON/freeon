...
!       CALL Invoke('IRInv',N,S,M)
  ...
  SUBROUTINE Force(cBAS,cGEO,N,O,S,G,B,M)
    ...
    REAL(DOUBLE)       :: GradVal
TYPE(DBL_RNK2)     :: AuxLatF,AuxLatF2,AuxCartF2
    !----------------------------------------------------------------------------!
    CALL New(S%Action,1)
...
CALL New(AuxLatF2,(/3,3/))
CALL New(AuxCartF2,(/3,3/))
AuxLatF2%D=Zero
AuxCartF2%D=Zero
...
!------------------------------------------------------------
!
   CALL PrintActGrad(N,AuxLatF2,AuxCartF2,G%Clone(1)%Natms,chGEO,'SForce')
!------------------------------------------------------------
    ! Kinetic energy piece
    CALL Invoke('TForce',N,S,M)
!------------------------------------------------------------
   CALL PrintActGrad(N,AuxLatF2,AuxCartF2,G%Clone(1)%Natms,chGEO,'TForce')
!------------------------------------------------------------
   ...
   CALL PrintActGrad(N,AuxLatF2,AuxCartF2,G%Clone(1)%Natms,chGEO,'JForce')
!------------------------------------------------------------
...
#endif
!------------------------------------------------------------
   CALL PrintActGrad(N,AuxLatF2,AuxCartF2,G%Clone(1)%Natms,chGEO,'ONXForce')
CALL Delete(AuxLatF2)
CALL Delete(AuxCartF2)
!------------------------------------------------------------
!
...
!!$       ENDDO
    !  !
    !  ! Substitute total derivative of lattice force
    !  ! by partial one.
    !  !
...
!
       ...
       MaxBlkSize=0
       DO II=1,G%Clone(iCLONE)%NAtms; MaxBlkSize=MAX(MaxBlkSize,BSiz%I(II)); ENDDO
!
       CALL New(P)
       CALL New(J1)
       CALL New(J2)
...
CALL OpenASCII(OutFile,Out)
       WRITE(*,*) 'LatFrc_XC NUM'
       WRITE(Out,*) 'LatFrc_XC NUM'
       DO I=1,3
          WRITE(*,*) (LatFrc_XC(I,J),J=1,3)
          WRITE(Out,*) (LatFrc_XC(I,J),J=1,3)
       ENDDO
...
!-------------------------------------------------------------------
!
   SUBROUTINE PrintActGrad(N,AuxLatF2,AuxCartF2,NatmsLoc,chGEO,Char)
     TYPE(DBL_RNK2)     :: AuxLatF,AuxLatF2,AuxCartF2,AuxCartF
     TYPE(FileNames)    :: N
     INTEGER            :: NatmsLoc,I,J
     CHARACTER(LEN=*)   :: Char,chGEO
     !
     CALL OpenASCII(OutFile,Out)
     CALL New(AuxLatF,(/3,3/))
     ...
     DO I=1,NatmsLoc
        WRITE(*,*) (AuxCartF%D(J,I)-AuxCartF2%D(J,I),J=1,3)
        WRITE(Out,*) (AuxCartF%D(J,I)-AuxCartF2%D(J,I),J=1,3)
     ENDDO
     AuxCartF2%D=AuxCartF%D
     !
     WRITE(*,*) 'LatFrc '//TRIM(Char)
     WRITE(Out,*) 'LatFrc '//TRIM(Char)
     DO I=1,3
        WRITE(*,*) (AuxLatF%D(I,J)-AuxLatF2%D(I,J),J=1,3)
        WRITE(Out,*) (AuxLatF%D(I,J)-AuxLatF2%D(I,J),J=1,3)
     ENDDO
     AuxLatF2%D=AuxLatF%D
     !
     CALL CloseHDFGroup(HDF_CurrentID)
     CALL CloseHDF(HDFFileID)
     CALL Delete(AuxLatF)
     CALL Delete(AuxCartF)
     CLOSE(Out,STATUS='KEEP')
   END SUBROUTINE PrintActGrad
!
!-------------------------------------------------------------------
!
!======================================================================================
