...
!    USING POLYNOMINAL INTERPOLATING GAUSSIAN WAVELET
!    Author: C. J. Tymczak
!-------------------------------------------------------------------------------
MODULE KxcGenDIPMW
  USE DerivedTypes
             ...
             DO NCA = 1,CS_OUT%NCells
                ...
                DO NCB = 1,CS_OUT%NCells
                   ...
                   Pair%AB2  = (Pair%A(1)-Pair%B(1))**2 &
                        + (Pair%A(2)-Pair%B(2))**2 &
                        + (Pair%A(3)-Pair%B(3))**2
                   IF(TestAtomPair(Pair,PIGWRoot%Box)) THEN
...
!                  Quick check to see if primitive touches the grid
                   IF(PExtent>Zero.AND.(.NOT.BoxOutSideBox(PBox,PIGWRoot%Box)))THEN
...
!                           Need alot more testing (leave for now unitll forces are 100%).
                            PExtent=MAX(PExtent,Extent(EllA+EllB,Prim%Zeta,HGBra%D(1:LenHG,IA,IB),TauRho,ExtraEll_O=1))
                         ENDDO
                      ENDDO
