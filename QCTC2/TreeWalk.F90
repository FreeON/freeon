...
!    USE InvExp
    ...
    DO 
       ...
       !
       PC2=Zero
       PCx=QC%Prim%P(1)-Q%Box%Center(1)
       PCy=QC%Prim%P(2)-Q%Box%Center(2)
       PCz=QC%Prim%P(3)-Q%Box%Center(3)
...
!!$       PQx=QC%Prim%P(1)-Q%Box%Center(1)
!!$       PQy=QC%Prim%P(2)-Q%Box%Center(2)
!!$       PQz=QC%Prim%P(3)-Q%Box%Center(3)
...
!!$                  Q%HERM%Zeta(EllQ)%D(1),QC%Prim%P(1),Q%HERM%Cent(EllQ)%D(1,1),       &
...
!         Q%Pole%C(0),Q%Pole%S(0),SPKetC(0),SPKetS(0))
    ...
    ENDDO
    TruMACError=Err1
    IF(ABS(TruMACError)>10D0*NewMACError)THEN
...
11     format(A10,12(2x,D12.6))
    ...
    WRITE(*,11)'SPKETC = ',SPKetC(0)
    WRITE(*,11)'SPKETS = ',SPKetS(0)
    ...
    WRITE(*,22)Q%Box%Tier,Q%Box%Number,TotEll,LOG10(TruMACError+1D-40),LOG10(NewMACError)
...
!             WRITE(*,*)'SPTemp = ',SPTempC(0:LenSP),SPTempS(0:LenSP)
             LP=P%Prim%Ell
             LQ=Ell
             LPQ=LP+LQ        
             ...
             PQ(1)=P%Prim%P(1)-Q%Herm%Cent(Ell)%D(1,I)
             PQ(2)=P%Prim%P(2)-Q%Herm%Cent(Ell)%D(2,I)
             PQ(3)=P%Prim%P(3)-Q%Herm%Cent(Ell)%D(3,I)
          ...
          DO Ell=0,Q%Herm%Ell
             DO I=1,Q%Herm%NQ(Ell)
                IF(Q%Herm%Zeta(Ell)%D(I)<1D10)THEN
                   PQx=P%Prim%P(1)-Q%Herm%Cent(Ell)%D(1,I)
                   PQy=P%Prim%P(2)-Q%Herm%Cent(Ell)%D(2,I)
                   PQz=P%Prim%P(3)-Q%Herm%Cent(Ell)%D(3,I)
                   PQ2=PQx*PQx+PQy*PQy+PQz*PQz
!                   WRITE(*,*)PQ2
