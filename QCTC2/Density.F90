...
!!$             IF(UpperT.AND.AtA==AtB)Psv%D=5D-1*Psv%D
    ...
    WRITE(*,33)DBLE(NNaive)/DBLE(NLink)
33  FORMAT('  Economization factor = ',F9.4)
    ! Next, calculate the Schwartz inequality and prune small links
    !
  END SUBROUTINE MakeRhoList
  !
  SUBROUTINE Collate(GM,RhoHead,R,P,NLink)
    TYPE(CRDS)                      :: GM
    TYPE(HGLL),POINTER              :: RhoHead,HGLink    !
    TYPE(CMPoles)                   :: P
    TYPE(HGRho)                     :: R
    ...
    REAL(DOUBLE)                    :: Zeta,ZZ,ChTot,RX,RY,RZ
...
!       WRITE(*,*)' I = ',I,' Ell = ',EllZ,' Zeta = ',Zeta,'-------------------------------'
       DO J=1,R%NQ%I(I)
          ...
          P%DPole%D=P%DPole%D+DPole(EllZ,ZZ,RX,RY,RZ,Rho%Co%D(JAdd:JAdd+LenZ-1))
...
!             31 FORMAT(I5,", ",I5,", ",I5,", ",3(", ",F14.5),", ",8(D12.6,", "))
    ...
    WRITE(*,*)' Total Charge = ',ChTot
    WRITE(*,33)ChTot,Nel
33  FORMAT('  Total Charge = ',D8.3,' NEl = ',I8)

    Mssg=ProcessName('QCTC','Density Build')
    Mssg=TRIM(Mssg)                                      &
  ...
  !
  SUBROUTINE AddNukes(GM,RhoHead)
    TYPE(CRDS)          :: GM
    TYPE(HGLL),POINTER  :: RhoHead,HGLink    !
    REAL(DOUBLE)        :: GSpike
    ...
    ENDDO
    NLink=HGLink%LNum
    GSpike=Half*(NuclearExpnt/Pi)**(ThreeHalves)
    ...
    DO N=1,GM%NAtms
       ...
       HGLink%Cent=GM%Carts%D(:,N)
       CALL New(HGLink%Coef,1)
       HGLink%Coef%D(1)=-GM%AtNum%D(N)*GSpike
    ENDDO
    NULLIFY(HGLink%Next)
