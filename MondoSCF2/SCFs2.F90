...
!===============================================================================
  ...
  FUNCTION SCFCycle(cSCF,cBAS,cGEO,N,S,O,G,M,ETot,DMax,DIIS,CPSCF_O) 
    ...
    TYPE(Parallel)   :: M
    TYPE(DBL_RNK2)   :: ETot,DMax,DIIS
    INTEGER          :: cSCF,cBAS,cGEO,iCLONE,Modl
    LOGICAL,OPTIONAL :: CPSCF_O
    LOGICAL          :: SCFCycle,DoCPSCF,DoODA,RebuildPostODA, &
                        DoMIX,RebuildPostMIX
    ...
    IF(.NOT.DoODA.AND..NOT.DoMIX)THEN
    ...
    IF(DoODA)THEN
       ...
       S%Action%C(1)=Tmp
       IF(cSCF>1)THEN
          S%Current%I=S%Current%I-(/1,0,0/)
          CALL DensityLogic(cSCF-1,cBAS,cGEO,S,O)
          CALL DensityBuild(N,S,M)
          CALL FockBuild(cSCF,cBAS,N,S,O,M)
...
!          CALL Invoke('FBuild',N,S,M)
    ...
    SCFCycle=.FALSE.!ConvergedQ(cSCF,cBAS,N,S,O,G,ETot,DMax,DIIS,CPSCF_O)
