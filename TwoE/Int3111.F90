...
! ---------------------------------------------------------- 
   SUBROUTINE Int3111(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo,I, & 
      ...
      ENDDO ! |N0) loop
      ! HRR 
      I((OA+1)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+1)*LDD)=I2Bar1
      I((OA+2)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+1)*LDD)=I3Bar1
      I((OA+3)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+1)*LDD)=I4Bar1
