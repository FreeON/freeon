! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f sp|p s) 
! ---------------------------------------------------------- 
   SUBROUTINE IntB10020301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, & 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCT10231(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|p,s)^(0) 
      CALL KetHRR31(45,HRR) 
      ! Generating (f,sp|p,s)^(0) 
      DO L=1,1
         DO K=2,4
          ...
          ENDDO 
      ENDDO 
    END SUBROUTINE IntB10020301
    SUBROUTINE CNTRCT10231(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(35,4,0:5)
      REAL(DOUBLE)  :: HRR(45,4,1)
