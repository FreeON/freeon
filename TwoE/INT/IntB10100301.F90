! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f f|p s) 
! ---------------------------------------------------------- 
   SUBROUTINE IntB10100301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, & 
      ...
      REAL(DOUBLE)  :: VRR(84,4,0:7)
      REAL(DOUBLE)  :: HRR(84,4,1)
      INTEGER       :: OffSet,OA,LDA,OB,LDB,OC,LDC,OD,LDD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(84*4,HRR(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Generating (f0|s0)^(4)
            CALL VRRf0s0(84,4,VRR(1,1,4),VRR(1,1,5))
            ! Generating (f0|s0)^(3)
            CALL VRRf0s0(84,4,VRR(1,1,3),VRR(1,1,4))
            ! Generating (f0|s0)^(2)
            CALL VRRf0s0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (f0|s0)^(1)
            CALL VRRf0s0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (f0|s0)^(0)
            CALL VRRf0s0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (g0|s0)^(3)
            CALL VRRg0s0(84,4,VRR(1,1,3),VRR(1,1,4))
            ! Generating (g0|s0)^(2)
            CALL VRRg0s0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (g0|s0)^(1)
            CALL VRRg0s0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (g0|s0)^(0)
            CALL VRRg0s0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (h0|s0)^(2)
            CALL VRRh0s0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (h0|s0)^(1)
            CALL VRRh0s0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (h0|s0)^(0)
            CALL VRRh0s0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (i0|s0)^(1)
            CALL VRRi0s0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (i0|s0)^(0)
            CALL VRRi0s0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (s0|p0)^(6)
            ...
            ! Generating (d0|p0)^(4)
            CALL VRRd0p0(84,4,VRR(1,1,4),VRR(1,1,5))
            ! Generating (d0|p0)^(3)
            CALL VRRd0p0(84,4,VRR(1,1,3),VRR(1,1,4))
            ! Generating (d0|p0)^(2)
            CALL VRRd0p0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (d0|p0)^(1)
            CALL VRRd0p0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (d0|p0)^(0)
            CALL VRRd0p0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (f0|p0)^(3)
            CALL VRRf0p0(84,4,VRR(1,1,3),VRR(1,1,4))
            ! Generating (f0|p0)^(2)
            CALL VRRf0p0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (f0|p0)^(1)
            CALL VRRf0p0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (f0|p0)^(0)
            CALL VRRf0p0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (g0|p0)^(2)
            CALL VRRg0p0(84,4,VRR(1,1,2),VRR(1,1,3))
            ! Generating (g0|p0)^(1)
            CALL VRRg0p0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (g0|p0)^(0)
            CALL VRRg0p0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (h0|p0)^(1)
            CALL VRRh0p0(84,4,VRR(1,1,1),VRR(1,1,2))
            ! Generating (h0|p0)^(0)
            CALL VRRh0p0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Generating (i0|p0)^(0)
            CALL VRRi0p0(84,4,VRR(1,1,0),VRR(1,1,1))
            ! Contracting ... 
            CALL DBLAXPY(336,HRR(1,1,1),VRR(1,1,0)) 
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|p,s)^(0) 
      CALL KetHRR31(84,HRR) 
      ! Generating (f,f|p,s)^(0) 
      DO L=1,1
         DO K=2,4
