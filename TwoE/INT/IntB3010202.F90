...
! ----------------------------------------------------------
   SUBROUTINE IntB3010202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      REAL(DOUBLE)  :: VRR(4,10,0:3)
      REAL(DOUBLE)  :: HRR(4,18,4)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Generating (s0|d0)^(1)
            CALL VRRs0d0(4,10,VRR(1,1,1),VRR(1,1,2))
            ! Generating (s0|d0)^(0)
            CALL VRRs0d0(4,10,VRR(1,1,0),VRR(1,1,1))
            ! Generating (p0|d0)^(0)
            CALL VRRp0d0(4,10,VRR(1,1,0),VRR(1,1,1))
            ! Contracting ...
            CALL CNTRCT3122(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (p,0|sp,sp)^(0)
      CALL KetHRR22(4,HRR)
      ! Generating (p,s|sp,sp)^(0)
      DO L=1,4
         DO K=1,4
            CDOffSet=(OC+K-1)*LDC+(OD+L-1)*LDD
            CALL BraHRR31(OA,OB,LDA,LDB,CDOffSet,HRR(1,K,L),INTGRL)
          ENDDO
      ENDDO
    END SUBROUTINE IntB3010202
    SUBROUTINE CNTRCT3122(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(4,10,0:3)
      REAL(DOUBLE)  :: HRR(4,18,4)
