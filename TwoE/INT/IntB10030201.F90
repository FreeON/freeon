...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f p|sp s)
! ----------------------------------------------------------
   SUBROUTINE IntB10030201(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT10321(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|sp,s)^(0)
      CALL KetHRR21(35,HRR)
      ! Generating (f,p|sp,s)^(0)
      DO L=1,1
         DO K=1,4
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB10030201
    SUBROUTINE CNTRCT10321(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(35,4,0:5)
      REAL(DOUBLE)  :: HRR(35,5,1)
