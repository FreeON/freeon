...
! ----------------------------------------------------------
   SUBROUTINE IntB3030301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT3331(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! No need to generate (p,0|p,s)^(0)
      ! Generating (p,p|p,s)^(0)
      DO L=1,1
         DO K=2,4
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB3030301
    SUBROUTINE CNTRCT3331(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(10,4,0:3)
      REAL(DOUBLE)  :: HRR(10,4,1)
