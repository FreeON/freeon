...
! ---------------------------------------------------------- 
   SUBROUTINE IntB6060301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, & 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCT6631(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (d,0|p,s)^(0) 
      CALL KetHRR31(35,HRR) 
      ! Generating (d,d|p,s)^(0) 
      DO L=1,1
         DO K=2,4
          ...
          ENDDO 
      ENDDO 
    END SUBROUTINE IntB6060301
    SUBROUTINE CNTRCT6631(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(35,4,0:5)
      REAL(DOUBLE)  :: HRR(35,4,1)
