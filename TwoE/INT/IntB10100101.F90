...
! ---------------------------------------------------------- 
   SUBROUTINE IntB10100101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, & 
      ...
      REAL(DOUBLE)  :: VRR(84,1,0:6)
      REAL(DOUBLE)  :: HRR(84,1,1)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Generating (f0|s0)^(3)
            CALL VRRf0s0(84,1,VRR(1,1,3),VRR(1,1,4))
            ! Generating (f0|s0)^(2)
            CALL VRRf0s0(84,1,VRR(1,1,2),VRR(1,1,3))
            ! Generating (f0|s0)^(1)
            CALL VRRf0s0(84,1,VRR(1,1,1),VRR(1,1,2))
            ! Generating (f0|s0)^(0)
            CALL VRRf0s0(84,1,VRR(1,1,0),VRR(1,1,1))
            ! Generating (g0|s0)^(2)
            CALL VRRg0s0(84,1,VRR(1,1,2),VRR(1,1,3))
            ! Generating (g0|s0)^(1)
            CALL VRRg0s0(84,1,VRR(1,1,1),VRR(1,1,2))
            ! Generating (g0|s0)^(0)
            CALL VRRg0s0(84,1,VRR(1,1,0),VRR(1,1,1))
            ! Generating (h0|s0)^(1)
            CALL VRRh0s0(84,1,VRR(1,1,1),VRR(1,1,2))
            ! Generating (h0|s0)^(0)
            CALL VRRh0s0(84,1,VRR(1,1,0),VRR(1,1,1))
            ! Generating (i0|s0)^(0)
            CALL VRRi0s0(84,1,VRR(1,1,0),VRR(1,1,1))
            ! Contracting ... 
            CALL DBLAXPY(84,HRR(1,1,1),VRR(1,1,0)) 
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|s,s)^(0) 
      CALL KetHRR11(84,HRR) 
      ! Generating (f,f|s,s)^(0) 
      DO L=1,1
         DO K=1,1
