...
! ----------------------------------------------------------
   SUBROUTINE IntB2010101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT2111(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! No need to generate (sp,0|s,s)^(0)
      ! Generating (sp,s|s,s)^(0)
      DO L=1,1
         DO K=1,1
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB2010101
    SUBROUTINE CNTRCT2111(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(4,1,0:1)
      REAL(DOUBLE)  :: HRR(5,1,1)
