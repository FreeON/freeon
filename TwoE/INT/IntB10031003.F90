...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f p|f p)
! ----------------------------------------------------------
   SUBROUTINE IntB10031003(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL DBLAXPY(1225,HRR(1,1,1),VRR(1,1,0))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|f,p)^(0)
      CALL KetHRR103(35,HRR)
      ! Generating (f,p|f,p)^(0)
      DO L=2,4
         DO K=11,20
