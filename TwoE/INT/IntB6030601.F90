...
! ----------------------------------------------------------
   SUBROUTINE IntB6030601(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT6361(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! No need to generate (d,0|d,s)^(0)
      ! Generating (d,p|d,s)^(0)
      DO L=1,1
         DO K=5,10
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB6030601
    SUBROUTINE CNTRCT6361(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(20,10,0:5)
      REAL(DOUBLE)  :: HRR(20,10,1)
