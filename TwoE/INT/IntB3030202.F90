...
! ----------------------------------------------------------
   SUBROUTINE IntB3030202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT3322(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (p,0|sp,sp)^(0)
      CALL KetHRR22(10,HRR)
      ! Generating (p,p|sp,sp)^(0)
      DO L=1,4
         DO K=1,4
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB3030202
    SUBROUTINE CNTRCT3322(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(10,10,0:4)
      REAL(DOUBLE)  :: HRR(10,18,4)
