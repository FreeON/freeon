...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f s|sp s)
! ----------------------------------------------------------
   SUBROUTINE IntB10010201(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ! Contracting ...
            CALL CNTRCT10121(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|sp,s)^(0)
      CALL KetHRR21(20,HRR)
      ! Generating (f,s|sp,s)^(0)
      DO L=1,1
         DO K=1,4
          ...
          ENDDO
      ENDDO
    END SUBROUTINE IntB10010201
    SUBROUTINE CNTRCT10121(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(20,4,0:4)
      REAL(DOUBLE)  :: HRR(20,5,1)
