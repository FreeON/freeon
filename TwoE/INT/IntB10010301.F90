! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|p s) 
! ---------------------------------------------------------- 
   SUBROUTINE IntB10010301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, & 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCT10131(VRR,HRR)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|p,s)^(0) 
      CALL KetHRR31(20,HRR) 
      ! Generating (f,s|p,s)^(0) 
      DO L=1,1
         DO K=2,4
          ...
          ENDDO 
      ENDDO 
    END SUBROUTINE IntB10010301
    SUBROUTINE CNTRCT10131(VRR,HRR)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: VRR(20,4,0:4)
      REAL(DOUBLE)  :: HRR(20,4,1)
