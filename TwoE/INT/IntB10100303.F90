...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f f|p p)
! ----------------------------------------------------------
   SUBROUTINE IntB10100303(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
      ...
      REAL(DOUBLE)  :: HRR(84,10,4)
      INTEGER       :: OffSet,OA,LDA,OB,LDB,OC,LDC,OD,LDD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(84*10,HRR(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      CALL KetHRR33(84,HRR)
      ! Generating (f,f|p,p)^(0)
      DO L=2,4
         DO K=2,4
