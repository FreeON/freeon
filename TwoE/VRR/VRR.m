...
If[iell+kell>0,
   ...
   WriteString[VRRSubroutine,"SUBROUTINE ",MVRRSubName,"(IXYZ,LBS,LKS,VS0,VS1,LBR,LKR,VR1)\n"];
   WriteString[VRRSubroutine,"USE DerivedTypes\nUSE VScratchB\nUSE GlobalScalars\nIMPLICIT NONE\n"];
   WriteString[VRRSubroutine,"INTEGER IXYZ,LBS,LKS,LBR,LKR\nREAL(DOUBLE) VS0(LBS,LKS),VS1(LBS,LKS),VR1(LBR,LKR)\n"];   
   WriteString[VRRSubroutine,"SELECT CASE(IXYZ)\n"];
   Do[
   WriteString[VRRSubroutine,"CASE(",ToString[ixyz],")\n"];
   Do[Do[
      (* Kount=Kount+1; *)
       a={lx[i], my[i], nz[i]}; 
       c={lx[k], my[k], nz[k]}; 
       ...
       If[iell>kell,
          If[p==1,PA="PAx";WP="WPx";one={1,0,0};two={2,0,0};];
          If[p==2,PA="PAy";WP="WPy";one={0,1,0};two={0,2,0};];
          If[p==3,PA="PAz";WP="WPz";one={0,0,1};two={0,0,2};];

          am1=a-one; 
          cm1=c-one; 
          am2=a-two; 
          cm2=c-two; 
          ia  =LMNDex[a  [[1]],a  [[2]],a  [[3]]];
          ic  =LMNDex[c  [[1]],c  [[2]],c  [[3]]];
          iam1=LMNDex[am1[[1]],am1[[2]],am1[[3]]];
          icm1=LMNDex[cm1[[1]],cm1[[2]],cm1[[3]]];
          iam2=LMNDex[am2[[1]],am2[[2]],am2[[3]]];
          icm2=LMNDex[cm2[[1]],cm2[[2]],cm2[[3]]];

          MVRR="";(*" _empty_";*)
          If[p==ixyz,MVRR=StringJoin["+r1x2Z*VR1(",ToString[iam1],",",ToString[ic],")"];];

          Ai1="";
          If[a[[p]]-1==1,Ai1="+";];
          If[a[[p]]-1 >1,Ai1=StringJoin["+",ToString[(a[[p]]-1)],"D0*"];];
          If[a[[p]]-1 >0,Ai1=StringJoin["&\n   ",Ai1,"r1x2Z*(VS0(",ToString[iam2],",",ToString[ic],")-ExZpE*VS1(",ToString[iam2],",",ToString[ic],"))"];];

          Ci1="";
          If[c[[p]]==1,Ci1="+";];
          If[c[[p]] >1,Ci1=StringJoin["+",ToString[(c[[p]])],"D0*"];];
          If[c[[p]] >0,Ci1=StringJoin["&\n   ",Ci1,"HfxZpE*VS1(",ToString[iam1],",",ToString[icm1],")"];];

          WriteString[VRRSubroutine,"VS0(",i,",",k,")=",\
                        PA,"*VS0(",ToString[iam1],",",ToString[ic],")+",\
                        WP,"*VS1(",ToString[iam1],",",ToString[ic],")",\
                        MVRR,\
                        Ai1,\
                        Ci1,\
                        "\n"];
       ,
          If[q==1,QC="QCx";WQ="WQx";one={1,0,0};two={2,0,0};];
          If[q==2,QC="QCy";WQ="WQy";one={0,1,0};two={0,2,0};];
          If[q==3,QC="QCz";WQ="WQz";one={0,0,1};two={0,0,2};];
          ...
          MVRR="";(*" _empty_";*)
          If[q==ixyz,MVRR=StringJoin["-r1x2E*VR1(",ToString[ia],",",ToString[icm1],")"];];

          Ci2="";
          If[c[[q]]-1==1,Ci2="+";];
          If[c[[q]]-1 >1,Ci2=StringJoin["+",ToString[(c[[q]]-1)],"D0*"];];
          If[c[[q]]-1 >0,Ci2=StringJoin["&\n   ",Ci2,"r1x2E*(VS0(",ToString[ia],",",ToString[icm2],")-ZxZpE*VS1(",ToString[ia],",",ToString[icm2],"))"];];

          Ai2="";
          If[a[[q]]==1,Ai2="+";];
          If[a[[q]] >1,Ai2=StringJoin["+",ToString[(a[[q]])],"D0*"];];
          If[a[[q]] >0,Ai2=StringJoin["&\n   ",Ai2,"HfxZpE*VS1(",ToString[iam1],",",ToString[icm1],")"];];

          WriteString[VRRSubroutine,"VS0(",i,",",k,")=",\
                        QC,"*VS0(",ToString[ia],",",ToString[icm1],")+",\
                        WQ,"*VS1(",ToString[ia],",",ToString[icm1],")",\
                        MVRR,\
                        Ci2,\
                        Ai2,\
                        "\n"];
    ...
    ,{ixyz,1,3}];
   WriteString[VRRSubroutine,"CASE DEFAULT\nWRITE(*,*) 'STOP IN ",MVRRSubName,"'\nSTOP\n"];
   WriteString[VRRSubroutine,"END SELECT\n"];
   WriteString[VRRSubroutine,"END SUBROUTINE ",MVRRSubName,"\n"];
...
,{kell,0,KetEll}];
          ...
          (*
          If[TotA==1&&TotC==0,
            WriteString[VRRSubroutine,"VRRS(",i,",",k,")=",\
                        PA,"*VRRS(",ToString[iam1],",",ToString[ic],")+",\
                        WP,"*VRRS(",ToString[iam1],",",ToString[ic],")",MVRR,"\n"];
           ];
           *)
