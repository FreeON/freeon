...
! ---------------------------------------------------------- 
   SUBROUTINE Int1113(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo,I, & 
      ...
      ENDDO ! |N0) loop
      ! HRR 
      I((OA+1)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+1)*LDD)=CDx*I1Bar1+I1Bar2
      I((OA+1)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+2)*LDD)=CDy*I1Bar1+I1Bar3
      I((OA+1)*LDA+(OB+1)*LDB+(OC+1)*LDC+(OD+3)*LDD)=CDz*I1Bar1+I1Bar4
