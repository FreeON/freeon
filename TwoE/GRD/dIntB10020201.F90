...
! ---------------------------------------------------------- 
SUBROUTINE dIntB10020201(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE), DIMENSION(45,5,1) :: HRR 
      REAL(DOUBLE), DIMENSION(56,5,1) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(45,10,1) :: HRRC 
      REAL(DOUBLE)  :: VRR(56,10,0:6)
      REAL(DOUBLE)  :: VRRS(45,5,0:5,3)
      REAL(DOUBLE)  :: HRRS(45,5,1,9)
      ...
      CALL InitDbl(45*10,HRRC(1,1,1))
      CALL InitDbl(9*1*45*5,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG10221(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                     & VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (<>CType[11]<>,0|sp,s)
      ! Dont need to generate (<>CType[16]<>,0|sp,s)^a
      ! Dont need to generate (<>CType[16]<>,0|sp,s)^b
      ! Dont need to generate (<>CType[11]<>,0|spd,s)^c
      ! Stress: No need to generate [f,0|sp,s] 
      DO L=1,1
      
         !K = 1
         CDOffSet=(OC+1-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',sp|1,L)  and (f,sp'|1,L)
         CALL BraHRR102ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,1,L),&
                      HRRA(1,1,L),HRRB(1,1,L),GRADIENTS(1,1),FP(1),&
                      STRESS(1,1))
         ! Generating (f,sp|1_x,L)  and (f,sp|1,L_x)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,2,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|1_y,L)  and (f,sp|1,L_y)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,3,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|1_z,L)  and (f,sp|1,L_z)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRC(1,4,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
      
         !K = 2
         CDOffSet=(OC+2-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',sp|2,L)  and (f,sp'|2,L)
         CALL BraHRR102ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,2,L),&
                      HRRA(1,2,L),HRRB(1,2,L),GRADIENTS(1,1),FP(1),&
                      STRESS(1,1))
         ! Generating (f,sp|2_x,L)  and (f,sp|2,L_x)
         HRRTmp(1:35)=HRRC(1:35,5,L)-1D0*HRR(1:35,1,L)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|2_y,L)  and (f,sp|2,L_y)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,6,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|2_z,L)  and (f,sp|2,L_z)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRC(1,8,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
      
         !K = 3
         CDOffSet=(OC+3-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',sp|3,L)  and (f,sp'|3,L)
         CALL BraHRR102ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,3,L),&
                      HRRA(1,3,L),HRRB(1,3,L),GRADIENTS(1,1),FP(1),&
                      STRESS(1,1))
         ! Generating (f,sp|3_x,L)  and (f,sp|3,L_x)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,6,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|3_y,L)  and (f,sp|3,L_y)
         HRRTmp(1:35)=HRRC(1:35,7,L)-1D0*HRR(1:35,1,L)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|3_z,L)  and (f,sp|3,L_z)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRC(1,9,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
      
         !K = 4
         CDOffSet=(OC+4-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',sp|4,L)  and (f,sp'|4,L)
         CALL BraHRR102ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,4,L),&
                      HRRA(1,4,L),HRRB(1,4,L),GRADIENTS(1,1),FP(1),&
                      STRESS(1,1))
         ! Generating (f,sp|4_x,L)  and (f,sp|4,L_x)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,8,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|4_y,L)  and (f,sp|4,L_y)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,9,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,sp|4_z,L)  and (f,sp|4,L_z)
         HRRTmp(1:35)=HRRC(1:35,10,L)-1D0*HRR(1:35,1,L)
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO 
      ! Stress: Generating (f,sp|sp,s)^(1) 
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,1
          DO K=1,4
            ...
            CALL BraHRR102(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO 
        ENDDO 
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB10020201
    SUBROUTINE CNTRCTG10221(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(56,10,0:6)
      REAL(DOUBLE)  :: VRRS(45,5,0:5,3)
      REAL(DOUBLE)  :: HRRS(45,5,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
