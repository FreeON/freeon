...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f s|s s)
! ----------------------------------------------------------
SUBROUTINE dIntB10010101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(20,1,0:3,3)
      REAL(DOUBLE)  :: HRRS(20,1,1,9)
      ...
      CALL InitDbl(9*1*20*1,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            CALL MVRRf0s0(1,20,1,VRRS(1,1,0,1),VRRS(1,1,1,1),35,4,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [f0|s0]^(0)
            CALL MVRRf0s0(2,20,1,VRRS(1,1,0,2),VRRS(1,1,1,2),35,4,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [f0|s0]^(0)
            CALL MVRRf0s0(3,20,1,VRRS(1,1,0,3),VRRS(1,1,1,3),35,4,VRR(1,1,1))
            ENDIF
            ! Contracting ...
            CALL CNTRCTG10111(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
      ...
      DO L=1,1
         ...
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,2,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,s|1_y,L)  and (f,s|1,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,3,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (f,s|1_z,L)  and (f,s|1,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRC(1,4,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO
      ! Stress: Generating (f,s|s,s)^(1)
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,1
          DO K=1,1
            ...
            CALL BraHRR101(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO
        ENDDO
      ENDDO
      ENDDO
   END SUBROUTINE dIntB10010101
    SUBROUTINE CNTRCTG10111(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(35,4,0:4)
      REAL(DOUBLE)  :: VRRS(20,1,0:3,3)
      REAL(DOUBLE)  :: HRRS(20,1,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
