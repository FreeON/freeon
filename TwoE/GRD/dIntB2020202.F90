...
! ---------------------------------------------------------- 
SUBROUTINE dIntB2020202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(28) :: HRRTmp 
      REAL(DOUBLE), DIMENSION(28,28,10) :: HRR,HRRA,HRRB,HRRC 
      REAL(DOUBLE)  :: VRR(20,20,0:5)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(28*28,HRR(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Generating (p0|s0)^(4)
            CALL VRRp0s0(20,20,VRR(1,1,4),VRR(1,1,5))
            ! Generating (p0|s0)^(3)
            CALL VRRp0s0(20,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (p0|s0)^(2)
            CALL VRRp0s0(20,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (p0|s0)^(1)
            CALL VRRp0s0(20,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (p0|s0)^(0)
            CALL VRRp0s0(20,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (d0|s0)^(3)
            CALL VRRd0s0(20,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (d0|s0)^(2)
            CALL VRRd0s0(20,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (d0|s0)^(1)
            CALL VRRd0s0(20,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (d0|s0)^(0)
            CALL VRRd0s0(20,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (f0|s0)^(2)
            ...
            ! Generating (s0|p0)^(4)
            CALL VRRs0p0(20,20,VRR(1,1,4),VRR(1,1,5))
            ! Generating (s0|p0)^(3)
            CALL VRRs0p0(20,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (s0|p0)^(2)
            CALL VRRs0p0(20,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (s0|p0)^(1)
            CALL VRRs0p0(20,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (s0|p0)^(0)
            CALL VRRs0p0(20,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (p0|p0)^(3)
            CALL VRRp0p0(20,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (p0|p0)^(2)
            CALL VRRp0p0(20,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (p0|p0)^(1)
            CALL VRRp0p0(20,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (p0|p0)^(0)
            CALL VRRp0p0(20,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (d0|p0)^(2)
            ...
            ! Contracting ... 
            CALL DBLAXPY( 400,HRR(1,1,1),       VRR(1,1,0)) 
            CALL DBLAXPZY(400,HRRA(1,1,1),Alpha,VRR(1,1,0)) 
            CALL DBLAXPZY(400,HRRB(1,1,1),Beta, VRR(1,1,0)) 
            CALL DBLAXPZY(400,HRRC(1,1,1),Gamma,VRR(1,1,0)) 
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (d,0|p,sp)
      CALL KetHRR22(18,HRR) 
      ! Generating (f,0|p,sp)^a
      CALL KetHRR22(18,HRRA) 
      ! Generating (f,0|p,sp)^b
      CALL KetHRR22(18,HRRB) 
      ! Generating (d,0|d,sp)^c
      CALL KetHRR52(18,HRRC) 
      DO L=1,4
      
         !K = 1
         CDOffSet=(OC+1-1)*LDC+(OD+L-1)*LDD
         ! Generating (sp',sp|1,L)  and (sp,sp'|1,L)
         ...
         ! Generating (sp',sp|2,L)  and (sp,sp'|2,L)
         CALL BraHRR22ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,2,L),&
                          HRRA(1,2,L),HRRB(1,2,L),GRADIENTS(1,1))
         ! Generating (sp,sp|2_x,L)  and (sp,sp|2,L_x)
         HRRTmp(1:10)=HRRC(1:10,5,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (sp,sp|2_y,L)  and (sp,sp|2,L_y)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (sp,sp|2_z,L)  and (sp,sp|2,L_z)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,8,L),GRADIENTS(1,1))
      
         !K = 3
         CDOffSet=(OC+3-1)*LDC+(OD+L-1)*LDD
         ! Generating (sp',sp|3,L)  and (sp,sp'|3,L)
         CALL BraHRR22ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,3,L),&
                          HRRA(1,3,L),HRRB(1,3,L),GRADIENTS(1,1))
         ! Generating (sp,sp|3_x,L)  and (sp,sp|3,L_x)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (sp,sp|3_y,L)  and (sp,sp|3,L_y)
         HRRTmp(1:10)=HRRC(1:10,7,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
         ! Generating (sp,sp|3_z,L)  and (sp,sp|3,L_z)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,9,L),GRADIENTS(1,1))
      
         !K = 4
         CDOffSet=(OC+4-1)*LDC+(OD+L-1)*LDD
         ! Generating (sp',sp|4,L)  and (sp,sp'|4,L)
         CALL BraHRR22ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,4,L),&
                          HRRA(1,4,L),HRRB(1,4,L),GRADIENTS(1,1))
         ! Generating (sp,sp|4_x,L)  and (sp,sp|4,L_x)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,8,L),GRADIENTS(1,1))
         ! Generating (sp,sp|4_y,L)  and (sp,sp|4,L_y)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,9,L),GRADIENTS(1,1))
         ! Generating (sp,sp|4_z,L)  and (sp,sp|4,L_z)
         HRRTmp(1:10)=HRRC(1:10,10,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB2020202
