...
SUBROUTINE dIntB2020202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(18,18,0:4,3)
      REAL(DOUBLE)  :: HRRS(18,18,4,9)
      ...
      CALL InitDbl(9*4*18*18,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      ! Stress: Generating [sp,0|sp,sp] 
      DO IJ=1,9
        CALL KetHRR22(18,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
         ...
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,2,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (sp,sp|1_y,L)  and (sp,sp|1,L_y)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,3,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (sp,sp|1_z,L)  and (sp,sp|1,L_z)
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
         ...
         CALL BraHRR22cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO 
      ! Stress: Generating (sp,sp|sp,sp)^(1) 
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,4
          DO K=1,4
            ...
            CALL BraHRR22(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO 
        ENDDO 
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB2020202
    SUBROUTINE CNTRCTG2222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(20,20,0:5)
      REAL(DOUBLE)  :: VRRS(18,18,0:4,3)
      REAL(DOUBLE)  :: HRRS(18,18,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
