! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f sp|p p) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10020303(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE), DIMENSION(45,10,4) :: HRR 
      ...
      CALL InitDbl(9*4*45*10,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG10233(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                     & VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (<>CType[11]<>,0|p,p)
      CALL KetHRR33(45,HRR) 
      ! Generating (<>CType[16]<>,0|p,p)^a
      CALL KetHRR33(56,HRRA) 
      ! Generating (<>CType[16]<>,0|p,p)^b
      CALL KetHRR33(56,HRRB) 
      ! Generating (<>CType[11]<>,0|d,p)^c
      CALL KetHRR63(45,HRRC) 
      ! Stress: Generating [f,0|p,p] 
      DO IJ=1,9
        CALL KetHRR33(45,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=2,4
      ...
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB10020303
    SUBROUTINE CNTRCTG10233(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(56,20,0:7)
      REAL(DOUBLE)  :: VRRS(45,10,0:6,3)
      REAL(DOUBLE)  :: HRRS(45,10,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
