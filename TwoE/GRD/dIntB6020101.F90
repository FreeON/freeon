...
SUBROUTINE dIntB6020101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(26,1,0:3,3)
      REAL(DOUBLE)  :: HRRS(26,1,1,9)
      ...
      CALL InitDbl(9*1*26*1,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO L=1,1
         ...
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,2,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (d,sp|1_y,L)  and (d,sp|1,L_y)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,3,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (d,sp|1_z,L)  and (d,sp|1,L_z)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRC(1,4,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO 
      ! Stress: Generating (d,sp|s,s)^(1) 
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,1
          DO K=1,1
            ...
            CALL BraHRR62(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO 
        ENDDO 
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB6020101
    SUBROUTINE CNTRCTG6211(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(35,4,0:4)
      REAL(DOUBLE)  :: VRRS(26,1,0:3,3)
      REAL(DOUBLE)  :: HRRS(26,1,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
