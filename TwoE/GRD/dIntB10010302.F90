! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|p sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10010302(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(20,13,4) :: HRR 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG10132(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|p,sp)
      CALL KetHRR32(20,HRR) 
      ! Generating (g,0|p,sp)^a
      CALL KetHRR32(35,HRRA) 
      ! Generating (g,0|p,sp)^b
      CALL KetHRR32(35,HRRB) 
      ! Generating (f,0|d,sp)^c
      CALL KetHRR62(20,HRRC) 
      DO L=1,4
      ...
      ENDDO 
    END SUBROUTINE dIntB10010302
    SUBROUTINE CNTRCTG10132(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
