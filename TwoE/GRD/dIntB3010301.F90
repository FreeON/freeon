...
SUBROUTINE dIntB3010301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (sp,0|p,s)
      ! Dont need to generate (spd,0|p,s)^a
      ! Dont need to generate (spd,0|p,s)^b
      ! Dont need to generate (sp,0|<>CType[5]<>,s)^c
      DO L=1,1
         ...
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
    ...
    SUBROUTINE CNTRCTG3131(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
