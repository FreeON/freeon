 ...
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG3232(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ...
      DO L=1,4
         ...
         CALL BraHRR32ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,2,L),&
                          HRRA(1,2,L),HRRB(1,2,L),GRADIENTS(1,1))
         ! Generating (p,sp|2_x,L)  and (p,sp|2,L_x)
         HRRTmp(1:10)=HRRC(1:10,5,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (p,sp|2_y,L)  and (p,sp|2,L_y)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (p,sp|2_z,L)  and (p,sp|2,L_z)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,8,L),GRADIENTS(1,1))
      
         !K = 3
         CDOffSet=(OC+3-2)*LDC+(OD+L-1)*LDD
         ! Generating (p',sp|3,L)  and (p,sp'|3,L)
         CALL BraHRR32ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,3,L),&
                          HRRA(1,3,L),HRRB(1,3,L),GRADIENTS(1,1))
         ! Generating (p,sp|3_x,L)  and (p,sp|3,L_x)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (p,sp|3_y,L)  and (p,sp|3,L_y)
         HRRTmp(1:10)=HRRC(1:10,7,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (p,sp|3_z,L)  and (p,sp|3,L_z)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,9,L),GRADIENTS(1,1))
      
         !K = 4
         CDOffSet=(OC+4-2)*LDC+(OD+L-1)*LDD
         ! Generating (p',sp|4,L)  and (p,sp'|4,L)
         CALL BraHRR32ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,4,L),&
                          HRRA(1,4,L),HRRB(1,4,L),GRADIENTS(1,1))
         ! Generating (p,sp|4_x,L)  and (p,sp|4,L_x)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,8,L),GRADIENTS(1,1))
         ! Generating (p,sp|4_y,L)  and (p,sp|4,L_y)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,9,L),GRADIENTS(1,1))
         ! Generating (p,sp|4_z,L)  and (p,sp|4,L_z)
         HRRTmp(1:10)=HRRC(1:10,10,L)-1D0*HRR(1:10,1,L)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB3020302
    SUBROUTINE CNTRCTG3232(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
      REAL(DOUBLE), DIMENSION(13,13,4) :: HRR 
      ...
      HRR(11,11,1)=HRR(11,11,1)+FnSpB*FnSpK*VRR(2,2,0)
      HRR(11,2,1)=HRR(11,2,1)+FnSpB*VRR(2,2,0)
      HRR(2,11,1)=HRR(2,11,1)+FnSpK*VRR(2,2,0)
      HRR(11,12,1)=HRR(11,12,1)+FnSpB*FnSpK*VRR(2,3,0)
      HRR(11,3,1)=HRR(11,3,1)+FnSpB*VRR(2,3,0)
      HRR(2,12,1)=HRR(2,12,1)+FnSpK*VRR(2,3,0)
      HRR(11,13,1)=HRR(11,13,1)+FnSpB*FnSpK*VRR(2,4,0)
      HRR(11,4,1)=HRR(11,4,1)+FnSpB*VRR(2,4,0)
      HRR(2,13,1)=HRR(2,13,1)+FnSpK*VRR(2,4,0)
      HRR(11,5,1)=HRR(11,5,1)+FnSpB*VRR(2,5,0)
      HRR(11,6,1)=HRR(11,6,1)+FnSpB*VRR(2,6,0)
      HRR(11,7,1)=HRR(11,7,1)+FnSpB*VRR(2,7,0)
      HRR(11,8,1)=HRR(11,8,1)+FnSpB*VRR(2,8,0)
      HRR(11,9,1)=HRR(11,9,1)+FnSpB*VRR(2,9,0)
      HRR(11,10,1)=HRR(11,10,1)+FnSpB*VRR(2,10,0)
      HRR(11,11,1)=HRR(11,11,1)+FnSpB*VRR(2,11,0)
      HRR(11,12,1)=HRR(11,12,1)+FnSpB*VRR(2,12,0)
      HRR(11,13,1)=HRR(11,13,1)+FnSpB*VRR(2,13,0)
      HRR(11,14,1)=HRR(11,14,1)+FnSpB*VRR(2,14,0)
      HRR(11,15,1)=HRR(11,15,1)+FnSpB*VRR(2,15,0)
      HRR(11,16,1)=HRR(11,16,1)+FnSpB*VRR(2,16,0)
      HRR(11,17,1)=HRR(11,17,1)+FnSpB*VRR(2,17,0)
      HRR(11,18,1)=HRR(11,18,1)+FnSpB*VRR(2,18,0)
      HRR(11,19,1)=HRR(11,19,1)+FnSpB*VRR(2,19,0)
      HRR(11,20,1)=HRR(11,20,1)+FnSpB*VRR(2,20,0)
      HRR(12,1,1)=HRR(12,1,1)+FnSpB*VRR(3,1,0)
      HRR(12,11,1)=HRR(12,11,1)+FnSpB*FnSpK*VRR(3,2,0)
      HRR(12,2,1)=HRR(12,2,1)+FnSpB*VRR(3,2,0)
      HRR(3,11,1)=HRR(3,11,1)+FnSpK*VRR(3,2,0)
      HRR(12,12,1)=HRR(12,12,1)+FnSpB*FnSpK*VRR(3,3,0)
      HRR(12,3,1)=HRR(12,3,1)+FnSpB*VRR(3,3,0)
      HRR(3,12,1)=HRR(3,12,1)+FnSpK*VRR(3,3,0)
      HRR(12,13,1)=HRR(12,13,1)+FnSpB*FnSpK*VRR(3,4,0)
      HRR(12,4,1)=HRR(12,4,1)+FnSpB*VRR(3,4,0)
      HRR(3,13,1)=HRR(3,13,1)+FnSpK*VRR(3,4,0)
      HRR(12,5,1)=HRR(12,5,1)+FnSpB*VRR(3,5,0)
      HRR(12,6,1)=HRR(12,6,1)+FnSpB*VRR(3,6,0)
      HRR(12,7,1)=HRR(12,7,1)+FnSpB*VRR(3,7,0)
      HRR(12,8,1)=HRR(12,8,1)+FnSpB*VRR(3,8,0)
      HRR(12,9,1)=HRR(12,9,1)+FnSpB*VRR(3,9,0)
      HRR(12,10,1)=HRR(12,10,1)+FnSpB*VRR(3,10,0)
      HRR(12,11,1)=HRR(12,11,1)+FnSpB*VRR(3,11,0)
      HRR(12,12,1)=HRR(12,12,1)+FnSpB*VRR(3,12,0)
      HRR(12,13,1)=HRR(12,13,1)+FnSpB*VRR(3,13,0)
      HRR(12,14,1)=HRR(12,14,1)+FnSpB*VRR(3,14,0)
      HRR(12,15,1)=HRR(12,15,1)+FnSpB*VRR(3,15,0)
      HRR(12,16,1)=HRR(12,16,1)+FnSpB*VRR(3,16,0)
      HRR(12,17,1)=HRR(12,17,1)+FnSpB*VRR(3,17,0)
      HRR(12,18,1)=HRR(12,18,1)+FnSpB*VRR(3,18,0)
      HRR(12,19,1)=HRR(12,19,1)+FnSpB*VRR(3,19,0)
      HRR(12,20,1)=HRR(12,20,1)+FnSpB*VRR(3,20,0)
      HRR(13,1,1)=HRR(13,1,1)+FnSpB*VRR(4,1,0)
      HRR(13,11,1)=HRR(13,11,1)+FnSpB*FnSpK*VRR(4,2,0)
      HRR(13,2,1)=HRR(13,2,1)+FnSpB*VRR(4,2,0)
      HRR(4,11,1)=HRR(4,11,1)+FnSpK*VRR(4,2,0)
      HRR(13,12,1)=HRR(13,12,1)+FnSpB*FnSpK*VRR(4,3,0)
      HRR(13,3,1)=HRR(13,3,1)+FnSpB*VRR(4,3,0)
      HRR(4,12,1)=HRR(4,12,1)+FnSpK*VRR(4,3,0)
      HRR(13,13,1)=HRR(13,13,1)+FnSpB*FnSpK*VRR(4,4,0)
      HRR(13,4,1)=HRR(13,4,1)+FnSpB*VRR(4,4,0)
      HRR(4,13,1)=HRR(4,13,1)+FnSpK*VRR(4,4,0)
      HRR(13,5,1)=HRR(13,5,1)+FnSpB*VRR(4,5,0)
      HRR(13,6,1)=HRR(13,6,1)+FnSpB*VRR(4,6,0)
      HRR(13,7,1)=HRR(13,7,1)+FnSpB*VRR(4,7,0)
      HRR(13,8,1)=HRR(13,8,1)+FnSpB*VRR(4,8,0)
      HRR(13,9,1)=HRR(13,9,1)+FnSpB*VRR(4,9,0)
      HRR(13,10,1)=HRR(13,10,1)+FnSpB*VRR(4,10,0)
      HRR(13,11,1)=HRR(13,11,1)+FnSpB*VRR(4,11,0)
      HRR(13,12,1)=HRR(13,12,1)+FnSpB*VRR(4,12,0)
      HRR(13,13,1)=HRR(13,13,1)+FnSpB*VRR(4,13,0)
      HRR(13,14,1)=HRR(13,14,1)+FnSpB*VRR(4,14,0)
      HRR(13,15,1)=HRR(13,15,1)+FnSpB*VRR(4,15,0)
      HRR(13,16,1)=HRR(13,16,1)+FnSpB*VRR(4,16,0)
      HRR(13,17,1)=HRR(13,17,1)+FnSpB*VRR(4,17,0)
      HRR(13,18,1)=HRR(13,18,1)+FnSpB*VRR(4,18,0)
      HRR(13,19,1)=HRR(13,19,1)+FnSpB*VRR(4,19,0)
      HRR(13,20,1)=HRR(13,20,1)+FnSpB*VRR(4,20,0)
