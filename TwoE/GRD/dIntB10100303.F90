...
! ----------------------------------------------------------
SUBROUTINE dIntB10100303(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE), DIMENSION(84,10,4) :: HRR
      REAL(DOUBLE), DIMENSION(120,10,4) :: HRRA,HRRB
      REAL(DOUBLE), DIMENSION(84,20,4) :: HRRC
      REAL(DOUBLE)  :: VRR(120,20,0:9)
      REAL(DOUBLE)  :: VRRS(84,10,0:8,3)
      REAL(DOUBLE)  :: HRRS(84,10,4,9)
      ...
      CALL InitDbl(9*4*84*10,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ENDIF
            ! Contracting ...
            CALL CNTRCTG101033(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                       VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (i,0|p,p)
      CALL KetHRR33(84,HRR)
      ! Generating (j,0|p,p)^a
      CALL KetHRR33(120,HRRA)
      ! Generating (j,0|p,p)^b
      CALL KetHRR33(120,HRRB)
      ! Generating (i,0|d,p)^c
      CALL KetHRR63(84,HRRC)
      ! Stress: Generating [f,0|p,p]
      DO IJ=1,9
        CALL KetHRR33(84,HRRS(1,1,1,IJ))
      ENDDO
      DO L=2,4
      ...
      ENDDO
      ENDDO
   END SUBROUTINE dIntB10100303
    SUBROUTINE CNTRCTG101033(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(120,20,0:9)
      REAL(DOUBLE)  :: VRRS(84,10,0:8,3)
      REAL(DOUBLE)  :: HRRS(84,10,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
