! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (sp s|s s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB2010101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(5,1,0:1,3)
      REAL(DOUBLE)  :: HRRS(5,1,1,9)
      ...
      CALL InitDbl(9*1*5*1,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG2111(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
      ...
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB2010101
    SUBROUTINE CNTRCTG2111(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(10,4,0:2)
      REAL(DOUBLE)  :: VRRS(5,1,0:1,3)
      REAL(DOUBLE)  :: HRRS(5,1,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
