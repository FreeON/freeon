! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10010202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(20,18,4) :: HRR 
      REAL(DOUBLE), DIMENSION(35,18,4) :: HRRA,HRRB 
      ...
      EXTERNAL InitDbl
      CALL InitDbl(20*18,HRR(1,1,1))
      CALL InitDbl(35*18,HRRA(1,1,1))
      CALL InitDbl(35*18,HRRB(1,1,1))
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG10122(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|sp,sp)
      CALL KetHRR22(20,HRR) 
      ! Generating (g,0|sp,sp)^a
      CALL KetHRR22(35,HRRA) 
      ! Generating (g,0|sp,sp)^b
      CALL KetHRR22(35,HRRB) 
      ! Generating (f,0|spd,sp)^c
      CALL KetHRR42(20,HRRC) 
      DO L=1,4
      
         !K = 1
         CDOffSet=(OC+1-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|1,L)  and (f,s'|1,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,1,L),&
                          HRRA(1,1,L),HRRB(1,1,L),GRADIENTS(1,1))
         ! Generating (f,s|1_x,L)  and (f,s|1,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,2,L),GRADIENTS(1,1))
         ! Generating (f,s|1_y,L)  and (f,s|1,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,3,L),GRADIENTS(1,1))
         ! Generating (f,s|1_z,L)  and (f,s|1,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,4,L),GRADIENTS(1,1))
      
         !K = 2
         CDOffSet=(OC+2-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|2,L)  and (f,s'|2,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,2,L),&
                          HRRA(1,2,L),HRRB(1,2,L),GRADIENTS(1,1))
         ! Generating (f,s|2_x,L)  and (f,s|2,L_x)
         ...
         ! Generating (f,s|2_y,L)  and (f,s|2,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (f,s|2_z,L)  and (f,s|2,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,8,L),GRADIENTS(1,1))
      
         !K = 3
         CDOffSet=(OC+3-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|3,L)  and (f,s'|3,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,3,L),&
                          HRRA(1,3,L),HRRB(1,3,L),GRADIENTS(1,1))
         ! Generating (f,s|3_x,L)  and (f,s|3,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (f,s|3_y,L)  and (f,s|3,L_y)
         ...
         ! Generating (f,s|3_z,L)  and (f,s|3,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,9,L),GRADIENTS(1,1))
      
         !K = 4
         CDOffSet=(OC+4-1)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|4,L)  and (f,s'|4,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,4,L),&
                          HRRA(1,4,L),HRRB(1,4,L),GRADIENTS(1,1))
         ! Generating (f,s|4_x,L)  and (f,s|4,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,8,L),GRADIENTS(1,1))
         ! Generating (f,s|4_y,L)  and (f,s|4,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,9,L),GRADIENTS(1,1))
         ! Generating (f,s|4_z,L)  and (f,s|4,L_z)
      ...
      ENDDO 
    END SUBROUTINE dIntB10010202
    SUBROUTINE CNTRCTG10122(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
