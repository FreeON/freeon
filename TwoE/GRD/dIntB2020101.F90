! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (sp sp|s s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB2020101(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG2211(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
      ...
      ! Stress: No need to generate [sp,0|s,s] 
      DO L=1,1
   ...
   END SUBROUTINE dIntB2020101
    SUBROUTINE CNTRCTG2211(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(20,4,0:3)
      REAL(DOUBLE)  :: VRRS(18,1,0:2,3)
      REAL(DOUBLE)  :: HRRS(18,1,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
