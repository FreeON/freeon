! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (p s|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB3010202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(4,18,4) :: HRR 
      REAL(DOUBLE), DIMENSION(10,18,4) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(4,20,4) :: HRRC 
      REAL(DOUBLE)  :: VRR(10,20,0:4)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(4*18,HRR(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Generating (p0|s0)^(3)
            CALL VRRp0s0(10,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (p0|s0)^(2)
            CALL VRRp0s0(10,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (p0|s0)^(1)
            CALL VRRp0s0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (p0|s0)^(0)
            CALL VRRp0s0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (d0|s0)^(2)
            CALL VRRd0s0(10,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (d0|s0)^(1)
            CALL VRRd0s0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (d0|s0)^(0)
            CALL VRRd0s0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (s0|p0)^(3)
            CALL VRRs0p0(10,20,VRR(1,1,3),VRR(1,1,4))
            ! Generating (s0|p0)^(2)
            CALL VRRs0p0(10,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (s0|p0)^(1)
            CALL VRRs0p0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (s0|p0)^(0)
            CALL VRRs0p0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (p0|p0)^(2)
            CALL VRRp0p0(10,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (p0|p0)^(1)
            CALL VRRp0p0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (p0|p0)^(0)
            CALL VRRp0p0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (d0|p0)^(1)
            CALL VRRd0p0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (d0|p0)^(0)
            CALL VRRd0p0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (s0|d0)^(2)
            CALL VRRs0d0(10,20,VRR(1,1,2),VRR(1,1,3))
            ! Generating (s0|d0)^(1)
            CALL VRRs0d0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (s0|d0)^(0)
            CALL VRRs0d0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (p0|d0)^(1)
            CALL VRRp0d0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (p0|d0)^(0)
            CALL VRRp0d0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (d0|d0)^(0)
            CALL VRRd0d0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (s0|f0)^(1)
            CALL VRRs0f0(10,20,VRR(1,1,1),VRR(1,1,2))
            ! Generating (s0|f0)^(0)
            CALL VRRs0f0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Generating (p0|f0)^(0)
            CALL VRRp0f0(10,20,VRR(1,1,0),VRR(1,1,1))
            ! Contracting ... 
            CALL DBLAXPY(40,HRR(1,1,1),VRR(1,1,0)) 
            CALL DBLAXPZY(100,HRRA(1,1,1),Alpha,VRR(1,1,0)) 
            CALL DBLAXPZY(100,HRRB(1,1,1),Beta, VRR(1,1,0)) 
            CALL DBLAXPZY(200,HRRC(1,1,1),Gamma,VRR(1,1,0)) 
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (p,0|sp,sp)
      ! Dont need to generate (d,0|sp,sp)^a
      ! Dont need to generate (d,0|sp,sp)^b
      ! Dont need to generate (p,0|spd,sp)^c
      ...
      DO L=1,4
         ...
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (p,s|2_y,L)  and (p,s|2,L_y)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (p,s|2_z,L)  and (p,s|2,L_z)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,8,L),GRADIENTS(1,1))
         ...
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,6,L),GRADIENTS(1,1))
         ! Generating (p,s|3_y,L)  and (p,s|3,L_y)
         HRRTmp(1:4)=HRRC(1:4,7,L)-1D0*HRR(1:4,1,L)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
         ! Generating (p,s|3_z,L)  and (p,s|3,L_z)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,9,L),GRADIENTS(1,1))
         ...
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,8,L),GRADIENTS(1,1))
         ! Generating (p,s|4_y,L)  and (p,s|4,L_y)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,9,L),GRADIENTS(1,1))
         ! Generating (p,s|4_z,L)  and (p,s|4,L_z)
         HRRTmp(1:4)=HRRC(1:4,10,L)-1D0*HRR(1:4,1,L)
         CALL BraHRR31cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB3010202
