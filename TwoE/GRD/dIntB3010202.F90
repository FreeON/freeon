...
SUBROUTINE dIntB3010202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(4,18,0:3,3)
      REAL(DOUBLE)  :: HRRS(4,18,4,9)
      ...
      CALL InitDbl(9*4*4*18,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            CALL MVRRp0d0(1,4,10,VRRS(1,1,0,1),VRRS(1,1,1,1),10,20,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [p0|d0]^(0)
            CALL MVRRp0d0(2,4,10,VRRS(1,1,0,2),VRRS(1,1,1,2),10,20,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [p0|d0]^(0)
            CALL MVRRp0d0(3,4,10,VRRS(1,1,0,3),VRRS(1,1,1,3),10,20,VRR(1,1,1))
            ENDIF
            ! Contracting ... 
      ...
      ! Stress: Generating [p,0|sp,sp] 
      DO IJ=1,9
        CALL KetHRR22(4,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
   ...
   END SUBROUTINE dIntB3010202
    SUBROUTINE CNTRCTG3122(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(10,20,0:4)
      REAL(DOUBLE)  :: VRRS(4,18,0:3,3)
      REAL(DOUBLE)  :: HRRS(4,18,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
