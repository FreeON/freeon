! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|p p) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10010303(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(20,16,4) :: HRR 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG10133(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (f,0|p,p)
      ...
      ! Generating (f,0|<>CType[5]<>,p)^c
      CALL KetHRR53(20,HRRC) 
      DO L=2,4
      ...
      ENDDO 
    END SUBROUTINE dIntB10010303
    SUBROUTINE CNTRCTG10133(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
