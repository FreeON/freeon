! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (d sp|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB6020202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(26,18,4) :: HRR 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG6222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (spdf,0|sp,sp)
      CALL KetHRR22(26,HRR) 
      ! Generating (<>CType[11]<>,0|sp,sp)^a
      CALL KetHRR22(35,HRRA) 
      ! Generating (<>CType[11]<>,0|sp,sp)^b
      CALL KetHRR22(35,HRRB) 
      ! Generating (spdf,0|spd,sp)^c
      CALL KetHRR42(26,HRRC) 
      DO L=1,4
      
         !K = 1
         CDOffSet=(OC+1-1)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|1,L)  and (d,sp'|1,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,1,L),&
                          HRRA(1,1,L),HRRB(1,1,L),GRADIENTS(1,1))
         ! Generating (d,sp|1_x,L)  and (d,sp|1,L_x)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,2,L),GRADIENTS(1,1))
         ! Generating (d,sp|1_y,L)  and (d,sp|1,L_y)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,3,L),GRADIENTS(1,1))
         ! Generating (d,sp|1_z,L)  and (d,sp|1,L_z)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,4,L),GRADIENTS(1,1))
      ...
      ENDDO 
    END SUBROUTINE dIntB6020202
    SUBROUTINE CNTRCTG6222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
