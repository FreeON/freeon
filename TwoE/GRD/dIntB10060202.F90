! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f d|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10060202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE), DIMENSION(56,18,4) :: HRR 
      REAL(DOUBLE), DIMENSION(84,18,4) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(56,20,4) :: HRRC 
      REAL(DOUBLE)  :: VRR(84,20,0:8)
      REAL(DOUBLE)  :: VRRS(56,18,0:7,3)
      REAL(DOUBLE)  :: HRRS(56,18,4,9)
      REAL(DOUBLE)  :: TOm,PQJ(3),FP(9)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L,IJ
      EXTERNAL InitDbl
      CALL InitDbl(56*18,HRR(1,1,1))
      CALL InitDbl(84*18,HRRA(1,1,1))
      CALL InitDbl(84*18,HRRB(1,1,1))
      CALL InitDbl(56*20,HRRC(1,1,1))
      CALL InitDbl(9*4*56*18,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG10622(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                     & VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (h,0|sp,sp)
      CALL KetHRR22(56,HRR) 
      ! Generating (i,0|sp,sp)^a
      CALL KetHRR22(84,HRRA) 
      ! Generating (i,0|sp,sp)^b
      CALL KetHRR22(84,HRRB) 
      ! Generating (h,0|spd,sp)^c
      CALL KetHRR42(56,HRRC) 
      ! Stress: Generating [f,0|sp,sp] 
      DO IJ=1,9
        CALL KetHRR22(56,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
      ...
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB10060202
    SUBROUTINE CNTRCTG10622(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(84,20,0:8)
      REAL(DOUBLE)  :: VRRS(56,18,0:7,3)
      REAL(DOUBLE)  :: HRRS(56,18,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
