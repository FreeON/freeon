! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (p s|sp s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB3010201(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(4,5,1) :: HRR 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG3121(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (p,0|sp,s)
      ! Dont need to generate (d,0|sp,s)^a
      ! Dont need to generate (d,0|sp,s)^b
      ! Dont need to generate (p,0|spd,s)^c
      DO L=1,1
      ...
      ENDDO 
    END SUBROUTINE dIntB3010201
    SUBROUTINE CNTRCTG3121(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
      REAL(DOUBLE), DIMENSION(4,5,1) :: HRR 
