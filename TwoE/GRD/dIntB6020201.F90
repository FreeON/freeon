! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (d sp|sp s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB6020201(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(26,5,1) :: HRR 
      REAL(DOUBLE), DIMENSION(35,5,1) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(26,10,1) :: HRRC 
      REAL(DOUBLE)  :: VRR(35,10,0:5)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(26*5,HRR(1,1,1))
      CALL InitDbl(35*5,HRRA(1,1,1))
      CALL InitDbl(35*5,HRRB(1,1,1))
      CALL InitDbl(26*10,HRRC(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (spdf,0|sp,s)
      ! Dont need to generate (<>CType[11]<>,0|sp,s)^a
      ! Dont need to generate (<>CType[11]<>,0|sp,s)^b
      ! Dont need to generate (spdf,0|spd,s)^c
      DO L=1,1
