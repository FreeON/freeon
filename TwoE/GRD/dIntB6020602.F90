...
! ---------------------------------------------------------- 
SUBROUTINE dIntB6020602(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(26,26,4) :: HRR 
      REAL(DOUBLE), DIMENSION(35,26,4) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(26,35,4) :: HRRC 
      REAL(DOUBLE)  :: VRR(35,35,0:7)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(26*26,HRR(1,1,1))
      CALL InitDbl(35*26,HRRA(1,1,1))
      CALL InitDbl(35*26,HRRB(1,1,1))
      CALL InitDbl(26*35,HRRC(1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG6262(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (spdf,0|d,sp)
      CALL KetHRR62(26,HRR) 
      ! Generating (<>CType[11]<>,0|d,sp)^a
      CALL KetHRR62(35,HRRA) 
      ! Generating (<>CType[11]<>,0|d,sp)^b
      CALL KetHRR62(35,HRRB) 
      ! Generating (spdf,0|f,sp)^c
      CALL KetHRR102(26,HRRC) 
      DO L=1,4
         ...
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB6020602
    SUBROUTINE CNTRCTG6262(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
