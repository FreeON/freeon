! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (p p|p sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB3030302(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      ENDDO ! |N0) loop
      ! Generating (d,0|p,sp)
      CALL KetHRR32(10,HRR) 
      ! Generating (f,0|p,sp)^a
      CALL KetHRR32(10,HRRA) 
      ! Generating (f,0|p,sp)^b
      CALL KetHRR32(10,HRRB) 
      ! Generating (d,0|d,sp)^c
      CALL KetHRR62(10,HRRC) 
      DO L=1,4
