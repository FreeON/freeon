...
SUBROUTINE dIntB3030302(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(10,13,0:4,3)
      REAL(DOUBLE)  :: HRRS(10,13,4,9)
      ...
      CALL InitDbl(9*4*10*13,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG3332(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
      ...
      ! Stress: Generating [p,0|p,sp] 
      DO IJ=1,9
        CALL KetHRR32(10,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
   ...
   END SUBROUTINE dIntB3030302
    SUBROUTINE CNTRCTG3332(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(20,20,0:5)
      REAL(DOUBLE)  :: VRRS(10,13,0:4,3)
      REAL(DOUBLE)  :: HRRS(10,13,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
