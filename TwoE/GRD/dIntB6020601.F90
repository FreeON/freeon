! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (d sp|d s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB6020601(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(26,10,1) :: HRR 
      REAL(DOUBLE), DIMENSION(35,10,1) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(26,20,1) :: HRRC 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG6261(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (spdf,0|d,s)
      ! Dont need to generate (<>CType[11]<>,0|d,s)^a
      ! Dont need to generate (<>CType[11]<>,0|d,s)^b
      ! Dont need to generate (spdf,0|f,s)^c
      DO L=1,1
      
         !K = 5
         CDOffSet=(OC+5-5)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|5,L)  and (d,sp'|5,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,5,L),&
                          HRRA(1,5,L),HRRB(1,5,L),GRADIENTS(1,1))
         ! Generating (d,sp|5_x,L)  and (d,sp|5,L_x)
         ...
         ! Generating (d,sp|5_y,L)  and (d,sp|5,L_y)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,12,L),GRADIENTS(1,1))
         ! Generating (d,sp|5_z,L)  and (d,sp|5,L_z)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,15,L),GRADIENTS(1,1))
      
         !K = 6
         CDOffSet=(OC+6-5)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|6,L)  and (d,sp'|6,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,6,L),&
                          HRRA(1,6,L),HRRB(1,6,L),GRADIENTS(1,1))
         ! Generating (d,sp|6_x,L)  and (d,sp|6,L_x)
         HRRTmp(1:20)=HRRC(1:20,12,L)-1D0*HRR(1:20,3,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (d,sp|6_y,L)  and (d,sp|6,L_y)
         ...
         ! Generating (d,sp|6_z,L)  and (d,sp|6,L_z)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,16,L),GRADIENTS(1,1))
      
         !K = 7
         CDOffSet=(OC+7-5)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|7,L)  and (d,sp'|7,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,7,L),&
                          HRRA(1,7,L),HRRB(1,7,L),GRADIENTS(1,1))
         ! Generating (d,sp|7_x,L)  and (d,sp|7,L_x)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,13,L),GRADIENTS(1,1))
         ! Generating (d,sp|7_y,L)  and (d,sp|7,L_y)
         HRRTmp(1:20)=HRRC(1:20,14,L)-2D0*HRR(1:20,3,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (d,sp|7_z,L)  and (d,sp|7,L_z)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,17,L),GRADIENTS(1,1))
      
         !K = 8
         CDOffSet=(OC+8-5)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|8,L)  and (d,sp'|8,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,8,L),&
                          HRRA(1,8,L),HRRB(1,8,L),GRADIENTS(1,1))
         ! Generating (d,sp|8_x,L)  and (d,sp|8,L_x)
         HRRTmp(1:20)=HRRC(1:20,15,L)-1D0*HRR(1:20,4,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (d,sp|8_y,L)  and (d,sp|8,L_y)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,16,L),GRADIENTS(1,1))
         ! Generating (d,sp|8_z,L)  and (d,sp|8,L_z)
         ...
         ! Generating (d',sp|9,L)  and (d,sp'|9,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,9,L),&
                          HRRA(1,9,L),HRRB(1,9,L),GRADIENTS(1,1))
         ! Generating (d,sp|9_x,L)  and (d,sp|9,L_x)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,16,L),GRADIENTS(1,1))
         ! Generating (d,sp|9_y,L)  and (d,sp|9,L_y)
         HRRTmp(1:20)=HRRC(1:20,17,L)-1D0*HRR(1:20,4,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (d,sp|9_z,L)  and (d,sp|9,L_z)
         HRRTmp(1:20)=HRRC(1:20,19,L)-1D0*HRR(1:20,3,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 10
         CDOffSet=(OC+10-5)*LDC+(OD+L-1)*LDD
         ! Generating (d',sp|10,L)  and (d,sp'|10,L)
         CALL BraHRR62ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,10,L),&
                          HRRA(1,10,L),HRRB(1,10,L),GRADIENTS(1,1))
         ! Generating (d,sp|10_x,L)  and (d,sp|10,L_x)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,18,L),GRADIENTS(1,1))
         ! Generating (d,sp|10_y,L)  and (d,sp|10,L_y)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,19,L),GRADIENTS(1,1))
         ! Generating (d,sp|10_z,L)  and (d,sp|10,L_z)
         HRRTmp(1:20)=HRRC(1:20,20,L)-2D0*HRR(1:20,4,L)
         CALL BraHRR62cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB6020601
    SUBROUTINE CNTRCTG6261(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
