...
SUBROUTINE dIntB3030301(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(10,4,0:3,3)
      REAL(DOUBLE)  :: HRRS(10,4,1,9)
      ...
      CALL InitDbl(20*4,HRRA(1,1,1))
      CALL InitDbl(20*4,HRRB(1,1,1))
      CALL InitDbl(10*10,HRRC(1,1,1))
      CALL InitDbl(9*1*10*4,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            CALL MVRRd0p0(1,10,4,VRRS(1,1,0,1),VRRS(1,1,1,1),20,10,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [d0|p0]^(0)
            CALL MVRRd0p0(2,10,4,VRRS(1,1,0,2),VRRS(1,1,1,2),20,10,VRR(1,1,1))
            ENDIF
            ...
            ! MIC-VRR: Generating [d0|p0]^(0)
            CALL MVRRd0p0(3,10,4,VRRS(1,1,0,3),VRRS(1,1,1,3),20,10,VRR(1,1,1))
            ENDIF
            ! Contracting ...
            CALL CNTRCTG3331(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
      ...
      DO L=1,1
         ...
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (p,p|2_y,L)  and (p,p|2,L_y)
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,6,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (p,p|2_z,L)  and (p,p|2,L_z)
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
      ...
      IJ=3*(J-1)+I
        DO L=1,1
          DO K=2,4
            ...
            CALL BraHRR33(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO
        ENDDO
      ENDDO
      ENDDO
   END SUBROUTINE dIntB3030301
    SUBROUTINE CNTRCTG3331(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(20,10,0:4)
      REAL(DOUBLE)  :: VRRS(10,4,0:3,3)
      REAL(DOUBLE)  :: HRRS(10,4,1,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
