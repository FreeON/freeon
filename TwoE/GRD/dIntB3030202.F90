! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (p p|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB3030202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (d,0|sp,sp)
      CALL KetHRR22(10,HRR) 
      ! Generating (f,0|sp,sp)^a
      CALL KetHRR22(10,HRRA) 
      ! Generating (f,0|sp,sp)^b
      CALL KetHRR22(10,HRRB) 
      ! Generating (d,0|spd,sp)^c
      CALL KetHRR42(10,HRRC) 
      DO L=1,4
      
         !K = 1
         CDOffSet=(OC+1-1)*LDC+(OD+L-1)*LDD
         ! Generating (p',p|1,L)  and (p,p'|1,L)
         CALL BraHRR33ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,1,L),&
                          HRRA(1,1,L),HRRB(1,1,L),GRADIENTS(1,1))
         ! Generating (p,p|1_x,L)  and (p,p|1,L_x)
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,2,L),GRADIENTS(1,1))
         ! Generating (p,p|1_y,L)  and (p,p|1,L_y)
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,3,L),GRADIENTS(1,1))
         ! Generating (p,p|1_z,L)  and (p,p|1,L_z)
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,4,L),GRADIENTS(1,1))
         ...
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,8,L),GRADIENTS(1,1))
         ...
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRC(1,9,L),GRADIENTS(1,1))
         ...
         CALL BraHRR33cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,3,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB3030202
