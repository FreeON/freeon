! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f d|p sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10060302(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE), DIMENSION(56,13,4) :: HRR 
      REAL(DOUBLE), DIMENSION(84,13,4) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(56,20,4) :: HRRC 
      REAL(DOUBLE)  :: VRR(84,20,0:8)
      REAL(DOUBLE)  :: VRRS(56,13,0:7,3)
      REAL(DOUBLE)  :: HRRS(56,13,4,9)
      REAL(DOUBLE)  :: TOm,PQJ(3),FP(9)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L,IJ
      EXTERNAL InitDbl
      CALL InitDbl(56*13,HRR(1,1,1))
      CALL InitDbl(84*13,HRRA(1,1,1))
      CALL InitDbl(84*13,HRRB(1,1,1))
      CALL InitDbl(56*20,HRRC(1,1,1))
      CALL InitDbl(9*4*56*13,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG10632(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                     & VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (h,0|p,sp)
      CALL KetHRR32(56,HRR) 
      ! Generating (i,0|p,sp)^a
      CALL KetHRR32(84,HRRA) 
      ! Generating (i,0|p,sp)^b
      CALL KetHRR32(84,HRRB) 
      ! Generating (h,0|d,sp)^c
      CALL KetHRR62(56,HRRC) 
      ! Stress: Generating [f,0|p,sp] 
      DO IJ=1,9
        CALL KetHRR32(56,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
      ...
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB10060302
    SUBROUTINE CNTRCTG10632(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(84,20,0:8)
      REAL(DOUBLE)  :: VRRS(56,13,0:7,3)
      REAL(DOUBLE)  :: HRRS(56,13,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
