! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|f s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10011001(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(20,20,1) :: HRR 
      REAL(DOUBLE), DIMENSION(35,20,1) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(20,35,1) :: HRRC 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG101101(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (f,0|f,s)
      ! Dont need to generate (g,0|f,s)^a
      ! Dont need to generate (g,0|f,s)^b
      ! Dont need to generate (f,0|g,s)^c
      DO L=1,1
      
         !K = 11
         CDOffSet=(OC+11-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|11,L)  and (f,s'|11,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,11,L),&
                          HRRA(1,11,L),HRRB(1,11,L),GRADIENTS(1,1))
         ! Generating (f,s|11_x,L)  and (f,s|11,L_x)
         HRRTmp(1:20)=HRRC(1:20,21,L)-3D0*HRR(1:20,5,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|11_y,L)  and (f,s|11,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,22,L),GRADIENTS(1,1))
         ! Generating (f,s|11_z,L)  and (f,s|11,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,26,L),GRADIENTS(1,1))
      
         !K = 12
         CDOffSet=(OC+12-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|12,L)  and (f,s'|12,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,12,L),&
                          HRRA(1,12,L),HRRB(1,12,L),GRADIENTS(1,1))
         ! Generating (f,s|12_x,L)  and (f,s|12,L_x)
         HRRTmp(1:20)=HRRC(1:20,22,L)-2D0*HRR(1:20,6,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|12_y,L)  and (f,s|12,L_y)
         HRRTmp(1:20)=HRRC(1:20,23,L)-1D0*HRR(1:20,5,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|12_z,L)  and (f,s|12,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,27,L),GRADIENTS(1,1))
      
         !K = 13
         CDOffSet=(OC+13-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|13,L)  and (f,s'|13,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,13,L),&
                          HRRA(1,13,L),HRRB(1,13,L),GRADIENTS(1,1))
         ! Generating (f,s|13_x,L)  and (f,s|13,L_x)
         HRRTmp(1:20)=HRRC(1:20,23,L)-1D0*HRR(1:20,7,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|13_y,L)  and (f,s|13,L_y)
         HRRTmp(1:20)=HRRC(1:20,24,L)-2D0*HRR(1:20,6,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|13_z,L)  and (f,s|13,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,28,L),GRADIENTS(1,1))
      
         !K = 14
         CDOffSet=(OC+14-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|14,L)  and (f,s'|14,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,14,L),&
                          HRRA(1,14,L),HRRB(1,14,L),GRADIENTS(1,1))
         ! Generating (f,s|14_x,L)  and (f,s|14,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,24,L),GRADIENTS(1,1))
         ! Generating (f,s|14_y,L)  and (f,s|14,L_y)
         HRRTmp(1:20)=HRRC(1:20,25,L)-3D0*HRR(1:20,7,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|14_z,L)  and (f,s|14,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,29,L),GRADIENTS(1,1))
      
         !K = 15
         CDOffSet=(OC+15-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|15,L)  and (f,s'|15,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,15,L),&
                          HRRA(1,15,L),HRRB(1,15,L),GRADIENTS(1,1))
         ! Generating (f,s|15_x,L)  and (f,s|15,L_x)
         HRRTmp(1:20)=HRRC(1:20,26,L)-2D0*HRR(1:20,8,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|15_y,L)  and (f,s|15,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,27,L),GRADIENTS(1,1))
         ! Generating (f,s|15_z,L)  and (f,s|15,L_z)
         HRRTmp(1:20)=HRRC(1:20,30,L)-1D0*HRR(1:20,5,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 16
         CDOffSet=(OC+16-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|16,L)  and (f,s'|16,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,16,L),&
                          HRRA(1,16,L),HRRB(1,16,L),GRADIENTS(1,1))
         ! Generating (f,s|16_x,L)  and (f,s|16,L_x)
         HRRTmp(1:20)=HRRC(1:20,27,L)-1D0*HRR(1:20,9,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|16_y,L)  and (f,s|16,L_y)
         HRRTmp(1:20)=HRRC(1:20,28,L)-1D0*HRR(1:20,8,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|16_z,L)  and (f,s|16,L_z)
         HRRTmp(1:20)=HRRC(1:20,31,L)-1D0*HRR(1:20,6,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 17
         CDOffSet=(OC+17-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|17,L)  and (f,s'|17,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,17,L),&
                          HRRA(1,17,L),HRRB(1,17,L),GRADIENTS(1,1))
         ! Generating (f,s|17_x,L)  and (f,s|17,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,28,L),GRADIENTS(1,1))
         ! Generating (f,s|17_y,L)  and (f,s|17,L_y)
         HRRTmp(1:20)=HRRC(1:20,29,L)-2D0*HRR(1:20,9,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|17_z,L)  and (f,s|17,L_z)
         HRRTmp(1:20)=HRRC(1:20,32,L)-1D0*HRR(1:20,7,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 18
         CDOffSet=(OC+18-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|18,L)  and (f,s'|18,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,18,L),&
                          HRRA(1,18,L),HRRB(1,18,L),GRADIENTS(1,1))
         ! Generating (f,s|18_x,L)  and (f,s|18,L_x)
         HRRTmp(1:20)=HRRC(1:20,30,L)-1D0*HRR(1:20,10,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|18_y,L)  and (f,s|18,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,31,L),GRADIENTS(1,1))
         ! Generating (f,s|18_z,L)  and (f,s|18,L_z)
         HRRTmp(1:20)=HRRC(1:20,33,L)-2D0*HRR(1:20,8,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 19
         CDOffSet=(OC+19-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|19,L)  and (f,s'|19,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,19,L),&
                          HRRA(1,19,L),HRRB(1,19,L),GRADIENTS(1,1))
         ! Generating (f,s|19_x,L)  and (f,s|19,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,31,L),GRADIENTS(1,1))
         ! Generating (f,s|19_y,L)  and (f,s|19,L_y)
         HRRTmp(1:20)=HRRC(1:20,32,L)-1D0*HRR(1:20,10,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|19_z,L)  and (f,s|19,L_z)
         HRRTmp(1:20)=HRRC(1:20,34,L)-2D0*HRR(1:20,9,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 20
         CDOffSet=(OC+20-11)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|20,L)  and (f,s'|20,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,20,L),&
                          HRRA(1,20,L),HRRB(1,20,L),GRADIENTS(1,1))
         ! Generating (f,s|20_x,L)  and (f,s|20,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,33,L),GRADIENTS(1,1))
         ! Generating (f,s|20_y,L)  and (f,s|20,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,34,L),GRADIENTS(1,1))
         ! Generating (f,s|20_z,L)  and (f,s|20,L_z)
         HRRTmp(1:20)=HRRC(1:20,35,L)-3D0*HRR(1:20,10,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB10011001
    SUBROUTINE CNTRCTG101101(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
      REAL(DOUBLE), DIMENSION(20,20,1) :: HRR 
