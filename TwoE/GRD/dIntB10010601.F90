! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f s|d s) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10010601(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(20,10,1) :: HRR 
      REAL(DOUBLE), DIMENSION(35,10,1) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(20,20,1) :: HRRC 
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG10161(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Dont need to generate (f,0|d,s)
      ! Dont need to generate (g,0|d,s)^a
      ! Dont need to generate (g,0|d,s)^b
      ! Dont need to generate (f,0|f,s)^c
      DO L=1,1
      
         !K = 5
         CDOffSet=(OC+5-5)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|5,L)  and (f,s'|5,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,5,L),&
                          HRRA(1,5,L),HRRB(1,5,L),GRADIENTS(1,1))
         ! Generating (f,s|5_x,L)  and (f,s|5,L_x)
         ...
         ! Generating (f,s|5_y,L)  and (f,s|5,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,12,L),GRADIENTS(1,1))
         ! Generating (f,s|5_z,L)  and (f,s|5,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,15,L),GRADIENTS(1,1))
      
         !K = 6
         CDOffSet=(OC+6-5)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|6,L)  and (f,s'|6,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,6,L),&
                          HRRA(1,6,L),HRRB(1,6,L),GRADIENTS(1,1))
         ! Generating (f,s|6_x,L)  and (f,s|6,L_x)
         HRRTmp(1:20)=HRRC(1:20,12,L)-1D0*HRR(1:20,3,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|6_y,L)  and (f,s|6,L_y)
         ...
         ! Generating (f,s|6_z,L)  and (f,s|6,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,16,L),GRADIENTS(1,1))
      
         !K = 7
         CDOffSet=(OC+7-5)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|7,L)  and (f,s'|7,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,7,L),&
                          HRRA(1,7,L),HRRB(1,7,L),GRADIENTS(1,1))
         ! Generating (f,s|7_x,L)  and (f,s|7,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,13,L),GRADIENTS(1,1))
         ! Generating (f,s|7_y,L)  and (f,s|7,L_y)
         HRRTmp(1:20)=HRRC(1:20,14,L)-2D0*HRR(1:20,3,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|7_z,L)  and (f,s|7,L_z)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,17,L),GRADIENTS(1,1))
      
         !K = 8
         CDOffSet=(OC+8-5)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|8,L)  and (f,s'|8,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,8,L),&
                          HRRA(1,8,L),HRRB(1,8,L),GRADIENTS(1,1))
         ! Generating (f,s|8_x,L)  and (f,s|8,L_x)
         HRRTmp(1:20)=HRRC(1:20,15,L)-1D0*HRR(1:20,4,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|8_y,L)  and (f,s|8,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,16,L),GRADIENTS(1,1))
         ! Generating (f,s|8_z,L)  and (f,s|8,L_z)
         ...
         ! Generating (f',s|9,L)  and (f,s'|9,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,9,L),&
                          HRRA(1,9,L),HRRB(1,9,L),GRADIENTS(1,1))
         ! Generating (f,s|9_x,L)  and (f,s|9,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,16,L),GRADIENTS(1,1))
         ! Generating (f,s|9_y,L)  and (f,s|9,L_y)
         HRRTmp(1:20)=HRRC(1:20,17,L)-1D0*HRR(1:20,4,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRTmp,GRADIENTS(1,1))
         ! Generating (f,s|9_z,L)  and (f,s|9,L_z)
         HRRTmp(1:20)=HRRC(1:20,19,L)-1D0*HRR(1:20,3,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      
         !K = 10
         CDOffSet=(OC+10-5)*LDC+(OD+L-1)*LDD
         ! Generating (f',s|10,L)  and (f,s'|10,L)
         CALL BraHRR101ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,10,L),&
                          HRRA(1,10,L),HRRB(1,10,L),GRADIENTS(1,1))
         ! Generating (f,s|10_x,L)  and (f,s|10,L_x)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,18,L),GRADIENTS(1,1))
         ! Generating (f,s|10_y,L)  and (f,s|10,L_y)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,19,L),GRADIENTS(1,1))
         ! Generating (f,s|10_z,L)  and (f,s|10,L_z)
         HRRTmp(1:20)=HRRC(1:20,20,L)-2D0*HRR(1:20,4,L)
         CALL BraHRR101cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRTmp,GRADIENTS(1,1))
      ENDDO 
    END SUBROUTINE dIntB10010601
    SUBROUTINE CNTRCTG10161(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
      REAL(DOUBLE), DIMENSION(20,10,1) :: HRR 
