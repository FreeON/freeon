...
SUBROUTINE dIntB3030303(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (spd,0|p,p)
      CALL KetHRR33(16,HRR) 
      ! Generating (spdf,0|p,p)^a
      CALL KetHRR33(20,HRRA) 
      ! Generating (spdf,0|p,p)^b
      CALL KetHRR33(20,HRRB) 
      ! Generating (spd,0|<>CType[5]<>,p)^c
      CALL KetHRR53(16,HRRC) 
      DO L=2,4
    ...
    SUBROUTINE CNTRCTG3333(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
