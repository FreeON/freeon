...
SUBROUTINE dIntB3020202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(13,18,0:4,3)
      REAL(DOUBLE)  :: HRRS(13,18,4,9)
      ...
      CALL InitDbl(9*4*13*18,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO IJ=1,9
        CALL KetHRR22(13,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
         ...
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,&
                      HRRC(1,2,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (p,sp|1_y,L)  and (p,sp|1,L_y)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,&
                      HRRC(1,3,L),GRADIENTS(1,1),FP(1),STRESS(1,1))
         ! Generating (p,sp|1_z,L)  and (p,sp|1,L_z)
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
         ...
         CALL BraHRR32cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO 
      ! Stress: Generating (p,sp|sp,sp)^(1) 
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,4
          DO K=1,4
            ...
            CALL BraHRR32(OA,OB,LDA,LDB,CDOffSet,HRRS(1,K,L,IJ),STRESS(1,IJ))
          ENDDO 
        ENDDO 
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB3020202
    SUBROUTINE CNTRCTG3222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(20,20,0:5)
      REAL(DOUBLE)  :: VRRS(13,18,0:4,3)
      REAL(DOUBLE)  :: HRRS(13,18,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
