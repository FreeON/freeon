...
!------------------------------------------------------------------------------
! ----------------------------------------------------------
! COMPUTES THE INTEGRAL CLASS (f f|f p)
! ----------------------------------------------------------
SUBROUTINE dIntB10101003(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      REAL(DOUBLE)  :: VRRS(84,35,0:10,3)
      REAL(DOUBLE)  :: HRRS(84,35,4,9)
      ...
      CALL InitDbl(9*4*84*35,HRRS(1,1,1,1))
      Ax=ACInfo%Atm1X
      Ay=ACInfo%Atm1Y
      Az=ACInfo%Atm1Z
      Bx=ACInfo%Atm2X
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop
            ...
            ENDIF
            ! Contracting ...
            CALL CNTRCTG1010103(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                       VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (i,0|f,p)
      CALL KetHRR103(84,HRR)
      ! Generating (j,0|f,p)^a
      CALL KetHRR103(120,HRRA)
      ! Generating (j,0|f,p)^b
      CALL KetHRR103(120,HRRB)
      ! Generating (i,0|g,p)^c
      CALL KetHRR153(84,HRRC)
      ! Stress: Generating [f,0|f,p]
      DO IJ=1,9
        CALL KetHRR103(84,HRRS(1,1,1,IJ))
      ENDDO
      DO L=2,4

         !K = 11
         CDOffSet=(OC+11-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 12
         CDOffSet=(OC+12-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 13
         CDOffSet=(OC+13-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 14
         CDOffSet=(OC+14-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 15
         CDOffSet=(OC+15-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 16
         CDOffSet=(OC+16-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 17
         CDOffSet=(OC+17-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 18
         CDOffSet=(OC+18-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 19
         CDOffSet=(OC+19-11)*LDC+(OD+L-2)*LDD
         ...
         !K = 20
         CDOffSet=(OC+20-11)*LDC+(OD+L-2)*LDD
      ...
      ENDDO
      ENDDO
   END SUBROUTINE dIntB10101003
    SUBROUTINE CNTRCTG1010103(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(120,56,0:11)
      REAL(DOUBLE)  :: VRRS(84,35,0:10,3)
      REAL(DOUBLE)  :: HRRS(84,35,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,35
