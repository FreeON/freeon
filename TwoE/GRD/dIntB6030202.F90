! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (d p|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB6030202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ! Contracting ... 
            CALL CNTRCTG6322(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ...
      DO L=1,4
         ...
         ! Generating (d',p|1,L)  and (d,p'|1,L)
         CALL BraHRR63ab(NINT,LDA,LDB,OA,OB,GOA,GOB,CDOffSet,HRR(1,1,L),&
                          HRRA(1,1,L),HRRB(1,1,L),GRADIENTS(1,1))
         ! Generating (d,p|1_x,L)  and (d,p|1,L_x)
         CALL BraHRR63cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,0,HRRC(1,2,L),GRADIENTS(1,1))
         ! Generating (d,p|1_y,L)  and (d,p|1,L_y)
         CALL BraHRR63cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,1,HRRC(1,3,L),GRADIENTS(1,1))
         ! Generating (d,p|1_z,L)  and (d,p|1,L_z)
         CALL BraHRR63cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,HRRC(1,4,L),GRADIENTS(1,1))
      ...
      ENDDO 
    END SUBROUTINE dIntB6030202
    SUBROUTINE CNTRCTG6322(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC)
      USE DerivedTypes
      USE VScratchB
      INTEGER :: K
      REAL(DOUBLE)  :: Alpha,Beta,Gamma
