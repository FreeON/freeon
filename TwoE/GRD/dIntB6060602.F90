! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (d d|d sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB6060602(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS)
      ...
      REAL(DOUBLE), DIMENSION(35,26,4) :: HRR 
      REAL(DOUBLE), DIMENSION(56,26,4) :: HRRA,HRRB 
      REAL(DOUBLE), DIMENSION(35,35,4) :: HRRC 
      REAL(DOUBLE)  :: VRR(56,35,0:8)
      INTEGER       :: OffSet,OA,LDA,GOA,OB,LDB,GOB,OC,LDC,GOC,OD,LDD,GOD,I,J,K,L
      EXTERNAL InitDbl
      CALL InitDbl(35*26,HRR(1,1,1))
      CALL InitDbl(56*26,HRRA(1,1,1))
      CALL InitDbl(56*26,HRRB(1,1,1))
      ...
      ENDDO ! |N0) loop
      ! Generating (g,0|d,sp)
      CALL KetHRR62(35,HRR) 
      ! Generating (h,0|d,sp)^a
      CALL KetHRR62(56,HRRA) 
      ! Generating (h,0|d,sp)^b
      CALL KetHRR62(56,HRRB) 
      ! Generating (g,0|f,sp)^c
      CALL KetHRR102(35,HRRC) 
      DO L=1,4
