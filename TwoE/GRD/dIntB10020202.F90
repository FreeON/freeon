! ---------------------------------------------------------- 
! COMPUTES THE INTEGRAL CLASS (f sp|sp sp) 
! ---------------------------------------------------------- 
SUBROUTINE dIntB10020202(PrmBufB,LBra,PrmBufK,LKet,ACInfo,BDInfo, &
 OA,LDA,OB,LDB,OC,LDC,OD,LDD,GOA,GOB,GOC,GOD,NINT,PBC,GRADIENTS,STRESS)
      ...
      DO J=1,LKet ! K^2 VRR |N0) loop 
         ...
         DO K=1,LBra ! K^2 VRR (M0| loop 
            ...
            ENDIF
            ! Contracting ... 
            CALL CNTRCTG10222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC, &
                     & VRRS,HRRS(1,1,1,1),PQJ(1),PBC%AutoW%I(1))
         ENDDO ! (M0| loop
      ENDDO ! |N0) loop
      ! Generating (<>CType[11]<>,0|sp,sp)
      CALL KetHRR22(45,HRR) 
      ! Generating (<>CType[16]<>,0|sp,sp)^a
      CALL KetHRR22(56,HRRA) 
      ! Generating (<>CType[16]<>,0|sp,sp)^b
      CALL KetHRR22(56,HRRB) 
      ! Generating (<>CType[11]<>,0|spd,sp)^c
      CALL KetHRR42(45,HRRC) 
      ! Stress: Generating [f,0|sp,sp] 
      DO IJ=1,9
        CALL KetHRR22(45,HRRS(1,1,1,IJ)) 
      ENDDO
      DO L=1,4
         ...
         CALL BraHRR102cd(NINT,LDA,LDB,OA,OB,GOA,GOB,GOC,GOD,CDOffSet,2,&
                      HRRTmp,GRADIENTS(1,1),FP(1),STRESS(1,1))
      ENDDO 
      ! Stress: Generating (f,sp|sp,sp)^(1) 
      DO J=1,3
      DO I=1,3
      IJ=3*(J-1)+I
        DO L=1,4
      ...
      ENDDO 
      ENDDO 
   END SUBROUTINE dIntB10020202
    SUBROUTINE CNTRCTG10222(VRR,HRR,Alpha,HRRA,Beta,HRRB,Gamma,HRRC,VRRS,HRRS,PQJ,IW)
      USE DerivedTypes
      USE VScratchB
      IMPLICIT NONE
      INTEGER :: K
      ...
      REAL(DOUBLE)  :: VRR(56,20,0:7)
      REAL(DOUBLE)  :: VRRS(45,18,0:6,3)
      REAL(DOUBLE)  :: HRRS(45,18,4,9),PQJ(3)
      INTEGER :: IJ,J,I,IW(3)
      DO K=1,10
