...
(PM) in Ref.~\onlinecite{APalser98}. 
...
The grand canonical McWeeny purification scheme \cite{RMcWeeny60,APalser98}
...
\commentoutA{
\begin{figure}[h]
\caption{Examples of spectral projection polynomials ${\cal P}_{\rm SP2}$ and 
${\cal P}_{\rm TRS4}(x) = {\cal F}(x) + \gamma {\cal G}(x)$
...
Trace Re-Setting method is quite general.  We note, for example, that  higher order
generalizations can be given with ${\cal F}_m({\bf X}) = {\bf X}^m[{\bf I} + m({\bf I} - {\bf X})]$
...
In this case the starting guess ${\bf X}_0$ must be modified with the normalized
...
encountered in the McWeeny or PM scheme, as one extra inflection may be obtained at the 
...
an asymmetric polynomial \cite{ANiklasson02A}.  In the case of TRS4, the combination of 
...
\commentoutB{level.}
\commentoutA{level, and is shown in Fig.~\ref{xefig}.} 
...
\resizebox*{7in}{!}{\includegraphics{TP_Fig_F_G_large.eps}}
\end{center}
