...
\section{Introduction}
 ...
 for higher order response properties up fourth order in energy.
 ...
 for a series of 1D chain of water molecules. Thereafter, we demon state
...
The total energy may be developed in a Taylor expansion as
\begin{equation}
  \begin{split}
...
\end{equation}
 ...
 moment and $\E_a$ is the electric field in direction $a$. 
...
\begin{subequations}
  \begin{gather}
    \D^{0} =\D^{0} \D^{0},\\
    \D^{a} =\D^{a} \D^{0}+\D^{0}\D^{a},\\
    \D^{ab}=\D^{ab}\D^{0}+\D^{0}\D^{ab}+\D^{a}\D^{b}+\D^{b}\D^{a}\\
    \text{May be the third order???}
  \end{gather}
\end{subequations}
The following schematic algorithm is a higher order generalization of the
self consistent scheme for the solution of the CPSCF equations describe in \cite{SV} 
based on perturbed projection:
\begin{subequations}
  \begin{gather}
...
%    (\F^{0}+\sum_c\E^{c}\widetilde{\F}^{c}_n+\ldots))\bigg|_{\E=0} \label{DDeriv}
  \end{gather} 
\end{subequations}
with starting points $D^{a\ldots}_0=0$. In step~(\ref{FockBuild}),  $F^a_n$ is constructed in
...
\begin{equation}
  E^{abcd}=\frac{1}{24}P(a,b,c,d)\Tr([\D^{ab},\D^0] \D^c \F^d +[\D^a,\D^0](\D^{bc} \F^d + \D^b \F^{cd}))
\end{equation}
...
We have performed polarizability calculations on a series of water 
clusters at standard liquid density \cite{MChallacombe97,ESchwegler97},
up to (H$_2$O)$_{150}$. 
