...
iterations to achieve load balance.   
With the 3-21G basis set and a medium quality grid, Equal Time partitioning 
applied to taxol (62 heavy atoms) attained a speedup of 61 out of 64 processors,  
while for a 110 molecule water cluster at standard density it achieved a speedup 
of 113 out of 128.  The efficiency of Equal Time partitioning applied to 
Hierarchical Cubature improves as the grid work per processor increases. 
With a fine grid and the 6-311G(df,p) basis set, 
calculations on the 26 atom molecule $\alpha$-pinene achieved a parallel efficiency better 
than 99\% with 64 processors.  For more coarse grained calculations, 
superlinear speedups are found to result from reduced computational complexity 
associated with data parallelism.
...
%These are simple inconveniences, which can be easily addressed in future
...
threshold) divides the workload in half (see Figure~\ref{fig:shiftline}). 
Notice that the cutting direction may be varied
...
All runs on the first two systems were performed on a single SGI Origin 2000
with 128 (250 MHz MIPS R10000 MIPS) processors. 
For the calculations on $\alpha$-pinene, we  used a cluster of 
256 4 CPU HP/Compaq Alphaserver ES45s with the Quadrics QsNet High Speed 
Interconnect.  
...
\commentoutA{
\begin{figure}[t]
\resizebox*{3.5in}{!}{\includegraphics[clip]{good-thres-pinene.eps}}
\caption{
Scaling of parallel HiCu on $\alpha$-pinene RPBE/6-311G(df,p), with
$\tau = 1.0 \times 10^{-6}$. Speedups are relative to a 2-processor
calculation.
}\label{fig:good-thres-pinene}
\end{figure}
}
\commentoutA{
\begin{figure}[t]
\resizebox*{3.5in}{!}{\includegraphics[clip]{tight-thres-pinene.eps}}
\caption{
Scaling of parallel HiCu on $\alpha$-pinene RPBE/6-311G(df,p), with
$\tau = 1.0 \times 10^{-8}$. Speedups are relative to a 2-processor
calculation.
}\label{fig:tight-thres-pinene}
\end{figure}
}

A well studied system in parallel Quantum Chemistry  is 
6-311G(df,p) $\alpha$-pinene (C$_{10}$H$_{16}$)~\cite{Furlani_00v128,Sosa_00v26}.
To date the most competitive rate for this system is 37.3 out
of 64 processors, achieved by Furlani~{\it et al.}~\cite{Furlani_00v128} 
over a complete SCF cycle.  Scaling results for a medium accuracy grid 
corresponding to $\tau = 1.0 \times 10^{-6}$ are shown in Fig.~\ref{fig:good-thres-pinene},
with HiCu achieving a competitive rate of 45.11 (70\% efficiency) out of 64 processors.
More impressively, with a high accuracy grid ($\tau = 1.0 \times 10^{-8}$) Equal Time partitioning 
delivers an unrivaled 99.5\% efficiency with $\Np = 64$ as shown in Fig.~\ref{fig:tight-thres-pinene}.
These results demonstrate that the efficiency of  ET improves  when the grid work per processor is 
increased. This is not surprising as increasing the grid work decreases the effective granularity, 
reducing sensitivity to fluctuations in leaf-node times and leading to a more predictive Equal Time 
partition.
...
massive parallelism, which have been avoided here.  

With $\Np \leq 128$, the predominant cause of deviation from ideal scaling is due 
to basic non-linearities in ET partitioning of the HiCu grid, rather 
than overheads associated with  inefficient data structures or communication.  
These deviations increase as the processor to grid work ratio increases, {\em i.e.} with finer 
granularity.  The main factors that determine grid work are the number of atoms and $\tau$, the threshold 
controlling grid accuracy.   For example, calculations on (H$_2$O)$_{110}$ with a medium accuracy grid 
and $\sim 1$ heavy atom per processor are fine grained, achieving a parallel efficiency of 88\% with $\Np=128$.  
Even with $\Np/N_{\rm Atoms} > 2$ it is possible achieve excellent scaling by lowering $\tau$.
This was demonstrated by 6-311G(df,p) calculations on $\alpha$-pinene (C$_{10}$H$_{16}$) with $\Np=64$,
achieving a parallel efficiency of 99.5\%.  Apparently, this is the first time this effect has been demonstrated 
in the context of parallel Quantum Chemistry.  More importantly, because HiCu continues to achieve early onset 
linear scaling even with tight numerical thresholds~\cite{MChallacombe00A}, these results suggest the potential 
for very large, high quality calculations through a combination of massive parallelism and scalable ${\cal O}(N)$ 
algorithms.
...
discussion on the general Equal Time partitioning scheme with C.~J.~Tymczak is appreciated.
\end{acknowledgments}
...
%NoFig\begin{center}
%NoFigFigure 1, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{6.0in}{!}{\includegraphics[clip,angle=90]{CM.eps}}
%NoFig\end{center}
%NoFig
%NoFig\clearpage
%NoFig
%NoFig\begin{center}
%NoFigFigure 2, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{6.0in}{!}{\includegraphics[clip,angle=90]{ET.eps}}
%NoFig\end{center}
%NoFig
%NoFig\clearpage
%NoFig\begin{center}
%NoFigFigure 3, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{6.0in}{!}{\includegraphics[clip,angle=90]{PHiCuFlow.eps}}
%NoFig\end{center}
%NoFig
%NoFig\clearpage
%NoFig\begin{center}
%NoFigFigure 4, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{5.0in}{!}{\includegraphics[clip,angle=90]{taxol_speedup.eps}}
%NoFig\end{center}
%NoFig
%NoFig\pagebreak
%NoFig\begin{center}
%NoFigFigure 5, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{5.0in}{!}{\includegraphics[clip,angle=90]{3-21G_good_div10.eps}}
%NoFig\end{center}
%NoFig
%NoFig\pagebreak
%NoFig\begin{center}
%NoFigFigure 6, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{5.0in}{!}{\includegraphics[clip,angle=90]{good-thres-pinene.eps}}
%NoFig\end{center}
%NoFig
%NoFig\pagebreak
%NoFig\begin{center}
%NoFigFigure 7, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{5.0in}{!}{\includegraphics[clip,angle=90]{tight-thres-pinene.eps}}
%NoFig\end{center}
%NoFig
%NoFig\pagebreak
%NoFig\begin{center}
%NoFigFigure 8, C.~K.~Gan and M.~Challacombe \\[1.cm]
%NoFig\resizebox*{6.0in}{!}{\includegraphics[clip,angle=90]{binary.eps}}
%NoFig\end{center}
}
...
\commentoutA{
\begin{figure}[t]
\resizebox*{3.5in}{!}{\includegraphics[clip]{binary.eps}}
\caption{
An illustrative example to show the general Equal Time
partitioning scheme for the case of $m=5$.
}\label{fig:binary}
\end{figure}
}
...
a box into two subboxes with a workload ratio equal to 
