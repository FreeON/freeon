...
% through the processors when the method has to be brought to parallel machines.
 ...
 For example, the Foster's algorithm achieves an efficiency, for the 
...
% the most widely used communication tools.
 ...
 Our parallel ONX uses a partially distributed-data exchange build. While the density matrix is
 fully distributed on the processors, the corresponding exchange submatrices are not redistributed during
 its construction. This strategy avoids an expensive dynamic redistribution 
 ...
 The parallelization of the later requires expansive communications of data
 as do conventional algorithms.
...
\end{figure}
 ...
 the lower part of the exchange sub-matrix $K(D({\tt iPrc}))$ 
...
% block partition used throughout this work. 
 ...
 a non-negative cost function $\phi$ on contiguous atomic-blocks of
...
\subsection{2D Partition}
 ...
 is further sectioned into $p$ coloumns $[c_j,c_{j+1}]$ with $1 \le j < p$ of approximately equal cost.
