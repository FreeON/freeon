...
{\bf It is also possible to update the internal coordinate
Hessian, however this does not allow for the on-the-fly redefinition
of the internal coordinates, that is important for large molecules.
...
a higher than linear fit of the energy gradients may lead 
...
subspace, by numerical derivation or Hessian-update.
Using this numerical Hessian matrix,
ABNR carries out a Newton-Raphson or a Rational Function Optimization 
\cite{ABanerji85} step in the iterative subspace. 
...
or interpolating the error associated with them. The extrapolation
is done without any fitting, just by weighting and summing-up
different structures.
Since the error is usually defined as the norm of the gradient,
GDIIS cannot account for the curvature of the potential energy surface.
As a consequence, GDIIS oscillates in the vicinity of 
inflexion-points and fails to distinguish between minima and 
saddle points. Furthermore, the extrapolation/interpolation 
process is very sensitive to numerical errors in the error estimates.
The most recent GDIIS algorithms cure these problems by 
augmenting GDIIS into the Rational Function Optimization scheme
...
As opposed to GDIIS, QUICCA needs neither simple relaxation
steps nor external optimizer. Since QUICCA accounts
for the gradient instead of the gradient norm, it has the capability
to distinguish between minima, saddles and inflexion points.
This is due to the weighted curve-fit, that can be applied in any
kind of coordinate system exhibiting
reasonably small vibrational coupling. The curve fit avoids 
numerical problems associated with error estimates in GDIIS,
and it also averages noise coming from vibrational coupling.

A close relative of QUICCA is the string method from the
...
gradient-based line search method, that explores the potential energy
surface in directions
...
{\bf The merging of the primary bonding schemes guaranties the convergence
of the gradient transformation of an older Cartesian gradient vector
at a more recent geometry while it forgets about information from 
irrelevant steps.
}
