...
It is also possible to update the internal coordinate
...
typically involving force relaxation (as described in Section \ref{conventional}) or Rational Function Optimization methods.  These
simple relaxation structures are propagated along an abstract optimization
coordinate by extrapolating the error associated with them to zero through
solution of a least squares problem that minimizes errors associated with the
gradient norm.  GDIIS cannot account for the signed curvature of the potential
...
numerical problems associated with the condition number
of the GDIIS error matrix,
...
\cite{GFogarasi79,GFogarasi92,JBaker93,FEckert97,HSchlegel98}.  
...
However, a weighted filtering algorithm such as 
