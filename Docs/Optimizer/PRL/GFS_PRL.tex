...
molecular geometry {\bf as described in Ref.~\onlinecite{pulay_review}}.
...
geometry optimization is the geometric DIIS (GDIIS) \cite{Pulay_GDIIS} 
algorithm of Pulay
{\it et.al}. While geometric DIIS is a simple 
technique, its successful application may require many intricate modifications \cite{Farkas_GDIIS}.  
Recent reviews and comparisons of these algorithms and their technical details can be found in 
{\bf Refs.~\onlinecite{schlegel_review_03,bakken}}.
...
\cite{turner99,bofill_lanczos,billeter_coordtrf}. However, the lack of
...
The coordinate transformation bottleck clearly avoided
the development of geometry optimizers that are 
based solely on internal coordinates.
...
geometric DIIS (GDIIS) \cite{Pulay_GDIIS} algorithm of Pulay and to
the Adopted Basis set Newton-Raphson (ABNR) method of Karplus 
{\it et.al.} \cite{Brooks83}, since these algorithms all
do extrapolation and interpolation, however they do it in 
a different way.

All three methods are Krylov space \cite{YSaad92} algorithms, in the sence, 
that they define a limited-size iterative subspace and collect information 
about the the potential energy surface exclusively within this 
subspace. The way they do this, however, differs.
...
optimizer. It can also give an account of second order information,
...
GDIIS does not give any account of the curvature of the
error at the extrema, thus it fails to characterize
...
of an inflexion point the error curve is not derivabe, that
leads to oscillations. The error is usually counted
as the norm of the gradient. Defining the error as 
the scalar product of simple relaxation steps and local gradients 
may cure most of these problems, but it needs a reliable 
(and efficient) external optimizer. Such cures of GDIIS
are presented in Refs.~\onlinecite{eckert} and 
\onlinecite{Farkas_GDIIS}.

As opposed to GDIIS, QUICCA does not need simple relaxation
steps, produced by any external optimizer. Furthermore,
at points of the abstract optimization coordinate, not just
...
to identify the type of local extrema that traditional 
GDIIS can not give
an account for, due to the lack of fit and suitable errors.
Furthermore, the location of the extrema can be identified
...
and other similar reaction path finders \cite{Gonzalez90}, that
...
that are assumed to exhibite small vibrational coupling.
}
...
{\bf \cite{fogarasi_diaghess,Pulay_natural_internals,bakerstest,eckert,schlegel_on2iter}}.  
...
{\bf The merging of the bonding schemes garantees the convergence
...
at older geometries.
The merged coordinate set also allowes for more balanced geometry changes
...
Combined internal coordinates, such as the natural \cite{Pulay_natural_internals,arnim99} or the
...
accurate Hessian.  
%Note that, even with a good quality Hessian update, such as BFGS \cite{RFletcher},
%there is no guarantee to converge into a minimum instead of a transition state. 
{\bf We adopt the view of Ref.~\onlinecite{Pulay_natural_internals}, that
some of the 
...
as mentioned in Sect.~\onlinecite{stepsizectrl} is useful and should be explored
further.
}
...
\begin{figure}[h]
\resizebox*{3.5in}{!}{\includegraphics{snap.eps2}}
\caption{
...
\cite{Schlegel_plasminogen_opt}, but looser than the usual Baker's criteria.
