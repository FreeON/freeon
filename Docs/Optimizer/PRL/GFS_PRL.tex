...
vibrational decoupling of the curvilinear coordinates the 3N-dimensional
optimization problem splits into a quasi non-interacting
set of ${\cal{O}}(N)$ one-dimensional optimization problems, 
...
in the Newton scheme, updated Hessians exibit only superlinear
...
Even with the knowledge of modern Hessian update techniques, such 
as BFGS \cite{RFletcher} there is no strict garantee to converge into
a minimum, instead of a transition state. The only certain source
of determining whether an extremal point of the potential energy surface
is minimum, is the exact Hessian \cite{fogarasi_diaghess}.
