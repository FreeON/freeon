...
bendings and dihedral torsions.  These different motions, which are related to chemical concepts, 
lead to an approximate energy and length scale separation that is manifested in strong diagonal 
dominance in the matrix of second order energy derivatives (the Hessian), and also in higher order 
anharmonic energy tensors.  Off diagonal elements of the internal coordinate Hessian are typically 
an order of magnitude smaller than the diagonal ones
...
In the following, we overview conventional methods of geometry optimization (Section \ref{conventional}), 
...
and second order coupling will increase during the course of optimization unless the normal modes are 
...
are avoided.  It is not clear how the present implementation behaves in the 
case of a more delocalized transition state, spanning many internal coordinates, which rarely occur.  
...
\begin{figure}[t]
%\resizebox*{3.5in}{!}{\includegraphics{kinasepicture.ps}}
\caption{
...
\begin{figure}[b]
\resizebox*{3.5in}{!}{\includegraphics{R263logdata2.eps}}
\caption{
...
a certain tolerance increases with system size, roughly as ${\cal O}(N_{\rm atoms}^{1/3})$ 
according to Ref.~\ref{Schlegel_plasminogen_opt}.  Therefore, the rate of linear convergence achieved by an 
...
is transfered. This is a model reactant system obtained from Ref.~\onlinecite{grame} and is shown in 
...
at each step is shown.   In Fig.~\ref{logn-logde}, close to convergence there is  an increase in the rate 
of convergence $f$, demonstrating superlinear convergence.
