...
\title{\textbf{Linear scaling computation of the Fock matrix VIII. The exchange matrix
...
However, most of these calculations make us of ${\bf k}$-space integrations to 
...
\label{CryEq}
\end{equation}
Let us consider two case; First let us consider Gamma point only, then,
\begin{equation}
...
and this leads to a translationally invariant exchange matrix, but then these integrals do not converge. 
...
We do this by defining  a Minimum Image Distance (MID) conditio, which is similiar to what is done
...
also exist for any finite set of lattice vectors ${\bf n}$. Consider 
...
our methodology for doing the exchange matrix at the Gamma point is correct.
...
Even in this very dense periodic system for a medium basis set with d-functions, 
linear scaling for the matrix build commences very early. 
