...
simple ideas and arguments, we can rectify it.
%
\vskip 0.07 in
...
\pagebreak
These schemes have been implemented in the
...
of the unit cell.  Translational invariance can be restored to the exchange $\Gamma$-point approximation 
...
$\bf k$-space sampling, and may benefit from employing the minimum image criterion. 
...
modified by straightforwardly performing double loops over the lattice vectors $\bf v$ and $\bf u$
around the original ONX loop structures \cite{}.  Thus, two ordered bra and ket distribution buffers are 
...
minimum image criterion in the contraction stage of the vertical recurrence relations.  While this 
implementation is linear scaling and simple, it suffers from a number of drawbacks that we will set 
...
described in Sections \ref{gammapoint} and \ref{implementation}, while periodic {\sc QCTC} has 
...
small super-cell calculations will typically be higher than the $\bf k$-space integration limit.
...
\begin{table}[h]
\caption{Comparative progression of Hartree-Fock $\Gamma$-point super-cell calculations 
...
\begin{figure}[h]
\resizebox*{3.5in}{!}{\includegraphics{Diamond_En_vs_a.ps}}
...
\resizebox*{3.5in}{!}{\includegraphics{Timing_Diamond_ONX.ps}}
\end{figure}
...
It is also important that we have demonstrated convergence of our $\Gamma$-point
super-cell calculations with the well established {\bf k}-space integration methods
in {\sc Crystal98} to within 8 digits.  We have not carried out calculations involving 
{\it d}-functions, because the use of fitting functions in {\sc Crystal98} makes a 
direct comparison more difficult.  We note important differences between the 
...
as might be expected.  Nevertheless, achieving high order translational invariance 
...
A new translationaly invariant definition of the periodic Hartree-Fock 
