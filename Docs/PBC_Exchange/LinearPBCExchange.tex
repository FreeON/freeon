...
%
\vskip 0.07 in
\noindent{\bf Keywords}: Self-consistent-field, linear-scaling, periodic systems, exact-exchange
%
\end{abstract}

\pacs{71.15.-m,71.15.Ap,71.15.Mb,71.70.Gm}
...
approximation and also a starting point for correlated ``wave-function'' methods.  
...
which generate wave-functions with the correct translational symmetries.   While the  
...
two Gaussian centered at ${\bf A}$ and ${\bf C}$ decays also as a Gaussian with 
$|{\bf A}-{\bf C}|$. 
...
reducing Eq.~(\ref{CryEq}) to the less cumbersome relation (note, we have re-ordered the lattice sums for 
simplicity)
\begin{equation}
K_{ab}=
...
\cite{NMetropolis53,DTheodorou85,MAllen90,MHloucha98}, where the arbitrary truncation of short range potentials 
also leads to numerical artifacts in energies and forces.  In both cases, translational invariance is  restored 
by introducing the Minimum Image Convention (MIC), which ensures that the distance between neighboring images is 
always used in computing interactions. In computation of the Hartree-Fock exchange matrix, the MIC 
$\Gamma$-point approximation is just
\begin{equation}
K_{ab}=
\sum _{{\bf H G} c d} P_{cd}
\left(
...
\end{eqnarray}
\end{subequations}
where $\delta \approx 10^{-15}$.
%
%\footnote{\bf could use some more explanation here.  Maybe a footnote would be appropriate?}
%
...
(EKPS).  With greater expense, we could recover the EKPS by  explicitly symmetrizing the primitive Gaussian products 
...
where  ${\bf F}$ is the Fockian, ${\bf T}$ is the kinetic energy matrix  and $E_{\rm ne}$ is 
the nuclear-electrostatic energy. This  modified ODA leads to the correct quadratic parameterization 
and guarantees convergence of the HF-MIC model.  
...
\begin{table}[ht]
\caption{Lattice Constants in angstroms for the Diamond System for different 
...
a relative accuracy of 6-7 digits. The potential minimum for the {\sc Crystal98} curve is $7.144$\AA~
...
%
\begin{table}[ht]
\caption{Fractional coordinates for the 8 atom unit cell of Diamond, corresponding to the supercell 
...
\begin{table}[ht]
\caption{The various thresholds which enter into our three levels of accuracy. The loose accuracy
will give approximately 4-5 digits in the SCF energy; the good accuracy will give approximately 6-7 digits 
in the SCF energy; and the tight accuracy will give approximately 8-9 digits in the SCF energy }
\label{Table:AccurLevels}
\center{
\begin{tabular}{ccccc}
\toprule
Level  & Matrix & Penetration & QCTC/ONX & Distribution \\ 
\colrule
