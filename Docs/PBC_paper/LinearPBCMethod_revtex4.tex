...
\section{data structures and error estimates}
...
while the cube-tree contains a hierarchical grid for integration of the exchange-correlation potential.  The grid 
is composed of a Cartesian non-product integration rule on each node, with the gridpoints enclosed by the AABB.
The cube-tree is constructed iteratively through recusrive bisection of the primary volume (the root AABB), using 
exact error bounds to achieve arbitrary accuracy of the integrated density and its gradients.   As the cube-tree 
is built, AABB intersection tests are performed while traversing the rho-tree,  winowing out parts of the density 
that do not overlap with that portion of the integration grid.  With the cube-tree completed and the exchange-correlation
evaluated on the grid, the inverse procedure is carried out;  for each primitive distribution, the cube-tree 
is walked selecting only overlaping portions of the grid via the AABB intersection test.
...
Both HiCu and QCTC employ a Hermite Guassian representation of distributions
\begin{equation}
\rho_q ({\bf r}) = \sum_{lmn} d_{lmn} \Lambda^q_{lmn}({\bf r}),
\end{equation}
where 
\begin{equation}
\Lambda^q_{lmn}({\bf r}) = \frac{\partial^{l+m+n} }{\partial Q^l_x \partial Q^m_y \partial Q^n_z} e^{-\zeta_q ({\bf r}-{\bf Q})}.
\end{equation}
which provides an intermediate with elements of the density matrix subsumed in the coefficients $d_{lmn}$, and allows
the use of McMurchie-Davidson recurrence relations in analytic integral evaluation.   

To achieve early onset linear
scaling, the best case distribution - tree error estimate is used, based on the bound provided by Cramer's inequality:
\begin{equation}
...
\begin{cases}
\zeta_q   & l+m+n= 0 \\
\frac{1}{2} \zeta_q   & {\rm else}
\end{cases}
\end{equation}
With the behavior of the Gaussian primitive rigorously bounded and simply represented by an $s$-type
Gaussian, it is simple to compute its extent.  In the case of HiCu, the overlap extent of a distribution 
$R^{\rm o}_q$, is the 
the range at which numerical evaluation of $\rho_q$ at a point results in a contribution less than $\tau$, 
\begin{equation}
C_q e^{ -\widetilde \zeta_q R^2_{\rm o} }  = \tau
\end{equation}

\begin{equation}
C_q \int \left[ \left(\pi/{\widetilde \zeta_q} \right)^{3/2} 
\delta({ r})  - e^{ -\widetilde \zeta_q {r}^2 } \right] \left|{ r}-{ R_{\rm p}} \right|^{-1} {\rm d} r = \tau
\end{equation}




These employ the use of the Axis Aligned Bounding Box (AABB) tests to dynamically determine the
course of traversal; if the boxes touch, the tree is descened while if they a

box - tree interactions.  
In the case of HiCu, a hierarchical grid of Cartesian cubature rules is constructed through 
recusion.  Each node in the integration tree is a box, which interacts with 




Over the last 10 years, two hierarchical multipole methods have evolved for solving the
Quantum Coulomb problem: methods related to the continous fast multipole method (CFMM) \cite{}
and methods related to the Quantum Chemical Tree Code (QCTC) \cite{}.   The CFMM related methods 
employ worst case box-box error estimates and are ${\cal O}(N)$ with an error that increases
as $N{1/6}$, or ${\cal O}(N {\rm lg} N)$ with a constant error \cite{}.  The QCTC like 
algorithms employ a best case distribution-box error estimate and are ${\cal O}(N {\rm lg} N)$ 
with a fixed error.  The efficiency of both methods depends significantly on the error estimates
used to control both penetration errors and errors in the multipole expansion.  In the case of 
QCTC, these error estimates are checked with a high frequency as the tree is traversed, 
...
diff between FMM and TREECODE
worstcase error
more powerfull error estimates and data structures

Advantages of multipole approximation
PERIODIC, Nighbor DW
}
...
Both methods can be extended to periodic boundary conditions 
...
code \cite{WarSal93,WarSal95,Salmon:93,Challacombe96A} to compute the integrals in equation (\ref{Jqctc}).
