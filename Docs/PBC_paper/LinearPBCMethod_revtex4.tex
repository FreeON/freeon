...
Quantum Chemistry code {\sc MondoSCF}. For the two-electron Coulomb
...
density. Finally, the current capabilities of {\sc MondoSCF} for large condensed
...
complicated operators often involved in response properties 
\cite{FHarris66,LMcmurchie78,FHarris83,JHead89}.
...
correlated wavefunction methods \cite{MHaser93,MSchutz99}, the exact HF exchange is central to hybrid HF/DFT models 
%\cite{}.
The use of hybrid methods in the condensed phase has been pioneered by the {\sc CRYSTAL} group,
demonstrating blah gnu snort for difficult problems \cite{Crystal98}.
...
%\cite{} 
and linear scaling algorithms for computing the periodic HF exchange \cite{CTymczak04B}, it is now possible
to perform condensed phase  HF/DFT calculations on systems larger than 500 atoms with a single processor.
...
approximation are introduced, and the avoidance linear dependencies is discused. Next, in 
...
$ associated with integration over $V_{\rm UC}$ and the corresponding density 
...
phase \cite{MTowler00}.  Also, there are at least two (albiet related) libraries \cite{CRYSTALLib,TowlerLib} of Gaussian 
...
the cluster.    Second, it correctly takes into account the angular symmetry  of the primative Gaussian distributions.   
...
While recuring down the density tree to the level of individual primitives precludes well developed technologies 
...
For this form,  Cramer's inequality \cite{} 
...
distribution $\rho_q$ yields an accumulation less than $\tau_{\rm \scriptscriptstyle ov}$;
\begin{equation}
C_q e^{ -\widetilde \zeta_q (R^{\rm o} _q)^2 }  = \tau_{\rm \scriptscriptstyle ov} .
\end{equation}
...
by Challacombe, White and Head-Gordon \cite{MChallacombe97D} to lattice summation of the irregular solid harmonic multipole interaction tensor. 
This Ewald-like summation is taken over the periodic far field, $V_{\rm PFF}$, and is equivalent to a  direct lattice summation (not a true Ewald sum) 
excluding an inner region, $V_{\rm In}$, surrounding the unit cell.  This inner region has been subtracted to avoid penetration
errors and to guarantee convergence of the multipole expansion. 
With the summed interaction tensors cheaply precomputed and reused, the cost of Coulomb summation over the PFF 
scales as as ${\cal O}(N p^2)$, where $p$ is the order of the multipole expansion.  With this partition,  the  $N$-scaling periodic quantum Coulomb sums 
...
${\bf J}^{\rm PFF}$ will be developed in the following section, while in Section~\ref{tinfoil} the term ${\bf J}^{\rm TF}$, necessary to 
...
and is {\em not} equivalent to Ewald summation.  Nevertheless, with this simplification,  the ${\cal O}(p^2 N)$  working equation 
\begin{equation}
\label{WorkingPFF}
...
${\bf J}^{\rm In}$.  With QCTC accuracy is controlled on the fly by the MAC and PAC, establishing a dynamic near/far-field partition, while 
...
${\cal C}$ is the asymtotic Uns{\"o}ld weight of the total density and $\tau_{\rm \scriptscriptstyle MAC}$ is the threshold controlling the 
translation errors. See Ref.~\onlinecite{CTymczak04c} for development of this expression  and further explenation of these parameters.
...
\caption{Transformation between  the unit cell with volume  $V_{\rm UC}$ (described by the primitive lattice vectors {\bf a}, {\bf b} and {\bf c}) 
...
\label{Kxc_pbc_0}
\end{equation}
While we have written Eq.~\ref{Kxc_pbc_0} in terms of the exchange-correlation for simplicity, in practice 
HiCu employs the Pople, Gill and Johnson formulation \cite{JPople92}. 
%
...
The solution to this problem  again comes from the ray tracing literature, in the form of a modified ray-AABB \cite{MGomez99} and sphere-AABB test \cite{GGems}.  
...
\begin{table}
\caption{Comparison of {\sc CRYSTAL} and {\sc MondoSCF} $\Gamma$-point calculations on  NaCl at the 
8-511/8-631G/BLYP level of theory at a {\tt TIGHT} accuracy level. The lattice coordinates for 
$N_{\rm at}=2$ and 8 are  are given in Ref.~[\onlinecite{PBCCoordinates}].}
\label{NaCl8511}
\center{
\begin{tabular}{lcll}
\toprule
...
{\sc CRYSTAL}     & $2^f$  &  -622.39114    & -311.19557    \\
{\sc MondoSCF}      & $8^g$  &  -2490.0016   & -311.25020  \\
{\sc CRYSTAL}     & $8^g$  &  -2490.0013   & -311.25016   \\
\botrule
$ ^f$Triclinic \\
$ ^g$Cubic
\end{tabular}}
...
\caption{Convergence of the $\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         $k$-space integration with {\sc CRYSTAL} for NaCl at the STO-3G/LDA level
         of theory at a {\tt TIGHT} accuracy level.}\label{NaClGammaPt}
\center{
\begin{tabular}{lcll}
\toprule
Program         & $N_{\rm at}$              & Energy (Hatree) & Energy/$N_{\rm at}$\\ 
...
\hline
{\sc CRYSTAL}$^h$ & 2$^f$    &  -611.09228    & -305.54614    \\ 
\botrule
$ ^f$Triclinic \\
$ ^g$Cubic  \\
...
\begin{table}
\caption{Convergence of the$\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         $k$-space integration with {\sc CRYSTAL} for MgO at the 8-61G/8-51G/BLYP
         level of theory at a {\tt TIGHT} accuracy level. The lattice coordinates for 
	 $N_{\rm at}=2$ and 8 are from Ref.~[\onlinecite{PBCCoordinates}].}
\label{MgOGammaPt}
\center{
\begin{tabular}{lrll}
\toprule
...
\hline
{\sc CRYSTAL }$^h$ & 2$^f$    &   -275.47547  & -137.73774  \\ 
\botrule
$ ^f$Triclinic \\
$ ^g$Cubic  \\
\multicolumn{4}{l}{$ ^h6\times6\times6$ $k$-space grid.}
...
\caption{Convergence of the $\Gamma$-point super-cell approximation for Diamond, 
         computed with {\sc MondoSCF} at the 6-21G*/BLYP level of theory at a {\tt GOOD} accuracy level. The  
         lattice coordinates for $N_{\rm at}=8$ are from Ref.~[\onlinecite{PBCCoordinates}].}
...
\section{Validation}\label{validate}
...
NaCl and MgO obtained with the {\sc MondoSCF} {\tt TIGHT} precision level 
...
delivering a relative precision of 8-9 digits in the converged total energy.  
...
The {\tt BASIS} parameter determines the auxiliary functions used to fit the exchange-correlation potential
...
diamond, using the {\sc MondoSCF} {\tt GOOD} precision (targeting 6 digits), the BLYP functional and the
...
of cubic diamond at the 6-21G*/BLYP level of theory using a {\tt GOOD} (6 digit) level of precision}
...
builds with  6-21G*/BLYP diamond at its standard density.  These timings correspond to a {\tt GOOD} level of
accuracy, targeting 6 digits in the total energy and corresponding to the values listed in Table\ref{DiamondGammaPt}.
...
boundary conditions into the linear scaling quantum chemistry code {\sc MondoSCF}.
%
...
%
We then showed the capabilities of {\sc MondoSCF} for three test condensed
phases , Sodium Chloride, Magnesium Oxide and Carbon in the Diamond
structure. We then compared these results to the periodic
Gaussian code {\sc CRYSTAL}. 
In all cases we achieved excellent comparison with the  {\sc CRYSTAL}
results, including k-space summation in comparison with ever 
increasing super-cells, to within the accuracy that {\sc CRYSTAL} can obtain.
%
Finally, we demonstrated that {\sc MondoSCF} achieves {\it true} linear scaling
...
\end{equation}
and \( \beta  \) is a parameter which controls the convergence, and $\Gamma(n,r)$ is the
incomplete gamma function \cite{GArfken01}.
...
%
\begin{eqnarray}
{\cal M}^{m}_{l} & = & \sum _{\mathbf{R}'\in V_{{\rm out}}}\, M_{l}^{m}[\mathbf{R}]
\nonumber\\
 & = & \sum _{\mathbf{R}'\in V_{{\rm out}}}\widetilde{P}_{l}^{m}\left( \cos \theta _{\mathbf{R}}
...
\begin{eqnarray}
{\cal M}^{m}_{l} &=&\sum _{\mathbf{R}'\in V_{{\rm out}}}\, \widetilde{P}_{l}^{m}
...
{\cal M}^{m}_{l} & = & \sum _{{\mathbf{R}'}\in V_{{\rm out}}}\, M_{l}^{m}[\mathbf{R}]
\nonumber\\
& = & \sum _{{\mathbf{R}'}\in V_{{\rm out}}}\, \widetilde{P}_{l}^{m}\left( \cos \left( 
...
\begin{eqnarray}
{\cal M}^{m}_{l} 
 & = & \sum _{{\mathbf{R}'}\in V_{{\rm out}}}\, \left( l-m\right) !\, P_{l}^{m}\left( \cos 
\left( \theta _{\mathbf{R}}\right) \right) e^{im\phi _{\mathbf{R}}}\, {\cal G}_{l}\left( \beta ,\left| 
\mathbf{R}\right| \right) 
\nonumber\\
 & - & \sum _{{\mathbf{R}'}\in V_{\rm in}}\, \left( l-m\right) !\, P_{l}^{m}\left( \cos \left( \theta _{
...
where,
\begin{eqnarray}
{\cal C}_1 = \frac{4\pi ^{\frac{3}{2}}(\frac{i}{2})^{l}}{V_{\rm UC}\Gamma \left( l+\frac{1}{2}\right) }
\nonumber\\
{\cal C}_2 =  \frac{4\pi ^{\frac{3}{2}}(\frac{i}{2})^{l}}{A_{\rm UC}\Gamma \left( l+\frac{1}{2}\right) }
\label{C10p}
\end{eqnarray}
and \( A_{UC} \) is the area of the unit cell along the non-periodic
...
the distribution threshold  $\tau_{\rm \scriptscriptstyle DIST}$ described in Section \ref{datastruct},
...
Coulomb matrix as discussed in Section \ref{datastruct}, where   $\tau_{\rm \scriptscriptstyle 2E}=
\tau_{\rm \scriptscriptstyle PAC}=\tau_{\rm \scriptscriptstyle MAC}$.
