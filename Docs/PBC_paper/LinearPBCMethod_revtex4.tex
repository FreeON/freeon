...
\end{equation}
corresponding to the three separate regions shown in Fig.~\ref{regions}.   Here, ${\bf J}^{\rm in}$ is computed using 
the fast ${\cal O}(N {\rm lg} N)$ QCTC algorithm outlined previously in Section~\ref{datastruct}. Construction of ${\bf J}^{\rm PFF}$ 
...
In Table~\ref{NaCl8511}, comparision is made for $\Gamma$-point NaCl with the 8-511G\cite{CBS:8511G:Na} basis set
for the sodium, the 8-631G\cite{CBS:8631G:Cl} basis set for the clorine and BLYP functional \cite{DFT_LYP:88,BLYP:88}.
%
...
6-31G*\cite{CBS:621G:C} basis set.  Since {\sc MondoSCF} employs 6-$d$ and 10-$f$ functions, while {\sc Crystal98} employs 
