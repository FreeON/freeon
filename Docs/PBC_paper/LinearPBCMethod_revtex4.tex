...
In recent years several successful methods for the simulating molecular or 
condensed matter systems at the atomic level have been developed
\cite{Metropolis49,Alder59,Slater51,Hohenberg64}
These methods all attempt to solve for the electronic structure via
the many-body quantum problem to within some consistent approximation scheme.
several examples include Hatree-Fock \cite{Slater51,Roothaan51}, Density Functional theory 
\cite{Hohenberg64,KohnSham65} and Configuration interaction \cite{Choudhury79}. 
...
scale very poorly with system size, ($O(N^3)$), which severely limits the systems 
...
chemistry codes have yet to incorporate Periodic boundary conditions, but many
technically important systems can only be addressed within a condensed environment,
where surface effects have been removed \cite{Allen90}. Therefore, the inclusion of 
periodic boundary conditions into an existing linear scaling quantum chemistry
codes is essential. 
...
code is large systems, where k-space integrations become unnessary for insulators 
\cite{Nunes94}.
...
\end{equation}
where the effective halmiltonian is
\begin{equation}
...
\end{equation}
and the exchange correaltion potential can be calculated from several approximated 
functionals
\cite{Becke92,Hertwig97,Bauschlicher95,Adamo00}. In what follows we will 
discuse the calculation of each of the terms in the hamiltonian 
...
local distributions via Periodic Loacalization transformation  and equations (\ref{rho_pbc})
and (\ref{rho_pbc_ab}), 
...
\label{EW_pot}
\end{equation}
where $V$ is the volume of the simulation cell, $R_0$ is a arbitrary refference point in the cell, 
${\bf D}$ is the dipole moment and $Q$ is the quadripole moment.
...
SlaterDirac and B3LYP. We obtain excellent agreement with \textbf{Crystal98} to the accuracy
...
system size, and is akin to k-space integration, and also has excelent agreement
...
we show the convergence of the energy per atom of a Mgnesium Oxide
test system for increasing system size and we also has excelent agreement
with \textbf{Crystal98} results in which we do k-space integration.
...
test system for increasing system size and we again have excelent agreement
with \textbf{Crystal98} results in which we do k-space integration.
...
early. We also show in figure \ref{figure:EnergyperNandError} the energy per atom and 
the relative errror in the total energy. Agian we see the convergence of the total energy
...
\section{Big, Cool Example System}
...
data structures (k-d trees) that maximally exploits locality of the
density, and allows for an efficient treatment of the periodic effects.
Next, we show the capabilities of \textbf{MondoSCF} for three test condensed
...
some conclusion concerning the accuracy and utility of \textbf{MondoSCF} and
...
Sodium Chloride test system. The \textbf{MondoSCF} calcuations where done to
the accuracy of the quoted digets. All calculations where done at the Gamma point
for comparision except the last \textbf{Crystal98} calculation, which was done at ${\bf k}=(6,6,6)$.
...
Magnesium Oxide test system.   The \textbf{MondoSCF} calcuations where done to
the accuracy of the quoted digets. All calculations where done at the Gamma point
for comparision except the last \textbf{Crystal98} calculation, which was done at ${\bf k}=(6,6,6)$.
...
Diamond test system.  The \textbf{MondoSCF} calcuations where done to
the accuracy of the quoted digets. All calculations where done at the Gamma point
for comparision except the last \textbf{Crystal98} calculation, which was done at ${\bf k}=(2,2,2)$.
...
shown is the HiCu cube in which we do the exchange-correlation intergrals}
...
Crysral98 basis set 6-21G* \cite{C98Basis} and the density functional BLYP \cite{Becke92}}
...
periodic system up to 512 atoms using the Crysral98 basis set 6-21G* \cite{C98Basis} and the 
density functional BLYP \cite{Becke92}. Also shown on the right axis is the relative errors
in the total energy}
...
of a 512 atom diamond system using the  Crysral98 basis set 6-21G* \cite{C98Basis} and the 
