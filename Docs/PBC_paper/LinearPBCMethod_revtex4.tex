...
\title{Linear scaling computation of the Fock matrix. VII.  
       Periodic \\ boundary conditions for the pure Density Functional Theory.}
...
{\bf CJ, to do list:}
\begin{itemize}
\item Need a correct FMM estimate and appropriate citations for PFF: how much do we want to 
      say about QCTC error bounds.  In the static limit, are they really different from the FMM estimate?
\item Need specific details of benchmark calculations (how many digits, etc?) and a much much much 
      better accounting of the various thresholds used.  Good way to do this is with footnotes.
\item Need the geometry details for NaCl
\item Need correct citations (not just generic website) for all the crystal basis sets used
\item Need the correct BLYP citations
\item Need the specific details of how errors computed for diamond
\end{itemize}
{\bf Matt to do list:}
\begin{itemize}
\item Intro, HiCu, Conclusions, Appendix A.
\end{itemize}
...
yield all possible translational symmetries through variation of the reciprocal lattice vector $\bf k$.     
...
primarily for the study of defects and vacancies rather than as a replacement for $\bf k$-space integration,
...
the unit cell volume, $V_{\rm UC}$ are retained. At first sight this would seem to make matrix construction quite 
different than in the gas phase,  where integrals are typically taken over all space, $V_\infty$.  
...
allowing use of conventional analytic integral technologies \cite{twoels}.  For example, 
elements of the periodic overlap matrix become
\begin{equation}
\label{Sab_pbc2}
S_{ab}=\sum _{\mathbf{R}}\int _{V_{\infty }}\, d{\mathbf{r}}\, \phi _{a}
({\mathbf{r}})\phi _{b}({\mathbf{r}+\mathbf{R}}).
\end{equation}
It is convenient to define the corresponding intermediate basis function products (distributions)
...
electron density $\rho^{\scriptscriptstyle \infty}({\bf{r}})= \sum _{ab} P_{ab} \rho _{ab}^{\scriptscriptstyle \infty}({\mathbf{r}})$,
where $\bf P$ is the one-electron reduced density matrix.  
In this convention, elements of periodic overlap matrix are simply 
...
exponential prefactor factor $e^{- \chi_{ab} ({\bf A}-{\bf B}-{\bf R})^2}$ corresponding to each term 
in term  $\rho^{\scriptscriptstyle \infty}_{ab}$.  Thus, non-zero terms resulting from the sum over 
over {\bf A}, {\bf B} and {\bf R} are easily eliminated {\em a priori} with a simple distance test.
However, for small exponents  $\chi_{ab}$, care must be exercised in 
truncation of periodic sums to avoid overlap matrices that are not positive definite.  While these situations
...
increasing the most diffuse uncontracted exponent(s). {\bf Need a few lines of literature review here}.
...
Also, the current version of QCTC is quite different from previous descriptions \cite{}, and deserves some
introduction.  
...
containing the spatial "extent" of each node and its children.    This scheme is almost identical to
well developed technologies for ray tracing and data base searches, allowing allowing fast 
${\cal O}({\rm lg} N)$ look up of overlapping components through AABB intersection tests \cite{}. 
In the case of QCTC, this fast look up constitutes the penetration acceptability criterion (PAC) \cite{}, which identifies
preliminary agglomerations $Q$ of the density that may be accurately represented via a multipole 
...
representation of the total density, leading to a best case, on the fly partition of near-field (NF) and far-field (FF) 
...
J_{ab} &=& \sum_{Q\in \rm FF} \sum_l \sum_m O^l_m[\rho_{ab}] \sum_{l'} \sum_{m'} M^{l+l'}_{m+m'} O^{l'}_{m'}[\rho_Q]  \nonumber \\
       &+& \sum_{q\in \rm NF} \int d {\bf r} \int d {\bf r'} \rho_{ab} ({\bf r}) \left|{\bf r}-{\bf r'} \right|^{-1} \rho_q({\bf r}) 
\end{eqnarray}
where $M^l_m$ is the irregular solid harmonic interaction tensor, $O^l_m[f]=\int d{\bf r} O^l_m({\bf r}) f({\bf r})$
is a moment of the regular solid harmonic\cite{}, 
...
The QCTC near/far-field partition pushes the multipole approximation to the limit, employing best case error estimates 
that depend on magnitude and extent of both bra and ket and allowing tree traversal to the resolution of 
primitive distributions.  While resolving individual primitives forgoes well developed technologies for the 
integral evaluation of contracted functions, it greatly accelerates the onset of linear scaling through the early
agglomeration of close centers.

The Quantum Chemical Tree Code generally employs the total density, which simplifies the code, allows 
electrostatic screening in MAC error estimates \cite{} and provides charge neutrality, an essential
feature for periodic calculations.  Thus, the Coulomb matrix we compute includes the electron-nuclear terms;
...
For both of these fast algorithms, the trade off between efficiency and accuracy is controlled by the AABB,  which in turn 
...
Both HiCu and QCTC employ a Hermite Gaussian (HG) representation of distributions
\begin{equation}
...
the use of McMurchie-Davidson recurrence relations \cite{} in analytic integral evaluation and density evaluation.
For this form,  Cramer's inequality \cite{} provides a bound for the behavior of a Hermite-Gaussian distribution: 
\begin{equation}
...
\begin{figure}
\resizebox*{3.5in}{!}{\includegraphics{Extent.eps}}\label{extent}
\caption{Behavior of the overlap extent $R^o$ and the penetration extent $R^p$ 
	 as a function of $\tau/C$ for an $s$-type Gaussian with exponent $\zeta=0.1$.
         For large $C$ and small $\tau$, $R^o$ and $R^p$ asymptote to the same value. However, 
         for small $C$ (occurring for example due to a large atom-atom separation and/or small density 
...
In Fig.~\ref{regions}, differences between the penetration and overlap extent are shown for a diffuse $s$-type
...
which was first described by Nijboer and De Wette (NDW) \cite{} and later reviewed and extended by us \cite{}
to lattice summation of the irregular solid harmonic multipole interaction tensor. 
This Ewald-like summation is taken over the periodic far field, $V_{\rm PFF}$, which is direct lattice summation excluding 
an inner region, $V_{\rm in}$, surrounding the unit cell.  This inner region has been subtracted to avoid penetration
errors and to guarantee convergence of the multipole expansion.  Because the summed interaction tensors maybe cheaply 
precomputed and reused, the cost of Coulomb summation over the PFF scales as as ${\cal O}(N p^2)$, where $p$ is the order 
of the multipole expansion.  With this feature, achieving $N$-scaling periodic quantum Coulomb sums involves the contributions
\begin{equation}
{\bf J} = {\bf J}^{\rm in} + {\bf J}^{\rm PFF} + {\bf J}^{\rm TF} \, ,
\end{equation}
corresponding to the three separate regions shown in Fig.~\ref{regions}.   Here, $J^{\rm in}$ is computed using 
the fast ${\cal O}(N {\rm lg} N)$ QCTC algorithm outlined previously in Section~\ref{datastruct}. Construction of ${\bf J}_{\rm PFF}$ 
will be developed in the following section, while in Section~\ref{tinfoil} the term ${\bf J}_{\rm TF}$ necessary to 
...
convergence of multipole expansion errors, as outlined in Fig.~\ref{regions} and discussed in the following. 
...
it with cheaper, multipole work in the computation of $J^{\rm PFF}$.  To this end, the inner region $V_{\rm in}$ is constructed 
from neighboring cells that have simple Gaussian overlap with the unit cell, defined by the radius $R^{o}$.  
As explained in Section~\ref{datastruct}, for the relatively large distances considered at this level the differences between the
...
This static error is controlled by using the FMM-like error estimate 
\begin{equation}
\frac{ {d}_{\rm max}^{2 p+1}}
{({R}^o)^{2 p+1}\left| {R}^o -2 {d}_{\rm max} \right| } 
> \epsilon \;,
\end{equation}
where $d_{\rm max}$ is the maximum translational distance, $R^o$ is the radius eliminating Gaussian overlap 
between the unit cell and $V_{\rm PFF}$, and $\epsilon$ is a target error.
({\bf OK CJ, we have to get our shit together with this estimate:  We don't really have an $C_\rho$ that
...
iinvariance and often incur dramatic charge sloshing instabilities.   The correction is 
...
\label{EW_pot}
\end{equation}
where ${\bf D}$ is the system dipole moment and $Q$ is the trace of the system quadrupole, and we have assumed origin centering, 
...
\end{equation}
where $S_{ab}$ is an element of the overlap matrix, and ${\bf d}_{ab}$ is the origin centered
...
\caption{\label{figure:SimCell} Simulation cell and its Bravais lattice vectors. Also
shown is the HiCu cube in which we do the exchange-correlation integrals}
{\centering \includegraphics{UnitCell_2.ps} \par}
\end{figure}

\section{The exchange-correlation matrix}
...
\caption{Comparison of {\sc Crystal98} and {\sc MondoSCF} $\Gamma$-point calculations on  NaCl at the 
         8-511/BLYP level of theory. The lattice coordinates for $N_{\rm at}=2$ and 8 
         are listed in Appendix~\ref{FracCoords}.}\label{NaCl8511}
...
\colrule
{\sc MondoSCF}      & $2^f$  &  -622.391011  & -311.195506  \\
{\sc Crystal98}     & $2^f$  &  -622.3911    & -311.1955    \\
{\sc MondoSCF}      & $8^g$  &  -2490.00163  & -311.250204  \\
...
\tablenotetext[6]{Triclinic}
\tablenotetext[7]{Cubic}
\begin{tabular}{lrll}
\toprule
Program         & $N_{\rm at}$              & Energy (hartree) & Energy/$N_{\rm at}$\\ 
\colrule
...
\hline
{\sc Crystal98}\footnote[8]{$6\times6\times6$ $k$-space grid.}
...
\begin{table}
\caption{Convergence of the$\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         $k$-space integration with {\sc Crystal98} for MgO at the 8-61G/BLYP
         level of theory. The lattice coordinates for $N_{\rm at}=2$ and 8 
         are listed in Appendix~\ref{FracCoords}.}\label{MgOGammaPt}
\tablenotetext[6]{Triclinic}
\tablenotetext[7]{Cubic}
\begin{tabular}{lrll}
\toprule
Program         & $N_{\rm at}$              & Energy (hartree) & Energy/$N_{\rm at}$\\ 
\colrule
{\sc MondoSCF}  & 2$^f$    &  -275.090965  & -137.545482  \\
                & 8$^g$    &  -1101.72945  & -137.716181  \\
                & 16$^f$   &  -2203.69044  & -137.730653  \\
                & 54$^f$   &  -7437.79892  & -137.737017  \\
                & 64$^g$   &  -8815.21310  & -137.737705  \\
                & 128$^f$  &  -17630.4304  & -137.737737  \\
                & 216$^g$  &  -29751.3517  & -137.737739  \\ 
\hline
{\sc Crystal98}\footnote[8]{$6\times6\times6$ $k$-space grid.}
                & 2$^f$    &   -275.47547  & -137.73774  \\ 
\botrule
\end{tabular}
\end{table}
...
\caption{Convergence of the $\Gamma$-point super-cell approximation for Diamond, 
         computed with {\sc MondoSCF} at the 6-21G*/BLYP level of theory. The  
         lattice coordinates for $N_{\rm at}=8$ are listed in Appendix~\ref{FracCoords}.}\label{DiamondGammaPt}
\begin{tabular}{rll}
\toprule
 $N_{\rm at}$  & Energy (hartree) & Energy/$N_{\rm at}$\\ 
\colrule
  8    &   -303.989 &  -37.9986 \\
  16   &   -608.667 &  -38.0417 \\
  32   &   -1218.02 &  -38.0632 \\
...
All calculations were implemented in a serial version of {\sc MondoSCF} v1.0$\alpha$7, a suite of 
linear scaling Quantum Chemistry codes\cite{MondoSCF}.  The code was compiled using the Portland 
...
highly optimized by symbolic manipulation and factorization, using real arithmatic and expansions through
7'th order in the calculation of ${\bf J}_{\rm in}$. The computation of ${\bf J}_{\rm PFF}$ 
...
matrix thresholds $\tau_{\rm trix}=10^{-5}$ for "good" and $\tau_{\rm trix}=10^{-6}$ for "tight" 
levels of precision.  All calculations were performed with $C_1$ (no) symmetry.
\newpage
\newpage
...
\caption{\label{ErrorPFF} Error in the Coulomb energy computed with the 
Nijboer and De Wette scheme relative to true Ewald summation.  Shown
is the approach with $p$ to zero with one (ws=1) and two (ws=2) layers of
cells in $V_{\rm PFF}$ for a periodic system of 64 classical water molecules.
{\bf CJ, this figure needs to be relabled.  Please use $p$ instead of L.
Also, please use ws=1 and ws=2 instead of one layer and two layer.
Also, is this just the PFF energy or IN+PFF ??
Also, is this natural log or log10? }
}
\end{figure}
...
The ability of our implimentation to reproduce true Ewald summation is shown in 
Fig.~\ref{ErrorPFF} for a periodic system of 64 classical water molecules.
...
peculiar to a code. Here, we make connection with the preemiminent Gaussian orbital 
...
condensed phase\cite{}, which tend to have less diffuse valence orbitals.  
Tables~\ref{NaCl8511}-\ref{MgOGammaPt} make a direct comparsion with {\sc Crystal98} for 
NaCl and MgO obtained with the {\sc MondoSCF} "tight" precision level 
...
delivering a relative precision of {\bf 8????} digits in the converged total energy.  
...
In Table~\ref{NaCl8511}, comparision is made for $\Gamma$-point NaCl with the 8-511G\cite{} basis set
and BLYP functional\cite{}, while in Table~\ref{NaClGammaPt} convergence of the supercell $\Gamma$-point approximation 
is demonstrated for NaCl with the STO-3G basis set and the local density approximation\cite{}.
In Table~\ref{MgOGammaPt}, convergence of the {\sc MondoSCF} supercell $\Gamma$-point approximation
to the {\sc Crystal98} $k$-space result is demonstrated for the 8-61G basis set \cite{} and the BLYP functional.
Next, in Table~\ref{DiamondGammaPt} convergence of the supercell $\Gamma$-point approximation is shown for
diamond, using the {\sc MondoSCF} "good" precsion (targeting 6 digits), the BLYP functional and the
6-31G* basis set.  Since {\sc MondoSCF} employs 6-$d$ and 10-$f$ functions, while {\sc Crystal98} employs 
5-$d$ and 7-$f$, we were not able to make a direct comparison.
...
\caption{Computational complexity for the $\bf J$ and $\bf K_{\rm xc}$ matrix builds
of cubic diamond at the 6-21G*/BLYP level of theory using a "good" (6 digit) level of precision}
\end{figure}

\begin{figure}
\centering \includegraphics{ErrorVsNumber_Diamond512.ps} 
\label{ErrorPerN} 
\caption{Relative error with system size for 6-21G*/BLYP diamond targeting a precision 
         of 6 digits in the total energy. {\bf if log10, please label log10!}}
\end{figure}

Demonstrating perfect linear scaling at the outset,  Fig.~\ref{Scaling_Matrix_Build} shows the CPU time for $\bf J$ and $\bf K_{\rm xc}$
builds with  6-21G*\cite{}/BLYP diamond at its standard density.  These timings coorespond to a "good" level of precision, 
targeting 6 digits in the total energy and corresponding to the values listed in Table\ref{DiamondGammaPt}.
Shown in Fig.~\ref{} is the precision of the computed energies, obtained by performing a second set of calculations with
{\bf CJ, exactly what happend in these calculations?  Converged SCFs or what?}.  For these calculations, the 
largest source of error is the numerical integration performed by HiCu, as the QCTC thresholds are significantly more 
conservative. 
...
\section{Lattice Coordinates}\label{FracCoords}

\begin{table}
\caption{Fractional coordinates for the triclinic 2 atom unit cell of NaCl, cooresponding to the 
         calculations reported in Tables~\ref{NaCl8511} and \ref{NaClGammaPt}.  Length of the unit cell is 
         $a=b=c=XX$\AA ~~{\bf DOUBLE CHECK}, with angles $\alpha=\beta=\gamma=60^\circ$.}
\begin{tabular}{cccc}
\toprule
 Atom &  X  & Y  & Z \\
\colrule
...
\caption{Fractional coordinates for the triclinic 2 atom unit cell of MgO, cooresponding to the 
         ...
         $a=b=c=2.977776807$\AA ~~{\bf NOT AU!!}, with angles $\alpha=\beta=\gamma=60^\circ$.}
\begin{tabular}{cccc}
\toprule
 Atom &  X  & Y  & Z \\
\colrule
...
\caption{Fractional coordinates for the 8 atom unit cell of Diamond, cooresponding to the supercell calculations
         reported in Table~\ref{DiamondGammaPt}.  Dimensions of the cubic unit cell are $a=b=c=3.57$\AA ~~{\bf NOT AU}.}
\begin{tabular}{cccc}
\toprule
 Atom &  X  & Y  & Z \\
\colrule
