...
Recently, quantum chemical methods for gas phase Density Functional Theory (DFT) 
have been developed which scale linearly with system size, $N$.  In this contribution, 
these methods are extended to periodic boundary conditions at the $\Gamma$-point.  

Quantum chemical methods typically employ Gaussian Type Atomic Orbitals (GTAOs), 
which offer a number of advantages. First, because they are local basis functions, it is possible 
to achieve linear scaling in insulating systems for both construction of the effective Hamiltonian 
or Fockian, as well as for solution of the density matrix.  Secondly,  almost all one and two electron integrals 
involving GTAOs are analytic, enabling the 
rapid evaluation of expectation values involving complicated operators often involved in response properties.
...
approximation in computation of relativistic effects with the four-component Dirac-Hartree-Fock \cite{dirac} 
and Dirac-Kohn-Sham \cite{rel4d} theories.  Perhaps most important though, the exact Hartree-Fock (HF) exchange
may be computed efficiently with a GTAO basis set.  In addition to providing a reference for 
correlated wavefunction methods \cite{}, the exact HF exchange is central to hybrid HF/DFT models \cite{}.
...
results presented here, $ {\cal O}(N)$ methods for solving the Self-Consistent-Field equations \cite{} and 
linear scaling algorithms for computing the periodic HF exchange \cite{CTymczak04B}, it is now possible
...
In Section~\ref{coulombsums}, we extend previous work on the Niboer and De Wette \cite{} lattice sum 
...
% Changed From Here
%
For accepted agglomerations a second test, the multipole acceptability criterion (MAC),
is performed to check translation errors in the multipole  expansion. This second test is critical
to the overall accuracy of the Coulomb matrix build. Because of this, 
in Ref.~[\onlinecite{CTymczak04c}] a new more accurate MAC is developed. This new MAC has several advantages
over commonly used MAC's.
%\cite{} 
First, it takes into account the decay of the multipole interaction; Second, it takes into account the weight of 
the distribution within the agglomeration; Third, it correctly takes into account the angular momentum type of the 
primative gaussian distributions; And finally, and most importent, if is always an exact bound to the multipole 
translation error. 
%
%
%
...
\leq \tau_{MAC} \;,
\end{equation}
where $d_{\rm max}$ is the maximum translational distance, ${\cal C}$ is the Asymtotic Uns{\"o}ld weight of the total density
and $\tau_{MAC}$ is the threshold controlling the precision of the calculation.
This expression is developed in Ref.~\onlinecite{CTymczak04c},
stemming from our new error bounds for the tree-code MAC that is both cheap and sensitive to the particulars
of the expansion, including magnitude of the charges etc, which is captured by the  Asymtotic Uns{\"o}ld weight 
${\cal C}$.  Because ${\cal C}$ will fluctuate slightly with the density between self-consistent-field cycles, so to will the
expansion order~$p$.  
...
\caption{Convergence of the $\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         $k$-space integration with {\sc CRYSATAL} for NaCl at the STO-3G/LDA level
...
\begin{table}
\caption{Convergence of the$\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         $k$-space integration with {\sc CRYSATAL} for MgO at the 8-61G/8-51G/BLYP
