...
{\bf CJ, to do list:}
\begin{itemize}
\item round values in tables to 8'th digit.
...
{\bf Matt to do list:}
\begin{itemize}
\item Intro, Conclusions, Appendix A.
\end{itemize}
...
\section{periodic 
boundary conditions, linear scaling and basis sets}
...
matrix are expressed simply  as $S_{ab}=\int  d{\mathbf{r}}\, \rho^{\scriptscriptstyle \infty}_{ab}({\mathbf{r}})$, while the electron count is
...
criteria, they are often due to the use of basis sets designed for gas phase calculations in conjunction with small UCs.  
...
by Gr{\"u}neich and He\ss \cite{AGruneich98} for even tempered basis sets, is to scale the exponents by the inverse square 
of the lattice constant.  In many cases though, especially for large systems, standard quantum chemistry basis sets
work well \cite{}.
...
${\cal O}({\rm lg} N)$ {\em range queries} of overlapping components through AABB intersection tests \cite{}. 
...
that depend on the magnitude and extent of both bra and ket and allowing tree traversal to the resolution of 
...
In Fig.~\ref{extent}, differences between the penetration and overlap extent are shown for a diffuse $s$-type
...
it with cheaper, multipole work in the computation of $J^{\rm PFF}$.  To this end, a fixed inner region $V_{\rm in}$ is constructed 
...
and $\tau$ is a threshold controling the precision. This expression is developed in Ref.~\onlinecite{CTymczak03c},
...
\label{EW_pot}
\end{equation}
where ${\bf D}$ is the system dipole moment and $Q$ is trace of the system quadrupole, and we have assumed origin centering, 
The tin-foil correction to the Coulomb matrix is then 
\begin{equation}
J_{ab}^{TF}  = \frac{2\pi }{3V_{\rm UC}} \left(  Q\, S_{ab}-2 \, \mathbf{d}_{ab}\cdot \mathbf{D} \right) \label{JTC}
\end{equation}
...
\begin{figure}
\resizebox*{3.5in}{!}{\includegraphics{BoxUC.eps}}
\label{BoxUC}
\caption{Transformation between the rectangular integration volume $V_\Box$ employed by HiCu and the
         unit cell with volume $V_{\rm UC}$, described by the vectors {\bf a}, {\bf b} and {\bf c}.}
\end{figure}
...
shown in Fig.~\ref{BoxUC},  are equivalent due to full periodicity of the distributions 
and the density.  However, because the distributions and density involve a double sum over 
lattice vectors, there will be a large number of atom-atom pairs that do not overlap
with $V_\Box$.  A similar situation is encounted in the gas phase for parallel versions 
of HiCu \cite{CGan02}, where each processor has a small, local cube-tree that may 
overlap only a few of the many possible atom-atom pairs.  

The solution again comes from the ray tracing litterature, in the form of a modified 
