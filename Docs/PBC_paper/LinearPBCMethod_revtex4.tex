...
\title{Linear scaling computation of the Fock matrix VII.  \\
       Periodic boundary conditions for Density Functional Theory.}
...
\item {\it Need the correct BLYP citations}
\end{itemize}
{\bf Matt to do list:}
\begin{itemize}
...
\leq \epsilon \;,
\end{equation}
where $d_{\rm max}$ is the maximum translational distance, ${\cal C}$ is the Uns{\"o}ld weight of $\rho_{tot}^{\infty}$, 
and $\epsilon$ is a target error. See reference \onlinecite{CTymczak03c} for a complete derivation and explanation of this error bound. 
...
\frac{2\pi }{3V_{\rm UC}} \left(  Q - 2 \, {\bf r } \cdot \mathbf{D} \right)
\label{EW_pot}
\end{equation}
...
8-511/BLYP level of theory at a {\it verytight} accuracy level. The lattice coordinates for 
$N_{\rm at}=2$ and 8 are listed in Appendix~\ref{FracCoords}.}\label{NaCl8511}
\center{
\begin{tabular}{lcll}
\toprule
...
\hline
{\sc Crystal98} & 2$^f$    &  -611.09228    & -305.54614    \\ 
\botrule
$ ^f$Triclinic \\
$ ^g$Cubic  \\
...
\caption{Convergence of the$\Gamma$-point super-cell approximation with {\sc MondoSCF} to 
         ...
         level of theory at a {\it tight} accuracy level. The lattice coordinates for $N_{\rm at}=2$ and 8 
...
\hline
{\sc Crystal98} & 2$^f$    &   -275.47547  & -137.73774  \\ 
\botrule
$ ^f$Triclinic \\
$ ^g$Cubic  \\
\multicolumn{4}{l}{$ ^h6\times6\times6$ $k$-space grid.}
...
NaCl and MgO obtained with the {\sc MondoSCF} {\it verytight} precision level 
...
and BLYP functional \cite{Becke88,ABecke96}, while in Table~\ref{NaClGammaPt} convergence of the supercell $\Gamma$-point approximation 
...
diamond, using the {\sc MondoSCF} {\it good} precsion (targeting 6 digits), the BLYP functional and the
...
builds with  6-21G*\cite{}/BLYP diamond at its standard density.  These timings coorespond to a {\it good} level of accuracy, 
...
all thresholds reduced by an order of magnitude.
For these calculations, the largest source of error is the numerical integration performed by HiCu, as the QCTC thresholds 
...
%
\begin{table}
\caption{The various thresholds which enter into our four levels of accuracy}
