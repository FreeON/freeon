...
complicated operators often involved in response properties \cite{}.
...
Thresholds controling the cost to accuracy ratio of HiCu and QCTC
...
controling the MAC is set as $\tau_{\rm \scriptscriptstyle MAC}=10^2 \tau_{\rm \scriptscriptstyle 2E}$.  
...
%It should be pointed out that of all the thresholding schemes,
%those govererining HiCu are the loosest, generally delivering just 4, 6 and 8 digits of accuracy with 
%{\tt LOOSE}, {\tt GOOD} and {\tt TIGHT} respectively.  
...
Eigensolution of the self-consistent-field equations has been used throughout, with the cooresponding
...
for sodium, the 8-631G\cite{CBS:8631G:Cl} basis set for chlorine and using the restricted BLYP functional \cite{DFT_LYP:88,BLYP:88}.
...
to the  $\bf k$-space integration result is demonstrated for MgO, using the 8-61G\cite{CBS:861G:MgO} basis set for magnesium,
the 8-51G\cite{CBS:861G:MgO} basis set for the oxygen, and the restricted BLYP functional.
...
We presented a systematic and efficient approach for incorporating periodic 
