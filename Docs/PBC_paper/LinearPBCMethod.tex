...
To calculate the exact exchange correlation matrix in the periodic
case, we take advantage of the periodicity of the functions being
integrated and the locality of the basis functions {[}ref{]}, this
allows us to show that the exact exchange matrix can be calculated
as

\begin{equation}
\label{Kx_pbc}
K_{ab}=\sum _{\mathbf{RR}'}\sum _{cd}\int d{\mathbf{r}}\, d{\mathbf{r}'}\, \frac{\phi _{a}\left( \mathbf{r}\right) \phi _{c}\left( \mathbf{r}+\mathbf{R}\right) \, P_{cd}\, \phi _{b}\left( \mathbf{r}'+\mathbf{R}'\right) \phi _{d}\left( \mathbf{r}'\right) }{\left| \mathbf{r}-\mathbf{r}'+\mathbf{R}'\right| }
\end{equation}
...
\caption{\label{ConvergMgO}Total energy per atom for magnesium oxide system
for increasing size for 8-511G/HF system. The reference Energy from
Crystal98 is \protect\( E=-137.3380172921\protect \).}
