...
behavior in solids or liquids \cite{Allen90}. We have implemented Periodic
boundary conditions into the linear scaling Quantum Chemistry code \textbf{MondoSCF}
\cite{Challacombe96,Challacombe97,Challacombe99}. For the one electron matrices
this has been achieved by a re-summation over periodic cell images, which because
of locality leads to an efficient calculation of the one electron integrals.
For the two-electron Coulomb matrix, this has been achieved with an exact multipole
expansion of the long range Coulomb field \cite{White97, Challacombe97}. This
yields a spherically summed boundary condition that is easily transformed to
Ewald boundary conditions \cite{Redlack72,Redlack75}. In order to achieve linear
scaling of the local Coulomb field, a Quantum Chemical Tree Code (QCTC) is used
{[}ref{]}. We have also incorporated Periodic boundary conditions into calculations
of the local exchange-correlation matrix. A Hierarchical Cubature (HiCu), pure
Cartesian adaptive grid is used. This method achieves linear scaling through
the use of advanced data structures (k-d trees) that maximally exploits locality
of the density, and allows for an efficient treatment of the periodic effects
{[}ref{]}. Next, we show the capabilities of MondoSCF for three test condensed
phases , Sodium Cloride, Magnesium Oxide and Carbon in the diamond and graphite
structures. We then compare these results to the periodic Gaussian code \textbf{Crystal98}
\cite{Cry98}. \textbf{}Next we demonstrate the utility of the code on very
large systems such as zeolite. Finally we give some conclusion concerning the
accuracy and utility of MondoSCF and future research directions.
...
all space. In the periodic case this is replaced by {[}ref{]},

\begin{equation}
\label{Sab_pbc1}
S_{ab}^{{\rm PBC}}=\sum _{\mathbf{R},\mathbf{R}'}\int _{V_{cell}}\, d{\mathbf{x}}\, \phi _{a}({\mathbf{x}+\mathbf{R}})\phi _{b}({\mathbf{x}+\mathbf{R}'})
\end{equation}
...
lattice vectors and \( {\mathbf{R}}=n{\mathbf{a}}+m{\mathbf{b}}+l{\mathbf{c}} \)
. Because of the locality of the atomic orbitals, we can remove the double sum
over lattice vectors to get

\begin{equation}
\label{Sab_pbc2}
S_{ab}^{{\rm PBC}}=\sum _{\mathbf{R}}\int _{V_{\infty }}\, d{\mathbf{x}}\, \phi _{a}({\mathbf{x}})\phi _{b}({\mathbf{x}+\mathbf{R}})
\end{equation}
...
\begin{equation}
\label{rho_loc}
\rho ^{{\rm loc}}({\mathbf{x}})=\sum _{\mathbf{R}}\sum _{ab}{\mathbf{P}}_{ab}\phi _{a}({\mathbf{x}})\phi _{b}({\mathbf{x}+\mathbf{R}})
\end{equation}
and because of periodicity,

\begin{equation}
\label{rho_pbc}
\rho ^{{\rm PBC}}({\mathbf{x}})=\sum _{\mathbf{R}}\, \rho ^{{\rm loc}}({\mathbf{x}+\mathbf{R}})
\end{equation}
We also define:\begin{equation}
\label{rho_loc_ab}
\rho _{ab}^{{\rm loc}}({\mathbf{x}})=\sum _{\mathbf{R}}\phi _{a}({\mathbf{x}})\phi _{b}({\mathbf{x}+\mathbf{R}})
\end{equation}


\begin{equation}
\label{rho_pbc_ab}
\rho _{ab}^{{\rm PBC}}({\mathbf{x}})=\sum _{\mathbf{R}}\, \rho _{ab}^{{\rm loc}}({\mathbf{x}+\mathbf{R}})
\end{equation}
Which allows us to rewrite the overlap matrix matrix

\begin{equation}
\label{Sab_pbc_3}
S_{ab}^{{\rm PBC}}=\int _{V_{\infty }}\, d{\mathbf{x}}\, \rho ^{{\rm loc}}_{ab}({\mathbf{x}})
\end{equation}
...
The Kinetic energy matrix for the non-periodic case is given by {[}ref{]}
...
\label{Tab_pbc}
T^{{\rm PBC}}_{ab}=-\frac{1}{2}\sum _{\mathbf{R}}\int _{V_{\infty }}\, d{\mathbf{x}}\, \phi _{a}({\mathbf{x}+\mathbf{R}})\nabla ^{2}\phi _{b}({\mathbf{x}})
\end{equation}
...
J_{ab}^{{\rm PBC}} & = & \frac{1}{2}\int _{V_{cell}}\int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm PBC}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm PBC}}\left( \mathbf{x}'\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\begin{array}{c}
\\

\end{array}\label{Jab_pbc1} \\
 & = & \frac{1}{2}\sum _{\mathbf{R}'}\int _{V_{\infty }}\int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'}+{\mathbf{R}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\begin{array}{c}
\\

\end{array}\label{Jab_pbc2} 
\end{eqnarray}
let us separate the Coulomb integral into \[
J_{ab}^{{\rm PBC}}=J_{ab}^{{\rm QCTC}}+J_{ab}^{{\rm PFF}}+J_{ab}^{{\rm EC}}\]
where \begin{eqnarray}
J_{ab}^{{\rm QCTC}} & = & \frac{1}{2}\sum _{{\mathbf{R}'}\in V_{in}}\, \int _{V_{\infty }}\, \int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'+\mathbf{R}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\begin{array}{c}
\\

\end{array}\label{Jqctc} \\
J_{ab}^{{\rm PFF}} & = & \frac{1}{2}\sum _{{\mathbf{R}'}\in V_{\infty }-V_{in}}\, \int _{V_{\infty }}\, \int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'+\mathbf{R}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\begin{array}{c}
\\

\end{array}\label{Jpff} \\
J_{ab}^{{\rm EC}} & = & \frac{1}{2}\sum _{{\mathbf{R}'}\in S_{\infty }}\, \int _{V_{\infty }}\, \int _{S_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'+\mathbf{R}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\begin{array}{c}
\\
...
contributes a shape dependent potential to the center cell. This is shown in
detail in figure \ref{figure: ReplicateCells}. 
...
\frac{1}{2}\int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\approx \frac{1}{2}\sum _{\left\langle i\right\rangle }\int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \Lambda ^{\left\langle i\right\rangle }\left( {\mathbf{x}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\qquad \qquad \qquad \qquad \]
\begin{equation}
\label{QCTC}
...
\end{equation}
where \( L \) is the maxium expansion pole, \( \left\langle i\right\rangle  \)
and \( \left\langle j\neq i\right\rangle  \) are the sums over distributions
determined from the density tree and,

\begin{eqnarray}
\rho _{l}^{m}\left[ {\mathbf{P}}\right]  & = & \int _{V_{\infty }}\, d{\mathbf{x}}\, O_{l}^{m}\left[ {\mathbf{r}-\mathbf{P}}\right] \rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \begin{array}{c}
\\
...
\end{eqnarray}
In reference {[}ref{]} we give a derivation of an exact error bound we use to
compute these integrals in the QCTC algorithm, and we show that this leads to
excellent control over the error.
...
the integral in equation (\ref{Jpff}) with the multipole sum {[}ref{]}

\begin{equation}
\label{jpff_2}
J_{ab}^{{\rm PFF}}=\frac{1}{2}\sum _{{\mathbf{R}'}\in V_{\infty }-V_{in}}\, \int _{V_{\infty }}\, d{\mathbf{x}}\, d{\mathbf{x}'}\frac{\rho _{ab}^{{\rm loc}}\left( {\mathbf{x}}\right) \: \rho ^{{\rm loc}}\left( {\mathbf{x}'+\mathbf{R}'}\right) }{\left| \mathbf{x}-\mathbf{x}'\right| }\qquad \qquad \qquad \qquad \qquad \qquad \qquad 
\end{equation}


\begin{equation}
\label{InBoxError}
=\frac{1}{2}\sum _{{\mathbf{R}'}\in V_{\infty }-V_{in}}\, \sum _{l=0}^{L}\, \sum _{l'=0}^{L'}\, \sum _{m=-l}^{l}\, \sum _{m'=-l'}^{l'}\, \left( -1\right) ^{l}\, \rho _{l}^{m}[{\mathbf{P}}]\, M_{l+l'}^{m+m'}[{\mathbf{P}-\mathbf{Q}}]\, \rho _{l'}^{m'}[{\mathbf{Q}+\mathbf{R}'}]
\end{equation}
which setting \( P=R_{0} \) and \( Q=R_{0}-R' \) we can rewrite as\begin{equation}
\label{sp_jpff_2}
J_{ab}^{{\rm PFF}}=\frac{1}{2}\sum _{l=0}^{L}\, \sum _{l'=0}^{L'}\, \sum _{m=-l}^{l}\, \sum _{m'=-l'}^{l'}\, \left( -1\right) ^{l}\, \rho _{l}^{m}[{\mathbf{R}_{0}}]\, {\cal M}_{l+l'}^{m+m'}\, \rho _{l'}^{m'}[{\mathbf{R}_{0}}]
\end{equation}
where
...
efficient method which allows us to calculate these conditional summations.
In the next section we describe the corrections to the Coulomb matrix which
allow us to obtain Ewald boundary conditions so that we can compare directly
to methods that use ewald summation such as \textbf{Crystal98}. Figure \ref{figure: ErrorPFF}
shows the error of the energy as we increase the Inner box sums for \( L=8,16 \)
for a sodium cloride test system, as can be seen we obtain very rapid convergence.
Inorder to calculate the size needed for the inner box sum, we borrow a formula
from reffernence \cite{Tymczak02}. 

\begin{equation}
\label{InBoxError}
\left| \mathbf{R}-\mathbf{Q}\right| \geq \left\{ \frac{O_{L+1}\left[ \mathbf{Q}\right] \: N(l_{max},L+1)}{\tau }\right\} ^{\frac{1}{l_{max}+L+2}}
\end{equation}
where \( \mathbf{Q} \) is a refference point in the box, \( l_{max} \) is
the maxium angular momentum, and

\begin{equation}
\label{WhatIsO}
O_{l}\left[ \mathbf{Q}\right] =\sqrt{\sum _{m=0}^{l}\left( l-m\right) !\, \left( l+m\right) !\left[ \left( C^{m}_{l}(\mathbf{Q})\right) ^{2}+\left( S^{m}_{l}(\mathbf{Q})\right) ^{2}\right] }
\end{equation}


\begin{equation}
\label{BigN}
N\left[ l_{p},l_{q}\right] =\frac{\sum _{m_{p}m_{q}}\frac{\left( l_{p}+l_{q}-m_{p}-m_{q}\right) !}{\left( l_{p}+m_{p}\right) !\left( l_{q}+m_{q}\right) !}}{\sum _{m_{p}}\frac{\left( l_{p}-m_{p}\right) !}{\left( l_{p}+m_{p}\right) !}\sum _{m_{q}}\frac{\left( l_{q}-m_{q}\right) !}{\left( l_{q}+m_{q}\right) !}}
\end{equation}
...
potential is \cite{Redlack72}, \begin{equation}
\label{EW_pot}
\Phi _{{\rm Ew}}\left( \mathbf{x}\right) =\Phi _{ss}\left( \mathbf{x}\right) -\frac{4\pi }{3}{\left( \mathbf{x}-\mathbf{R}_{0}\right) \cdot \mathbf{D}}+\frac{2\pi }{3}Q
\end{equation}
in terms of the matrix elements of \( J_{ab}^{{\rm PBC}} \) this gives the
corrections\begin{eqnarray}
J_{ab}^{{\rm EC}} & = & \int _{V_{\infty }}\, d{\mathbf{x}}\, \rho ^{{\rm loc}}_{ab}\left( {\mathbf{x}}\right) \left\{ \frac{2\pi }{3}Q-\frac{4\pi }{3}\left( \mathbf{x}-\mathbf{R}_{0}\right) \cdot \mathbf{D}\right\} \begin{array}{c}
\\

\end{array}\label{Jab_ec} \\
 & = & \frac{2\pi }{3}Q\, S_{ab}^{{\rm PBC}}-\frac{4\pi }{3}\mathbf{d}_{ab}\cdot \mathbf{D}\label{Jab_ec_2} 
\end{eqnarray}
where
...
\end{array}\label{Q} 
\end{eqnarray}
are the dipole and quadrapole moments. In appendix B we give a short derivation
of these terms.
...
\begin{eqnarray}
K_{ab}^{{\rm PBC}} & = & \int _{V_{{\rm cell}}}\, d\mathbf{x}\, \rho _{ab}^{{\rm PBC}}\left( \mathbf{x}\right) \: V_{xc}\left( \rho ^{{\rm PBC}}\left( \mathbf{x}\right) \right) \begin{array}{c}
\\

\end{array}\label{Kab_pbc_1} \\
 & = & \int _{V_{{\rm HiCu}}}\, d\mathbf{x}\, \rho _{ab}^{\{PBC}\left( \mathbf{x}\right) \: V_{xc}\left( \rho ^{PBC}\left( \mathbf{x}\right) \right) \begin{array}{c}
\\

\end{array}\label{Kab_pbc_2} \\
 & = & \sum _{\mathbf{RR}'}\int _{V_{{\rm HiCu}}}\, d\mathbf{x}\, \phi _{a}(\mathbf{x}+\mathbf{R})\phi _{b}(\mathbf{x}+\mathbf{R}')\, V_{xc}\left( \sum _{\mathbf{R}''}\, \rho ^{{\rm loc}}\left( \mathbf{x}+\mathbf{R}''\right) \right) \begin{array}{c}
\\

\end{array}\label{Kab_pbc_3} 
\end{eqnarray}
where \( V_{{\rm HiCu}} \) is the cubic region shown in figure \ref{figure: SimCell}.
...
the periodically summed local density \( \rho ^{{\rm loc}}\left( \mathbf{x}\right)  \),
...
Following the work of Nijboer-De Wette \cite{Nijboer57,Nijboer58a}. We start
...
\end{equation}
and \( \beta  \) is a parameter which controls the convergence. This allows
...
\end{array}\nonumber \\
 & + & \frac{4\pi ^{\frac{3}{2}}(\frac{i}{2})^{l}}{V_{cell}\Gamma \left( l+\frac{1}{2}\right) }\sum _{\mathbf{G}}\left| \mathbf{G}\right| ^{l-2}e^{-\frac{\pi ^{2}\left| \mathbf{G}\right| ^{2}}{\beta ^{2}}}\, \left( l-m\right) !\, P_{l}^{m}\left( \cos \left( \theta _{\mathbf{G}}\right) \right) e^{im\phi _{\mathbf{G}}}
\end{eqnarray}
...
\end{equation}
and are tabulated beforehand. We find that this version of the gamma function
...
for different cell shapes. We have also computed the \( {\cal M} \) tensor
in one and two dimensions. The \( {\cal M} \) tensor in one dimension can be
...
{\cal M}^{m}_{l} & = & \sum _{{\mathbf{R}'}\in V_{\infty }-V_{in}}\, M_{l}^{m}[\mathbf{R}]\\
 ...
 & = & \frac{\left( l-m\right) !\, P_{l}^{m}\left( \cos \left( \theta _{0}\right) \right) e^{im\phi _{0}}}{a_{o}^{l+1}}\left\{ \zeta (l+1)-\sum _{n=1}^{n_{0}-1}\frac{1}{n^{l+1}}\right\} \label{1D_M_ten} 
\end{eqnarray}
where \( a_{0} \), \( \theta _{0} \) and \( \phi _{0} \) are the intial box
dimension and angles which are independent of the summation. In two dimension,
the foureier integrals for the calculation of \( {\cal M} \) tensor become
a lot more complicated, therefore we have chosen to use the three dimensional
formula's and obtain the two dimensional moments via a numerical integration,
which is achieved by taking as the nonperiodic direction goes to infinity,
...
{\cal M}^{m}_{l} & = & \sum _{{\mathbf{R}'}\in V_{\infty }-V_{in}}\, \left( l-m\right) !\, P_{l}^{m}\left( \cos \left( \theta _{\mathbf{R}}\right) \right) e^{im\phi _{\mathbf{R}}}\, {\cal G}_{l}\left( \beta ,\left| \mathbf{R}\right| \right) \\
 ...
 &  & \qquad \sum _{\mathbf{G}}\int _{-\infty }^{\infty }\: dG_{z}\: \left| \mathbf{G}\right| ^{l-2}e^{-\frac{\pi ^{2}\left| \mathbf{G}\right| ^{2}}{\beta ^{2}}}\, \left( l-m\right) !\, P_{l}^{m}\left( \cos \left( \theta _{\mathbf{G}}\right) \right) e^{im\phi _{\mathbf{G}}}
\end{eqnarray*}
where \( A_{cell} \) is the area of the cell along the nonperiodic direction.
We have found that we can obtain the \( {\cal M} \) tensor matrix elements
to 8-10 digets of accuracy with this technique.


\section*{Computation of the Dipole and Quadrapole Corrections }

Following the work of Redlack and Grindley \cite{Redlack72} we show the corrections
needed inorder to obtain ewald boundry conditions. These corrections exist in
both two and three dimensions and are due the the field induced in the box from
the surface at infinity, \( S_{\infty } \). For three dimensions, 
...
\end{equation}
where \( {\mathbf{p}} \) and \( Q_{ij} \) are the dipole and quaripole moments
of \( \rho ^{loc}\left( {\mathbf{x}}\right)  \) . Both the dipole and the quadripole
terms contribute to the potential in the cell, the dipole directly and the quaripole
conditionally. Upon summation over the surface, the dipole and quadripole contributions
become\begin{equation}
\label{BN}
\Phi ^{{3D}}_{S_{\infty }}({\mathbf{x}})=-\frac{4\pi }{3V_{{\rm cell}}}{\mathbf{p}}\cdot \left( {\mathbf{x}-\mathbf{R}'}\right) +\frac{2\pi }{3V_{{\rm cell}}}\left( Q_{11}+Q_{22}+Q_{33}\right) .
\end{equation}
For two dimensions only the dipole contributes to the potential within the cell
as

\begin{equation}
\label{BN}
\Phi ^{{\rm 2D}}_{S_{\infty }}(z)=-\frac{2\pi }{A_{{\rm cell}}}p_{z}\, \left( z-R'_{z}\right) 
\end{equation}
Interestingly, for the method that we have chosen to sum our multipole tensor
in two-dimensions, this correction is automatically included.
...
{\par\centering \includegraphics{UnitCell.eps} \par}
\end{figure}
...
{\par\centering \includegraphics{RepCell.eps} \par}
\end{figure}
...
{\par\centering \resizebox*{1.1\textwidth}{!}{\includegraphics{ErrorPFF_InnerBox.eps}} \par}
\end{figure}
