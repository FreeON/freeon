...
\begin{equation}
J_{ab}^{In} = \sum_{{\bf R'} \in In} \int d^3{\bf r} \, d^3{\bf r'}\, \rho_{ab}({\bf r}) | {\bf r}-{\bf r'}+{\bf R'}|^{-1} \rho_{tot}({\bf r'})
\end{equation}
where
\begin{eqnarray}
\rho_{ab}({\bf r})  & = & \sum_{\bf R} \phi_a({\bf r}) \phi_b({\bf r}+{\bf R}) \nonumber\\
\rho_{tot}({\bf r}) & = & \sum_{ab} \sum_{\bf R} P_{ab} \, \phi_a({\bf r}) \phi_b({\bf r}+{\bf R}) -\sum_{a} Z_a \delta({\bf r}-{\bf a})
\nonumber  
\end{eqnarray}
The derivative of $J_{ab}^{In}$ with respect to the cell parameter $M_{ij}$ is
\begin{equation}
{{\partial J_{ab}^{In}} \over {\partial M_{ij}}} = Blah
\end{equation}
\subsubsection{Periodic far field}
The periodic far field correction to the colomb matrix is
\begin{equation}
  J_{ab}^{PFF}={1 \over 2}\sum_{\bf R} \sum_{ml,m'l'} (-1)^l \rho^{ml}_{ab}({\bf R}) \,\, {\cal M}_{ml,m'l'} \,\, \rho^{m'l'}_{tot}
\end{equation}
where
\begin{eqnarray}
\rho^{ml}_{ab}({\bf R}) & = & \int d^3{\bf r} \, \phi_a({\bf r}) \phi_b({\bf r}+{\bf R}) \, O_{ml}({\bf r}) \\
