...
%
  The infinite Coulomb summation is solved with the periodic multipole interaction
...
have also been observed for 3D systems such as MgO and urea.
...
The infinite Coulomb summation is solved with the periodic multipole 
interaction~\cite{Challacombe97,CTymczak04a}.
...
Hartree-Fock exchange matrix and $h_{nuc}$ is the nuclear-nuclear and nuclear-electron energy.
...
%    \frac{\partial E^{x}}{\partial\lambda}+\frac{\partial E^{xc}}{\partial\lambda}\\
    ...
    +\frac{\partial h_{nuc}}{\partial\lambda}\bigg|_P\\
...
the accounts for direct infinite lattice summation. 
...
%%  =\int_{V_\Box}f_{ab}(\mathbf{r})dV,
  ...
  =\int_{V_\Box}\tilde\varepsilon_{xc}[\rho(\mathbf{r})]dV
  =\int_{V_\Box}\tilde\varepsilon_{xc}(\mathbf{r})dV,
\end{equation}
...
%    \sum_{A}(f_{Aj}+n_j)\int_{V_{\Box}}\frac{\partial f(\mathbf{r})}{\partial A_i}dV\\
    &+\delta_{ij}\int_{S_{j\Box}}\tilde\varepsilon_{xc}(\mathbf{r})dS,
  \end{split}
\end{equation}
...
\begin{equation*}%\label{Eq:xcLatGrd}
  \int_{S_{j\Box}}\tilde\varepsilon_{xc}(\mathbf{r})dS=%\approx 
%  \frac{1}{2h}\int_{-h}^h\int_{S_{j\Box}}f_{ab}(\mathbf{r})dSdw_j
  \frac{1}{2h}\int_{V_{j\Box}}\tilde\varepsilon_{xc}(\mathbf{r})dV+\mathcal{O}(h^2)
