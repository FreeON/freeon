...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%In my opinion this is a well and neatly written paper suitable for 
%publication in JCP.
%The implementation of the algorithms for the calculation of DFT energy 
%gradients with respect to atomic positions and cell parameters in 
%MONDOSCF is illustrated properly. Attention is correctly focused on the 
%delicate aspects of the calculation of the derivatives with respect to 
%cell parameters, where handling Coulomb series requires some care and 
%the effects due to variation of the integration volume of the 
%exchange-correlation functional must be taken into account properly. 
%This latter effect probably corresponds to the much larger importance of 
%the integration weight gradients relative to cell parameters than to 
%atomic positions observed in approaches based on an atomic partition of 
%the electron charge density.
%Numerical results obtained with the implemented procedure are well 
%documented and shown to compare well with those obtained with standard 
%periodic codes for few, though representative, testcases.
%A few small suggestions:
%a) check references (for example, ref. 23)
%b) I guess that the factor 2 and lack of the derivative of rhotot in the 
%derivatives of eqs. (2) and (3) are related to the fact that rhoab and 
%rhotot span the same set of CGTFs. Perhaps a short comment about this 
%point might be helpful for a reader who is not very familiar with the 
%method like me.
%c) specification of the symmetry of the crystalline systems used in the 
%numerical examples (cubic and orthorhombic?) would probably be useful to 
%indicate more clearly which cell parameters need to be optimized and how 
%complex the structures are.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}
  ...
  is known to provide atomic and cell parameters in very good agreement
  ...
  within the $\Gamma$-point approximation
  has been introduced [Submitted to J. Chem. Phys. (2006)]. %[J. Chem. Phys. {\bf ???}, ????, (2006)].
...
%
  The infinite Coulomb summation is solved with an effective periodic summation of multipole tensors
  [Challacombe, White and HeadGordon, J. Chem. Phys. {\bf 107}, 9708, (1997)].
  While the evaluation of Coulomb and exchange-correlation gradients with 
  respect to atomic positions are similar to those in the gas phase limit,
  the gradients with respect to cell parameters needs to be treated with some care.
  The derivative of the periodic multipole interaction tensor needs to be carefully handled in
  both direct and reciprocal space and the exchange-correlation energy derivative 
  leads to a surface term that has its origin in derivatives of the integration 
  limits that depend on the cell.
%
  ...
  systems at the DFT and hybrid-HF/DFT levels. We also report
  the full relaxation of forsterite supercells at the B3LYP level of theory.
...
\footnotetext[1]{Preprint LA-UR-06-1746.}
...
%advances provide a framework for the application of hybrid-HF/DFT
...
Convergence of the $\Gamma$-point approximation to
...
theories, as well as for the Hartree-Fock 
atomic and cell gradients~\cite{VWeber05b}.
For one dimensional fluoric acid chains (HF)$_n$, convergence of the 
...
have also been observed for 3D systems such as MgO and urea~\cite{CTymczak04a,CTymczak04b,VWeber05b}.
In general, the $\Gamma$-point approach enables study of very large
...
The analytical cell gradient method of Density Functional Theory using Gaussian Type Atomic Orbital (GTAO) for
...
Recently Doll, Dovesi and Orlando~\cite{KDoll04} presented an
implementation of Hartree-Fock cell gradients~\cite{RDovesi00} in
...
periodic systems, in the frame of the {\sc Crystal03} package has been
...
This paper describes the complete implementation of the atomic and lattice gradients
within the Kohn-Sham approach to Density Functional Theory at the $\Gamma$-point.
The infinite Coulomb summation is solved with an effective periodic 
summation of multipole tensors~\cite{Challacombe97,CTymczak04a}.
We show that, like for the exact HF exchange, the evaluation 
of the atomic gradients within the $\Gamma$-point approximation 
does not lead to special difficulties. The implementation requires mainly 
the evaluation of the derivative of the basis function with respect 
to atomic positions.
However, difficulties arise when derivative of the total energy
is taken with respect to lattice parameters. For the periodic far field Coulomb term,
the derivative of the multipole interaction tensor needs to be carefully handled in 
both direct and reciprocal space. 
The exchange-correlation energy derivative leads to a surface term, which 
has its origin in derivatives of the integration limits that depend on the cell.
We finally demonstrate the convergence of geometrical parameters and total 
energy for a few periodic systems within the DFT and hybrid-HF/DFT $\Gamma$-point
super cell approach. % to the converged large cell $\Gamma$-point approximation.
The full relaxation of the geometrical parameters were done with the QUICCA
algorithm~\cite{KNemeth04,KNemeth05}.
...
Hartree-Fock exchange matrix and $h_{\rm nuc}$ is the nuclear-nuclear and nuclear-electron energy.
The factors $\alpha_1$ and $\alpha_2$ are mixing coefficients of the
exact Hartree-Fock exchange and exchange-correlation energies respectively.
...
%    \frac{\partial E^{x}}{\partial\lambda}+\frac{\partial E^{xc}}{\partial\lambda}\\
    ...
    +\frac{\partial h_{\rm nuc}}{\partial\lambda}\bigg|_P\\
...
%    -\frac{1}{2}(ac^\mathbf{m}|bd^\mathbf{n}) )+E_{nuc}
  \end{split}
\end{equation*}
where $|_P$ means that $P$ is held constant, $W=PFP$ is the energy-weighted density matrix and $F$ the Fockian or
...
(for more detail about the formalism see Refs.~\cite{Challacombe97,CTymczak04a})
\begin{equation*}
...
%\rho_{tot}({\bf r}) & = & \sum_{ab} P_{ab} \rho_{ab}({\bf r}) -\sum_{A} Z_A \delta({\bf r}-{\bf A}),
%\end{eqnarray*}
the index $A$ runs over the atoms in the cell, $Z_A$ is the charge of atom $A$ and $\delta({\bf r})$ is the Dirac delta function.
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where the factors of two arise from the bra/ket symmetry, 
the derivatives $\partial\rho_{ab}/\partial G_i$,
...
Note that similar terms arise for $h_{\rm nuc}$ and are treated in an equivalent way as $J_{ab}^{\rm In}$. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Coulomb matrix~\cite{Challacombe97,CTymczak04a}, which is 
\begin{equation}\label{Eq:JPFF}
J_{ab}^{\rm PFF}=\sum_{lml'm'} (-1)^l \rho^{lm}_{ab} {\cal M}_{l+l'}^{m+m'}  \rho^{l'm'}_{tot},
\end{equation}
where ${\cal M}_{l}^{m}$ is an effective multipole interaction tensor 
that accounts for the direct infinite lattice summation. 
The spherical densities $\rho^{lm}_{ab}$ and $\rho^{lm}_{tot}$ are obtained
...
$\partial {\cal M}_l^m/\partial M_{ij}$ %is straightforward but tedious and 
is shown in the Appendix~\ref{Apx:MDer}.% for completeness.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Tin-Foil}
\noindent The tin-foil correction to the Coulomb matrix is
\begin{equation}\label{Eq:JTF}
J_{ab}^{\rm TF}=\frac{2\pi}{3V_{\rm uc}}(QS_{ab}-2\mathbf{d}_{ab}\cdot\mathbf{D}),
\end{equation}
where $V_{\rm uc}$ is the unit cell volume, 
$Q$ is the trace of the system quadrupole, 
$S_{ab}$ is an element of the overlap matrix,
$\mathbf{D}$ the dipole moment of the system
and $\mathbf{d}_{ab}$ the dipole moment of the distribution $\rho_{ab}$.
...
\begin{equation*}
  \begin{split}
    ...
    \frac{2\pi}{3V_{\rm uc}}\bigg(\frac{\partial Q}{\partial G_i}\bigg|_P S_{ab}+Q\frac{\partial S_{ab}}{\partial G_i}\\
...
\begin{equation*}
  \begin{split}
    \frac{\partial J_{ab}^{\rm TF}}{\partial M_{ij}}\bigg|_P&=\frac{2\pi}{3V_{\rm uc}}
    \bigg(
    ...
    &-\frac{1}{V_{\rm uc}}\frac{\partial V_{\rm uc}}{\partial M_{ij}}(QS_{ab}-2\mathbf{d}_{ab}\cdot\mathbf{D})
    \bigg),
%    -\frac{2\pi}{3V_{\rm uc}^2}\frac{\partial V_{\rm uc}}{\partial M_{ij}}(QS_{ab}-2\mathbf{d}_{ab}\cdot\mathbf{D})\\
%    &+\frac{2\pi}{3V_{\rm uc}}\bigg(\frac{\partial Q}{\partial M_{ij}}\bigg|_P S_{ab}+Q\frac{\partial S_{ab}}{\partial M_{ij}}\\
%    &-2\frac{\partial \mathbf{d}_{ab}}{\partial M_{ij}}\cdot\mathbf{D}
...
the terms ${\partial S_{ab}}/{\partial M_{ij}}$ and ${\partial V_{\rm uc}}/{\partial M_{ij}}$ 
...
equivalent to the integration over the unit cell $V_{\rm uc}$.
...
%cj  E^{xc}%=\int_{V_\Box}\rho(\mathbf{r})\varepsilon_{xc}(\rho)dV
%cj  =\int_{V_\Box}\tilde\varepsilon_{xc}[\rho(\mathbf{r})]dV
%cj  =\int_{V_\Box}\tilde\varepsilon_{xc}(\mathbf{r})dV,
  E^{xc}%=\int_{V_\Box}\rho(\mathbf{r})\varepsilon_{xc}(\rho)dV
  =\int_{V_{\rm uc}}f(\mathbf{r})dV
  \equiv\int_{V_\Box}f(\mathbf{r})dV,
\end{equation}
where $\rho(\mathbf{r})=\sum_{ab{\bf R}}P_{ab}\rho_{ab}({\bf r+R})$ is the periodic 
electronic density.
...
the ``Becke weights''~\cite{ABecke88}, which requires integration over all space, $V_\infty$.
...
\begin{equation*}
  ...
  \int_{V_{\Box}}\frac{\partial\rho(\mathbf{r})}{\partial G_i}\bigg|_Pv_{xc}[\rho({\bf r})]dV,
\end{equation*}
and
...
%    &+\delta_{ij}\int_{S_{j}}\rho_{ab}(\mathbf{r})v_{xc}[\rho,\mathbf{r}]dS
    \int_{V_{\Box}}\frac{\partial\rho(\mathbf{r})}{\partial M_{ij}}\bigg|_Pv_{xc}[\rho({\bf r})]dV\\
%    \sum_{A}(f_{Aj}+n_j)\int_{V_{\Box}}\frac{\partial f(\mathbf{r})}{\partial A_i}dV\\
    &+\delta_{ij}\int_{S_{j\Box}}f(\mathbf{r})dS,
  \end{split}
\end{equation}
where $S_{j\Box}$ is the surface area of the cuboid cell with 
...
The derivative $\partial\rho(\mathbf{r})/\partial M_{ij}|_P$ is simple and does not need to be addressed in more detail.
%%The derivative $\partial\rho(\mathbf{r})/\partial M_{ij}|_P$ can be further simplified as
%%\begin{equation*}
%%\frac{\partial\rho(\mathbf{r})}{\partial M_{ij}}\bigg|_P=\sum_{A,\mathbf{n}}
%%     (f_{Aj}+n_j)\frac{\partial\rho(\mathbf{r})}{\partial A_{i}}\bigg|_P,
%%\end{equation*}
%%where $\partial\rho(\mathbf{r})/\partial A_{i}|_P$ is obtained during the evaluation of the atomic gradients.
...
All developments were implemented in the {\sc MondoSCF}~\cite{MondoSCF_1.0-alpha-11} suite of
...
Kohn-Sham Density Functional analytic atomic and cell gradients, we present in this Section
full optimization studies of 1D and 3D periodic systems without any cell
or atomic positions symmetry constraints. The optimizations were carried out with
the QUICCA algorithm~\cite{KNemeth04,KNemeth05}.
...
The second benchmark is cubic MgO and was optimized at the Perdew-Burke-Ernzerhof (PBE)~\cite{Perdew_96v77} 
and Becke 3-parameter Lee-Yang-Parr (B3LYP)~\cite{ABecke93} 
level of theory. Finally, orthorhombic magnesium orthosilicate (Mg$_2$SiO$_4$, forsterite) 
...
The reason for this disagreement can be found in the different basis set used.
{\sc Crystal03} uses spherical harmonics d-shell consisting of 5 atomic orbitals
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    Lengths, angles and energies are in Angstroms, degrees and atomic units respectively.
  }\label{Tab:PFE}
...
for various MgO supercells (cubic symmetry) at the $\Gamma$-point PBE~\cite{Perdew_96v77}
...
In Table~\ref{Tab:Forsterite}, we present the optimization of forsterite (orthorhombic symmetry) at the 
...
obtained by Jochym, Parlinski and Krzywiec~\cite{PJochym04} at the PW91 level of theory and
a $3\times 3\times 3$ $\mathbf{k}$-points integration grid
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    B3LYP/8-61G*(Mg)/88-31G*(Si)/8-51G*(O) level of theory and the {\tt TIGHT} thresholds.
...
% DZVP TIGHT
  ...
  \hline
    {\sc Vasp}\footnote[2]{$3\times 3\times 3$ $\mathbf{k}$-points and PW91~\cite{PJochym04}.}
    &  4 & 4.800 & 10.306 & 6.041 & \\
  \hline
...
%We finally demonstrate the convergence of geometrical parameters and total
%energy for few periodic systems within the DFT and hybrid-HF/DFT $\Gamma$-point
%super cell approach to the converged large cell $\Gamma$-point approximation.
...
%\end{equation*}
Following the work of Challacombe, White and HeadGordon~\cite{Challacombe97} 
and Tymczak and Challacombe~\cite{CTymczak04a}, the ${\cal M}_l^m$ tensor can be written into 
...
\begin{equation*}\label{Eq:MTensor}
  ...
  &+\frac{4\pi ^{\frac{3}{2}}(i/2)^{l}}{V_{\rm uc}\Gamma(l+1/2)}
...
$\widetilde{P}_{l}^{m}$ is a scaled associated Legendre polynomial, 
$\beta=\sqrt{\pi }/V_{\rm uc}^{\frac{1}{3}}$ for convenience,
\begin{equation*}
...
\begin{equation*}\label{Eq:DerMTensor}
  \begin{split}
   ...
   e^{im\phi_{\mathbf{R}}}
   \bigg(
   \widetilde{P}_{l}^{m}\frac{\partial{\cal G}_{l}}{\partial M_{ij}}\bigg|_\beta\\
   &+\bigg(
   \frac{\partial\widetilde{P}_{l}^{m}}{\partial M_{ij}}
   +im\widetilde{P}_{l}^{m}\frac{\partial\phi_{\mathbf{R}}}{\partial M_{ij}}
   \bigg){\cal G}_{l}
   \bigg)\\
   &-\sum_{\mathbf{R}\in {\rm In}}
   e^{im\phi_{\mathbf{R}}}
   \bigg(
   \widetilde{P}_{l}^{m}\frac{\partial{\cal F}_{l}}{\partial M_{ij}}\bigg|_\beta\\
   &+\bigg(
   \frac{\partial\widetilde{P}_{l}^{m}}{\partial M_{ij}}
   +im\widetilde{P}_{l}^{m}\frac{\partial\phi_{\mathbf{R}}}{\partial M_{ij}}
   \bigg){\cal F}_{l}
   ...
   &\times\bigg(
   \frac{-1}{V_{\rm uc}}\frac{\partial V_{\rm uc}}{\partial M_{ij}}
   G\widetilde{P}_{l}^{m}\\
   &+\bigg((l-2)
   -2\left(\frac{\pi G}{\beta}\right)^2
   \bigg)\frac{\partial G}{\partial M_{ij}}\widetilde{P}_{l}^{m}\\
   &+G\bigg(
   \frac{\partial\widetilde{P}_{l}^{m}}{\partial M_{ij}}
   +im\widetilde{P}_{l}^{m}\frac{\partial\phi_{\mathbf{G}}}{\partial M_{ij}}
   \bigg)
   \bigg).
%%%OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD 
%   &-\frac{4\pi^{\frac{3}{2}}(\frac{i}{2})^{l}}{V_{\rm uc}^2\Gamma\left(l+\frac{1}{2}\right)}
%   \frac{\partial V_{\rm uc}}{\partial M_{ij}}\\
...
%   \widetilde{P}_{l}^{m}\frac{\partial\phi_{\mathbf{G}}}{\partial M_{ij}}e^{im\phi_{\mathbf{G}}},
  \end{split}
\end{equation*}
Note that because ${\cal M}_l^m\ne{\cal M}_l^m(\beta)$, it follows that $\partial {\cal M}_l^m/\partial\beta=0$. 
The derivative of the scaled associated Legendre polynomial $\widetilde{P}_{l}^{m}(\cos\theta)$ is
\begin{equation*}
...
\begin{equation*}
%  \begin{split}
    \frac{\partial{\cal G}_{l}}{\partial M_{ij}}\bigg|_\beta=
    \bigg(\frac{ 2(\beta R)^{2l}e^{-\beta^2R^2} }{ \Gamma(l+1/2)R^{l+1}}
    -{\cal G}_{l}(\beta,R)\frac{(l+1)}{R}
    \bigg)\frac{\partial R}{\partial M_{ij}},
...
\begin{equation*}
%  \begin{split}
    \frac{\partial{\cal F}_{l}}{\partial M_{ij}}\bigg|_\beta=
    -\bigg(\frac{2(\beta R)^{2l}e^{-\beta^2R^2}}{\Gamma(l+1/2)R^{l+1}}
    +{\cal F}_{l}(\beta,R)\frac{(l+1)}{R}\bigg)\frac{\partial R}{\partial M_{ij}}.
...
\end{equation*}
%\begin{equation*}
%  \frac{\partial\beta}{\partial M_{ij}}=-\frac{1}{3}\sqrt{\pi}V_{\rm uc}^{-2/3}
%  \frac{\partial V_{\rm uc}}{\partial M_{ij}}
%\end{equation*}
The derivatives $\partial V_{\rm uc}/\partial M_{ij}$ and $\partial G/\partial M_{ij}$ can be
found in Ref.~\cite{KDoll04} and the remaining derivatives {\em i.e.}
$\partial R/\partial M_{ij}$,
$\partial\theta_{\mathbf{R}}/\partial M_{ij}$, 
$\partial\theta_{\mathbf{G}}/\partial M_{ij}$
$\partial\phi_{\mathbf{R}}/\partial M_{ij}$ and
$\partial\phi_{\mathbf{G}}/\partial M_{ij}$ are easily computed.
