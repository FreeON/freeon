...
Hartree-Fock exchange matrix and $h_{nuc}$ is the nuclear-nuclear electron-nuclear energy.
...
%    -\frac{1}{2}(ac^\mathbf{m}|bd^\mathbf{n}) )+E_{nuc}
  \end{split}
\end{equation*}
where $|_P$ stands for $P$ constant, $W=PFP$ is the energy-weighted density matrix and $F$ the Fockian.
...
$\partial\rho_{ab}/\partial M_{ij}$ are straitforward and 
...
\begin{eqnarray*}
\rho^{lm}_{ab}  & = & \int \rho_{ab}( {\bf r}) O_l^m({\bf r})dV \textrm{ and}\\
...
$\partial {\cal M}_l^m/\partial M_{ij}$ is straitforward but tedious
...
%cell volume $V_{uc}$. 
In Ref.~\cite{CTymczak04a} we showed that the exchange-correlation energy, 
for a periodic system, can be computed over a cubic region $V_{\Box}$, and is completely
equivalant to the integration over the unit cell $V_{uc}$.
