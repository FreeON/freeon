...
The derivative of the total energy Eq.~\ref{Eq:TotE} with respect to an external perturbation $\lambda$ is 
\begin{equation*}
  \begin{split}
...
%    -\frac{1}{2}(ac^\mathbf{m}|bd^\mathbf{n}) )+E_{nuc}
  \end{split}
\end{equation*}
where $W=PFP$ is the energy-weighted density matrix and $F$ is the Fockian.
...
{\it i.e.} $\partial K_{ab}/\partial\lambda |_P$, have been previously derived~\cite{VWeber05b}.
...
\end{equation}
where $J_{ab}^{In}$ is the inner cell set sum done directly, $J_{ab}^{PFF}$ is the 
periodic far field term and $J_{ab}^{TF}$ is a surface term which corrects 
the boundary at infinity.
\subsubsection{Inner Sum}
\noindent The inner cell set contribution to the Coulomb matrix is 
\begin{equation}
J_{ab}^{In} = \sum_{{\bf R'} \in In} \iint \rho_{ab}({\bf r}) | 
...
The derivative of $J_{ab}^{In}$, at $P$ constant, with respect to the cell parameter $M_{ij}$ is
\begin{eqnarray}
{{\partial J_{ab}^{In}} \over {\partial M_{ij}}}\bigg|_P &=& \sum_{{\bf R'} \in In} 2\iint  
...
\noindent We use a spherical multipole method inorder to compute the periodic far field correction to the 
Coulomb matrix, which is  \\ {\bf ??? CJ: does $O_{ml}({\bf r})$ come from the moon??? }
\begin{equation}
J_{ab}^{PFF}=\sum_{mlm'l'} (-1)^l \rho^{ml}_{ab} {\cal M}_{mlm'l'}  \rho^{m'l'}_{tot}
\end{equation}
where the spherical densities $\rho^{ml}_{ab}$ and $\rho^{ml}_{tot}$ are obtained
by projecting  $\rho_{ab}({\bf r})$ and $\rho_{tot}({\bf r})$, respectively, onto the spherical 
harmonics $O_{ml}({\bf r})$ as
\begin{eqnarray*}
\rho^{ml}_{ab}  & = & \int \rho_{ab}( {\bf r}) O_{ml}({\bf r})dV \textrm{ and}\\
\rho^{ml}_{tot} & = & \int \rho_{tot}({\bf r}) O_{ml}({\bf r})dV. 
\end{eqnarray*}
The ${\cal M}$ tensor is summed using a Ewald like partition [{\bf ??CJ which ref??}]
\begin{equation*}
{\cal M}_{mlm'l'} = \sum_{\bf R} M_{mlm'l'}({\bf R}).
\end{equation*}
The derivative of $J_{ab}^{PFF}$, at $P$ constant, with respect to the cell parameter $M_{ij}$ is
\begin{eqnarray}
{{\partial J_{ab}^{PFF}} \over {\partial M_{ij}}}\bigg|_P &=& \sum_{mlm'l'}
2 (-1)^l {{\partial \rho^{ml}_{ab}} \over {\partial M_{ij}}} {\cal M}_{mlm'l'} \rho^{m'l'}_{tot} \nonumber\\
&+& (-1)^l \rho^{ml}_{ab} {{\partial {\cal M}_{mlm'l'}}\over {\partial M_{ij}} } \rho^{m'l'}_{tot}
\end{eqnarray}
where the calculation of the derivatives of the ${\cal M}$ tensor is shown in Appendix A.
...
\begin{equation}
  \begin{split}
    \frac{\partial J_{ab}^{TF}}{\partial M_{ij}}\bigg|_P&=
    -\frac{2\pi}{3V^2}\frac{\partial V}{\partial M_{ij}}(QS_{ab}-2\mathbf{d}_{ab}\cdot\mathbf{D})\\
    &+\frac{2\pi}{3V}\bigg(\frac{\partial Q}{\partial M_{ij}}\bigg|_P S_{ab}+Q\frac{\partial S_{ab}}{\partial M_{ij}}\\
...
is carried out along the normal to the surface $S_{j\Box}$. This simple domain, {\it i.e.} $V_{j\Box}$, 
...
The larger systems  i.e. the $2\times 1\times 2$ and $3\times 1\times 2$ supercells
...
deviate no more that ...\% with respect to the experimental values.
