...
\end{equation*}
where $J_{ab}^{In}$ is the inner cell set sum evaluated by the 
Quantum Chemical Tree Code (QCTC)~\cite{MChallacombe97}, $J_{ab}^{PFF}$ is the 
...
&+ (-1)^l \rho^{ml}_{ab} {{\partial {\cal M}_{mlm'l'}}\over {\partial M_{ij}} } \rho^{m'l'}_{tot}
  \end{split}
\end{equation*}
where the calculation of $\partial {\cal M}/\partial M_{ij}$ is shown in the Appendix.%~\ref{Apx:MDer}.
...
cell volume $V$. The exchange-correlation energy is thus
...
electronic density associated with $V$.
...
\begin{eqnarray}
  ...
  \right)
  \nonumber\\
  &+&\frac{4\pi ^{\frac{3}{2}}(\frac{i}{2})^{l}}{V\Gamma \left( l+\frac{1}{2}\right) }
  \nonumber\\
  ...
  \label{C4}
\end{eqnarray}
where $\mathbf{G}$ is a reciprocal lattice vector, $\widetilde{P}_{l}^{m}$ 
is the associated Legendre polynomial, $\beta=\sqrt{\pi }/V^{\frac{1}{3}}$,
\begin{equation}
...
\begin{equation}
  ...
  {\Gamma \left( l+\frac{1}{2}\right) \, r^{l+1}} \, ,
\end{equation}
where $\Gamma(x)$ is the gamma function, $\gamma(a,x)$ is the incomplete gamma function~\cite{MAbramowitz87}.
