...
the unit cell volume (see for example Refs.~\cite{CKittel71,NAshcroft76}).
...
matrix of an insulator decays exponentially with interatomic separations.
...
in Figure~\ref{Fig:CnvgEnergy}. The behavior of the convergence of the total energy
without the MIC is expected to be inversely proportional to the volume of the 
unit-cell for 3D-periodic systems.\\
\\
\\
{\bf ??? SHALL WE HAVE AN EXTRA FIG WITH A FIT 1/L ????}\\
\\
...
have also been observed for the 3D-systems (MgO)$_n$ and (urea)$_n$. We belive
...
the system size. For 3D-periodic system, we belive that the convergence 
of the total energy without the MIC behaves inversely proportional to the volume of the
unit-cell.
