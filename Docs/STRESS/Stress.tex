...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This manuscript reports about an implementation of analytical gradients
%with respect to atomic positions and cell parameters, for periodic
%systems, at the Hartree-Fock level. The implementation is based on the
%usage of a single k-point, and a large cell is used to converge to the
%k-space integration limit.  Examples for geometry optimizations are given,
%and the convergence with system size is discussed. 
%
%The manuscript is concise and clearly written, and the topic is certainly
%interesting. However, I think the following points should be addressed: 
%
%1)  The authors have stated that they obtain an accuracy of 4 digits in
%the forces. In my opinion, it is crucial to document this accuracy, by
%providing a table where numerical and analytical gradients are compared.
%For example, one could choose geometries which are not the equilibrium
%geometry of HF, urea and MgO, and then compute the numerical and the
%analytical gradient with respect to atomic positions and with respect to
%the cell parameters. 
%
%2)  In Table IV and the corresponding part of the text, it is unclear what
%is meant with 'unsuitable use of the exact exchange Gamma-point
%approximation'.  It is not clear why the gradients do not vanish with n=4. 
%Even if they don't vanish, the gradients with respect to atomic positions
%should sum to zero. This needs more discussion. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
...
%-Add K. Doll in the Ref for Urea K. Doll Computer Physics Communications 137 (2001) 74-88\\
%-Add KDoll01a in mondo.bib\\
...
The {\tt TIGHT} and {\tt VERYTIGHT} level of numerical accuracies have been used throughout this work.  
Thresholds that define the {\tt TIGHT} ({\tt VERYTIGHT}) accuracy level include a matrix 
threshold $\tau=10^{-6}$ ($10^{-7}$), as well as other numerical thresholds 
detailed in Ref.~\cite{CTymczak04a}, which deliver at least 8 (10) digits of 
relative accuracy in the total energy and 4 (5) digits of absolute accuracy 
in the forces.  
...
%Force accuracy
In order to show the accuracy of the Hartree-Fock $\Gamma$-point gradients, we present
in Table~\ref{Tab:GradAccu} the analytical 
atomic and cell gradients for a linear 
(HF)$_6$ chain and a 3D (MgO)$_{32}$ with different thresholds 
at the RHF-MIC/6-31G and RHF-MIC/8-61G(Mg)/8-51G(O) level of theories, respectively. 
These gradients were generated by a slight deformation of the
equilibrium geometries. For HF the bond length and cell parameter were 
$d=0.9$\AA~and $a_0=2.4$\AA, respectively. For MgO, the cell parameter 
was $a_0=4.1$\AA~and a Mg atom was moved by $0.2$\AA~parallel to the $x$-direction.
Comparisons are made to numerical atomic and cell gradients obtained 
with the {\tt VERYTIGHT} threshold, 21 energy points
and a 6-th degree polynomial fit.
...
\begin{table}[t]
  \centering
  \caption{\protect
    Analytical Hartree-Fock atomic and 
    cell gradients for (HF)$_6$ and (MgO)$_{32}$
    with different thresholds at the RHF-MIC/6-31G and RHF-MIC/8-61G(Mg)/8-51G(O) 
    level of theories, respectively.
    Comparisons are made to numerical atomic and cell gradients obtained 
    with the {\tt VERYTIGHT} threshold.
    Gradients are in atomic units.
  }\label{Tab:GradAccu}
  \begin{tabular}{lcccc}
  \toprule
  &\multicolumn{2}{c}{(HF)$_6$}&\multicolumn{2}{c}{(MgO)$_{32}$}\\
  \cline{2-5}
  Threshold 
  & Atomic\footnote[1]{Gradient on the F atom along the periodic axis.}
  & Cell\footnote[2]{Component $\partial E/\partial a_x$.}
  & Atomic\footnote[3]{Gradient on the displaced Mg atom along the $x$-axis.}
  & Cell$^b$\\
  \hline
%  {\tt LOOSE}     & $-$0.047490 & $-$0.018567 & 0.091945 & $-$0.081510 \\
%  {\tt GOOD}      & $-$0.047787 & $-$0.023911 & 0.092476 & $-$0.129455 \\
  {\tt TIGHT}     & $-$0.047916 & $-$0.024155 & 0.092434 & $-$0.128109 \\
  {\tt VERYTIGHT} & $-$0.047941 & $-$0.024230 & 0.092436 & $-$0.128074 \\
  \hline		  
  Numerical       & $-$0.047941 & $-$0.024230 & 0.092435 & $-$0.128089 \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%hf          atomic          cell
%NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW 
%loose     0.0474905040  0.01 856730382
%good      0.0477867200  0.023 91065693
%tight     0.0479155941  0.0241 5513950
%verytight 0.0479412001  0.02423013890
%numeric   0.0479407686  0.02423009859
%OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD OLD 
%loose     0.0525413161  0.0 3339414863
%good      0.0479597270  0.024 49527375
%tight     0.0479479526  0.02422 863066
%verytight 0.0479415176  0.02423005344
%numeric   0.0479407686  0.02423009859
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \botrule
  \end{tabular}
...
%    $n$ is the number of HF units in the (super)cell.
    Bond lengths and cell parameters are in Angstrom and energies in atomic units.
  }\label{Tab:HF-n}
  \begin{tabular}{lrllc}
    \toprule
    & $n$ & $\;\;\;a_0$ & $\quad\; d$ & $E/n$ \\
...
The logarithm absolute error in the atomic and cell gradients
are also shown in Figure~\ref{Fig:CnvgEnergy}.
These gradients were generated by a slight contraction (0.01\AA)~of the bond length.
The calculations with the MIC show an exponential convergence of the total
energy and gradients. However, a naive implementation of 
...
% C. Gatti, V.R. Saunders and C. Roetti, Crystal field effects on the topological properties of 
% the electron density in molecular crystals. The case of urea, 
...
%    $n$ is the number of urea units in the (super)cell.
    Lengths, angles and energies are in Angstrom, degrees and atomic units respectively.
  }\label{Tab:Urea}
  \begin{tabular}{lccc}
% 2x2x2
...
%     1    -3581.6544536206
% 3x3x3
%          STRE_A         16.909420     -0.000209      0.000000     16.909420
%          STRE_B         16.909420     -0.000204      0.000000     16.909420
%          STRE_C         13.946308      0.000341      0.000000     13.946308
%          ALPHA          90.000000     -0.000745      0.000000     90.000000
%          BETA           90.000000      0.001057      0.000000     90.000000
%          GAMMA          90.000000     -0.002755      0.000000     90.000000
%          VOLM_L       3987.646695      0.000000      0.000000   3987.646695
%     1   -12088.1089123901
% Crystal03 1x1x1
% -2.23841752471450E+02 %Old%
%%%%New values from Bartolomeo
%  HF          & Mondo SCF& CRY/6-21G** & CRY/6-21G**(H exp. rescaled) & Old
%  a_0         &       5.630 &        5.656 &       5.652 &       5.524
%  c_0         &       4.660 &        4.667 &       4.678 &       4.642
%  E/n         & -223.853403 & -223.8379155 & -223.842647 & -223.841752
% Bond lengths						                 
%  CO          &       1.246 &        1.244 &       1.243 &       1.245
%  CN          &       1.334 &        1.333 &       1.333 &       1.330
%  NH_1        &       0.999 &        1.000 &       0.997 &       0.997
%  NH_2        &       0.998 &        1.000 &       0.996 &       0.995
%  Bond angles						                
%  OCN         &       121.7 &        121.7 &       121.7 &       121.5
%  CNH_1       &       119.5 &        119.4 &       119.3 &       119.2
%  CNH_2       &       120.6 &        120.5 &       120.6 &       120.6
%  Dihedral angles					                 
%  OCNH_1      &         0.0 &          0.0 &         0.0 &         0.0
%  OCNH_2      &       180.0 &        180.0 &       180.0 &       180.0
%  NCNH_1      &       180.0 &        180.0 &       180.0 &       180.0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \toprule
    ...
    NH$_1$       &         0.999 &         0.998 &         0.997 \\%cis-O
    NH$_2$       &         0.998 &         0.998 &         0.996 \\%trans-O
    &\multicolumn{3}{c}{Bond angles}     \\
    OCN          &         121.7 &         121.7 &         121.7 \\
    CNH$_1$      &         119.5 &         119.6 &         119.3 \\%cis-O
    CNH$_2$      &         120.6 &         120.5 &         120.6 \\%trans-O
    &\multicolumn{3}{c}{Dihedral angles} \\
    OCNH$_1$     &           0.0 &           0.0 &           0.0 \\%cis-O
    OCNH$_2$     &         180.0 &         180.0 &         180.0 \\%trans-O
...
%    $n$ is the number of MgO units in the (super)cell.
    cell parameters and energies are in Angstrom and atomic unit respectively.
  }\label{Tab:MgO}
  \begin{tabular}{lrccc}
  \toprule
  & $n$ & $a_0$ & $E/n$ & $f_O$ \\
...
In Table~\ref{Tab:GradMgO}, we present the progression 
of the average of the norm of the
atomic gradients for the (MgO)$_n$ systems with a cell parameter of $a_0=4.2$\AA. 
Note that the gradients with respect to atomic positions sum to zero within the accuracy requested.
While the smallest system, (MgO)$_4$, gives 
large unphysical gradients which reflects the use of a too small cell for a $\Gamma$-point calculation,
the larger systems provide gradients which are below the accuracy targeted by the 
{\tt TIGHT} thresholds ({\em i.e.} an absolute error 
of $10^{-4}$ a.u. in the forces).
%In Table~\ref{Tab:GradMgO}, we present the progression 
%of the average of the norm of the
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    on the Mg and O atoms for (MgO)$_n$ using the periodic 
    RHF-MIC/8-61G/8-51G level of theory and the {\tt TIGHT} thresholds. 
    Gradients are in atomic units.
  }\label{Tab:GradMgO}
  \begin{tabular}{rcc}
...
\end{center}
   ...
   FIG. 1: Convergence of the $\Gamma$-point relative energy, with and without the MIC,
    for the (HF)$_n$ chains with increasing cell length. The cell parameter
    $a_0=2.4745$\AA~and bond length $d=0.9453$\AA~were held fixed.
    Also shown is the absolute convergence of the atomic and cell forces for a slightly contracted
    bond length (see text for more detail).
    Calculations were performed at the RHF-MIC/6-31G level of theory with
    the {\tt TIGHT} thresholds.\\
   FIG. 2: Convergence of the $\Gamma$-point relative energy without the MIC
    for the (HF)$_n$ chains ($n$ ranges from 1 to 31) with respect to cell length.
    The cell parameter $a_0=2.4745$\AA~and bond length $d=0.9453$\AA~were held fixed.
    Calculations were performed at the RHF-MIC/6-31G level of theory and the {\tt TIGHT} thresholds.\\
