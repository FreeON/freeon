...
\keywords{Periodic boundary condition, exact Hartree-Fock exchange, lattice gradients.}
                              %display desired
\maketitle
...
\section{Hartree-Fock exchange}\label{Sec:Formalism}
...
\end{equation}
The position of a direct lattice is defined as $\mathbf{R}=M\mathbf{n}$,
with $\mathbf{n}=(n_a,n_b,n_c)$ being integer numbers.
The position of an atom $A$ in the cell $\mathbf{n}$ is $\mathbf{A}=M(\mathbf{f}_A+\mathbf{n})$,
and $\mathbf{f}_A=(f_{Aa},f_{Ab},f_{Ac})$ are the fractional coordinates of 
the atom $A$ in the working cell.

The unnormalized Cartesian Gaussian-type function (CGTF) centered on the atom $A$ reads
\begin{equation}
...
is given 
\begin{equation}
  K_{ab}= -\frac{1}{2}\sum_{\substack{\mathbf{m}\mathbf{n}\\c d}}
...
The exchange energy gradients acting on atom $G$ and direction $i$ 
...
Bravais vectors respectively and $E^x=\Tr(PK(P))$ is the Hartree-Fock exchange energy.
If we assume for example that $\partial PQ_k/\partial A_i=\delta_{ik}\zeta_a/(\zeta_a+\zeta_c)$
is valid everywhere, then it is possible to
...
incomplete gamma function by mean of a modified version of the OS VRR. Finally,
...
$F_m(x)$ and intermediate integrals, it is 
...
relative accuracy in the total energy.  

In order to demonstrate the capabilities of our implementation of the
Hartree-Fock exchange energy gradients and cell gradients, we present in this Section 
full optimization studies of 1D and 3D periodic systems without any symmetry constrain. 
...
These two latter systems have been recently optimize at the Hartree-Fock 
...
total energies $E$ computed for linear HF chains at 
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    constant $a_0$, bond length $d$ and total energy $E$ for HF chains
...
for urea supecells at the $\Gamma$-point RHF-MIC/6-21G* level of
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    and the {\tt TIGHT} threshold. $n$ is the number of urea units in the (super)cell.
...
\begin{table}[t]
  ...
  \caption{\protect
    ...
    and total energies $E$ for MgO using the periodic 
...
modified ERI. The latter integral arises from the use of the minimal image convention. 
We demonstrated how this new ERI can be computed with the help of a modified VRR 
in the frame of the Obara-Saika and Head-Gordon-Pople algorithm.  
