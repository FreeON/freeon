...
\begin{abstract}
  ...
  the analytical Hartree-Fock exchange energy gradients with respect to atomic positions and cell parameters at
  the $\Gamma$-point approximation is presented. While the evaluation of the
  exchange gradients with respect to atomic positions are similar to their gas phase limit, the 
  gradients with respect to cell parameters involve the accumulation of the gradients acting on atoms multiplied 
...
Difficulties arise when derivative of ERI's are taken with respect to lattice 
...
Figure~\ref{Fig:CnvgEnergy} shows the logarithm relative error to the 
converged large unit-cell $\Gamma$-point approximation
%${\bf k}$-space limit
with respect to the unit-cell length with and without the MIC.
The logarithm absolute convergences of the atomic and cell forces, where
the bond length has been slight contracted by 0.01\AA~to generate 
non-zero forces, are also shown in Figure~\ref{Fig:CnvgEnergy}.
The calculations with the MIC show an exponential convergence of the total
energy and forces to the converged large unit-cell $\Gamma$-point approximation
with respect to cell length. However, a naive implementation of 
...
\begin{figure}[h]
  \caption{Convergence of the logarithm $\Gamma$-point relative energy with and without the MIC to the corresponding 
    %${\bf k}$-space limit 
    converged large unit-cell $\Gamma$-point approximation
...
\begin{figure}[h]
  \caption{Convergence of the logarithm $\Gamma$-point relative energy without the MIC to the corresponding 
    converged large unit-cell MIC $\Gamma$-point approximation
    for the (HF)$_n$ chains ($n$ ranges from 1 to 31) with respect to logarithm unit-cell length. 
    The cell parameter $a_0=2.4745$\AA~and bond length $d=0.9453$\AA~were held fixed. 
    Calculations were performed at the RHF-MIC/6-31G level of theory and the {\tt TIGHT} threshold.}\label{Fig:CnvgEnergyNoMIC}
%  \resizebox*{3.6in}{!}{\includegraphics[angle=-90.00]{./DATA/HF_Energy_cnvg.ps}}
%  \resizebox*{3.6in}{!}{\includegraphics[angle=-90.00]{./DATA/HF_Energy_Forces_cnvg.ps}}
%  \resizebox*{3.6in}{!}{\includegraphics[angle=-90.00]{./DATA/HF_Energy_Forces_cnvg_2.ps}}
...
in energy of the two different basis set is of the same order as what we could observe
for gas phase calculations. We conclude that the differences observed for the geometry
...
%    $n$ is the number of urea units in the (super)cell.
  ...
  \begin{tabular}{lccc}
%          STRE_A         11.226421     -0.000017      0.000000     11.226421
%          STRE_B         11.226421      0.000000      0.000000     11.226421
%          STRE_C          9.306216      0.000015      0.000000      9.306216
%          ALPHA          90.000000      0.000126      0.000000     90.000000
%          BETA           90.000000      0.000505      0.000000     90.000000
%          GAMMA          90.000000     -0.001120      0.000000     90.000000
%          VOLM_L       1172.885894      0.000000      0.000000   1172.885894
%     1    -3581.5628751397
    \toprule
...
% -2.23841752471450E+02
    ...
    CO           & 1.246 & & 1.245 \\
    CN           & 1.334 & & 1.330 \\
    NH$_1$       & 1.002 & & 0.997 \\%cis-O
    NH$_2$       & 1.001 & & 0.995 \\%trans-O
...
gas phase limit, the exchange gradients with respect to ell parameters require the accumulation of 
...
that, for insulator, the convergence of the HF-MIC $\Gamma$-point total energy 
...
the system size. For 3D-periodic system, we believe that the convergence 
of the total energy without the MIC behaves inversely proportional to the 
unit-cell volume.
