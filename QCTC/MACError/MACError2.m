...
SetOptions[FortranAssign,AssignOptimize->True,AssignMaxSize->500,AssignBreak->{132," & \n          "},
	   AssignTemporary->{W,Array},AssignToArray->{BraO,KetO}];

SetOptions[OpenWrite, PageWidth -> 200];

(* PUT THE TRANSFORMATIONS TO FILE *)

FileName="MACErrBnd2.Inc";
Print[" Openned ",FileName];
OpenWrite[FileName];

Do[Do[
      WriteString[FileName,"     CASE(",100*M+L,") \n"];
      LHSList=Flatten[Table[Table[MACTnsr[m,l],{l,0,L}],{m,0,M}]];
      RHSList=Flatten[Table[Table[MACErr[m,l],{l,0,L}],{m,0,M}]];

      Write[FileName,FortranAssign[Evaluate[LHSList],RHSList,AssignReplace->{"Delta"->"Q%MAC%Delta","Ell"->"Q%Pole%Ell"}]];
