...
!!$#endif
  CASE DEFAULT
     CALL Halt('HONX: Do not recognize this action <'//TRIM(SCFActn)//'>.')
  END SELECT
  !
  !------------------------------------------------
...
!!$#else
  CALL CPU_TIME(Time1)
  CALL ComputD2K(D,HessX,ListC,ListC,GMc,BSc,CS_OUT)
  CALL CPU_TIME(Time2)
!!$#endif
  TmGx = Time2-Time1
...
!!$#else
  ! Print out.
  CALL PChkSum(HessX,'d2Kx/dRdR''['//TRIM(CurGeom)//']',Proc_O=Prog)  
  !
  CALL Multiply(HessX,KScale)
  !CALL Add()
  !
!!$#endif
  !
  CALL Put(HessX,'hessian',Tag_O=CurGeom)
  !
  !write(*,*) 'Grad Kx'
  !do i=1,natoms
