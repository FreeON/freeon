...
11              CALL Halt(' OpenASCII ERROR: IOS='//TRIM(IntToChar(IOS))// &
                ...
                IF(MyId==ROOT)THEN
                 ...
                 SUBROUTINE Get_CHR_VECT(A,VarName,Tag_O)
                   ...
                   IF(MyId==ROOT)THEN
                      ...
                      ALLOCATE(B(BufSize))
                      CALL ReadIntegerVector(Meta,B(1))
                      IF(NN /= B(1))CALL Halt('ERR: SIZE problem in Get_CHR_VECT')
                      RunInd = 2
                      DO II = 1, NN
                        TEMP = ' '
                        StrInd = 1
                ...
                IF(MyId==ROOT)THEN
                 ...
                 SUBROUTINE Get_CHR10_VECT(A,VarName,Tag_O)
                   ...
                   IF(MyId==ROOT)THEN
                      ...
                      ALLOCATE(B(BufSize))
                      CALL ReadIntegerVector(Meta,B(1))
                      IF(NN /= B(1))CALL Halt('ERR: SIZE problem in Get_CHR10_VECT')
                      RunInd = 2
                      DO II = 1, NN
                        TEMP = ' '
                        StrInd = 1
                        ...
                        A%C(II) = TEMP
                      ENDDO
                      IF(BufSize /= RunInd -1 )CALL Halt('ERR: Index problem in Get_CHR10_VECT')
                      DEALLOCATE(B)
#ifdef PARALLEL 
                      ENDIF       
                      ! not supported yet. IF(InParallel)CALL Bcast(A)
  ...
  SUBROUTINE Put_CellSet(CS,Name_O,Tag_O)
    TYPE(CellSet)                  :: CS
...
!    LOGICAL,         OPTIONAL,INTENT(IN) :: UnLimit_O
    ...
    ! (ie one at a time) or HDF death will ensue.

!    WRITE(*,*)' NCElls = ',CS%NCells
!    WRITE(*,*)' CellCarts = ',CS%CellCarts%D
