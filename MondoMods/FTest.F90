...
!---------------------------------------------------------------------
  NSamples=1000000
  IF(Gamma_Switch/=Exp_Switch)THEN
     WRITE(*,*)' Serious bug in LookUpTables! '
     WRITE(*,*)' Gamma and Exp switch are not the same '
     STOP
  ENDIF
!---------------------------------------------------------------------
...
!WRITE(*,*)T,Exp(-T),Test
     ...
     T=T+DT
  ENDDO
  MaxErr(0)=MAX(MaxErr(0),EXP(-Exp_Switch))
...
!=================================================================================
  ...
  DO K=1,NSamples
     ...
     T=T+DT
  ENDDO
  MaxErr(0)=MAX(MaxErr(0),One-DERF(Erf_Switch))
