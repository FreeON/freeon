...
!========================================================================
   CONTAINS
      ...
      SUBROUTINE AddBlok_FastMat(A,Row,Col,B)
...
!-----------------------------------------------------------------
         ...
         NULLIFY(D)
         NULLIFY(P)        
!        Search through linked list for row
         C=>A
         DO WHILE(ASSOCIATED(C%Next))
            IF(C%Row==Row)THEN
               EXIT
            ELSEIF(Row>C%Row.AND.Row<C%Next%Row)THEN
!              Insert link in assending order
               D=>C%Next
               NULLIFY(C%Next)
            ...
            ENDIF
         ENDDO
!        If row didn't turn up, add a new link to end of list
         IF(.NOT.ASSOCIATED(C%Next))CALL New(C%Next,Row)
!        Init global counter
         SRSTCount=C%Next%Nodes
!        Find/add node in this rows search tree
         P=>Insert_SRST(C%Next%RowRoot,Col)
...
!           Resum the blok
         ...
         ELSE
            ...
            CALL IncMem(MemStatus,0,N*M,'AddBlok_FastMat')
            P%MTrix=B
         ENDIF
!        Update counter
         C%Next%Nodes=SRSTCount
      END SUBROUTINE AddBlok_FastMat
!=================================================================
!
!=================================================================
      RECURSIVE FUNCTION Insert_SRST(A,Col) RESULT(C)
        TYPE(SRST), POINTER :: A,C
        INTEGER             :: Col,Split
!---------------------------------------------------------
        IF(A%Tier==0.AND.(Col<A%L.OR.Col>A%R)) &
           CALL Halt(' Logic error in Insert_SRST ')
        NULLIFY(C)
!       Halt recursion if we've found our node
...
!       Go left or right
        IF(Col>=A%L.AND.Col<=Split)THEN
           ...
           C=>Insert_SRST(A%Left,Col)
        ELSE
           IF(.NOT.ASSOCIATED(A%Right)) &
              CALL New_SRST(A%Right,Split+1,A%R,A%Tier+1)
           C=>Insert_SRST(A%Right,Col)
        ENDIF
      END FUNCTION Insert_SRST
!
...
!-----------------------------------------------------------------------------
         ...
         C%NAtms=A%NAtms
         Status=SymbolikMM_GENERIC_77(NAtoms,A%NAtms,A%NBlks,NAtoms,                &
...
!C23456789012345678901234567890123456789012345678901234567890123456789012
      INTEGER FUNCTION SymbolikMM_GENERIC_77(NAtoms77,                     &
                                             ...
                                             BSiz77,Flag77)
         IMPLICIT NONE
         INTEGER :: NAtoms77,ANAtms,ANBlks,ANAtoms,BNAtms,BNBlks,BNAtoms, &
...
!------------------------------------------------------------------------
         ...
         CALL Load(A,GlobalRowPtA)
         CALL Load(B,GlobalRowPtB)
         CALL NumerikMM_GENERIC(A%NAtms,OffSt%I(MyId),                        &
...
!===============================================================================
      SUBROUTINE NumerikMM_BCSR(A,B,C,Perf_O)
         IMPLICIT NONE
         TYPE(BCSR),         INTENT(IN)    :: A,B
         TYPE(BCSR),         INTENT(INOUT) :: C    
         TYPE(TIME),OPTIONAL,INTENT(INOUT) :: Perf_O         
...
!==================================================================
      ...
      IMPLICIT NONE
      INTEGER                                 :: Add_GENERIC
      INTEGER,                  INTENT(IN)    :: MxBlks,MxNon0,ANAtms,AOffSt
      REAL(DOUBLE),DIMENSION(:),INTENT(IN)    :: AMTrix,BMTrix
      INTEGER,     DIMENSION(:),INTENT(IN)    :: ARowPt,AColPt,ABlkPt, &
                                                 BRowPt,BColPt,BBlkPt, &
                                                 BSiz
      INTEGER,                  INTENT(OUT)   :: CNAtms,CNBlks,CNNon0
      REAL(DOUBLE),DIMENSION(:),INTENT(OUT)   :: CMTrix
      INTEGER,     DIMENSION(:),INTENT(OUT)   :: CRowPt,CColPt,CBlkPt
      INTEGER,     DIMENSION(:),INTENT(INOUT) :: Flag     
      REAL(DOUBLE),             INTENT(INOUT) :: FlOp
      REAL(DOUBLE)                            :: Op
...
!------------------------------------------------------------------------
         ...
         CALL New(Flag,NAtoms)
         CALL SetEq(Flag,0)
         Tmp=Dot_GENERIC(A%NAtms,OffSt%I(MyId),BSiz%I,Flag%I,FlOp,  &
                         A%RowPt%I,A%ColPt%I,A%BlkPt%I,A%MTrix%D,   & 
                         B%RowPt%I,B%ColPt%I,B%BlkPt%I,B%MTrix%D)
...
!--------------------------------------------------------------------
         ...
         CALL SetEq(Flag,0)
         Dot_BCSR=                                            &
         Dot_GENERIC(A%NAtms,0,BSiz%I,Flag%I,FlOp,            &
                     A%RowPt%I,A%ColPt%I,A%BlkPt%I,A%MTrix%D, & 
                     B%RowPt%I,B%ColPt%I,B%BlkPt%I,B%MTrix%D)
         CALL Delete(Flag)
         PerfMon%FLOP=PerfMon%FLOP+FlOp
...
!======================================================================
      FUNCTION Dot_GENERIC(ANAtms,OffStA,BSiz,Flag,FlOp, &
         ...
         INTEGER,                  INTENT(IN)    :: ANAtms,OffStA
         INTEGER,     DIMENSION(:),INTENT(IN)    :: ARowPt,AColPt,ABlkPt, &
                                                    BRowPt,BColPt,BBlkPt,BSiz        
         REAL(DOUBLE),DIMENSION(:),INTENT(IN)    :: AMTrix,BMTrix
         INTEGER,     DIMENSION(:),INTENT(INOUT) :: Flag
...
!--------------------------------------------------------------------
         ...
         IF(PRESENT(SetEq_O))THEN
            ...
            ENDIF
         ENDIF
         CALL FilterM_GENERIC(B%NAtms,OffSt%I(MyId),                   &
...
!-------------------------------------------------------------
         ...
         IF(MyId==ROOT)THEN
#endif
         CALL FilterM_GENERIC(B%NAtms,0,A%NAtms,A%NBlks,A%NNon0,       &
                              A%RowPt%I,A%ColPt%I,A%BlkPt%I,A%MTrix%D, &
                              B%RowPt%I,B%ColPt%I,B%BlkPt%I,B%MTrix%D, &
                              BSiz%I,Tol,FlOp)
#ifdef PARALLEL
...
!============================================================================
      SUBROUTINE FilterM_GENERIC(BNAtms,BOffSt,               &
                                 ...
                                 BRowPt,BColPt,BBlkPt,BMTrix, &
                                 MSiz,Tol,FlOp)
         INTEGER,                  INTENT(IN)  :: BNAtms,BOffSt
