...
!!! this routines should be used calculate proper overlaps
  ...
  SUBROUTINE CollectDXVects(RefStruct,LastStruct,DXVects,Delocs_O)
    ...
    DO I=1,NMem
      DO J=1,NCart
        CartVect%D(J)=RefStruct(J,I)-LastStruct(J)
      ENDDO
      DO J=1,NCart
        ScaledD%D(J,I)=CartVect%D(J)
      ENDDO
    ENDDO
    !
...
12  FORMAT(5F40.20)
  ...
  SUBROUTINE CleanPastGrads(RefStruct,RefGrad,PBCDim,GOpt,SCRPath,Print2)
    ...
    DO I=1,NDim
      DO J=1,NCart
        Carts%D(J)=RefStruct(J,I)
      ENDDO
      DO J=1,NCart
        CartGrad%D(J)=RefGrad(J,I)
      ENDDO
      ...
      IF(GOpt%TrfCtrl%DoTranslOff) THEN
        DO J=1,NCart-9
          AuxGrad%D(J)=CartGrad%D(J)
        ENDDO
        CALL TranslsOff(AuxGrad%D,Print2)
        DO J=1,NCart-9
          CartGrad%D(J)=AuxGrad%D(J)
        ENDDO
      ENDIF
