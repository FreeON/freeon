...
CONTAINS
  ...
  SUBROUTINE LoadCommands(N)
    ...
    if(len(trim(N%M_HOME)) == 0) then
      ...
      write(*,*) 'env variable $(MONDO_HOME) not set. Using '//trim(N%M_HOME)
    endif
    CALL GetEnv('MONDO_EXEC',N%M_EXEC)
    if(LEN(TRIM(N%M_EXEC)) == 0) then
      N%M_EXEC = HAVE_MONDO_HOME//"/bin"
      write(*,*) 'env variable $(MONDO_EXEC) not set. Using '//trim(N%M_EXEC)
    endif
    CALL GetEnv('MONDO_SCRATCH',N%M_SCRATCH)
    IF(LEN(TRIM(N%M_SCRATCH)) == 0) then
      write(*,*) 'env variable $(MONDO_SCRATCH) not set. Using /tmp'
      N%M_SCRATCH = "/tmp/"
      MONDO_SCRATCH = "/tmp/"
    endif
    ! Set path names etc
