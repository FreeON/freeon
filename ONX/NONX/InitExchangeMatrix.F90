...
!H
!H  Comments:
!H  - InitK_DBCSR should be removed.
!H---------------------------------------------------------------------------------
  USE DerivedTypes
  USE GlobalScalars
...
!H---------------------------------------------------------------------------------
!H SUBROUTINE InitK_BCSR(BS,GM,K)  !,NameBuf)
!H
!H---------------------------------------------------------------------------------
    TYPE(BSET    ), INTENT(IN   ) :: BS
    TYPE(CRDS    ), INTENT(IN   ) :: GM
    TYPE(BCSR    ), INTENT(INOUT) :: K
!old_stuff    TYPE(INT_VECT), INTENT(IN   ) :: NameBuf
    ...
    !K%MTrix%D(1:NElem)=0.0D0 !vw<--- This is done before.
    j=1; iPnt=1
    DO AtA=1,NAtoms
       KA=GM%AtTyp%I(AtA)
       NBFA=BS%BfKnd%I(KA)
