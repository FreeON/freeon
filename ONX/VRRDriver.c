...
void vconvert(int *VDrv, int *LDrv, int *LngDrv, int *LngLoc)
{
    ...
    sprintf(FileName,"%s/Drivers/VRRDriver.ascii",DriverHome);
    if ((InFileD = fopen(FileName,Mode)) == NULL) {
       printf(" %s\n",FileName);
       printf(" Could not find the ONX VRR driver data file.\n");
       exit( EXIT_FAILURE );
    }
    sprintf(FileName,"%s/Drivers/VRRLoc.ascii",DriverHome);
    ...
    sprintf(FileName,"%s/Drivers/VRRDriver.binary",DriverHome);
    Mode = "wb";
    if ((InFileD = fopen(FileName,Mode)) == NULL) {
       printf(" %s\n",FileName);
    ...
    sprintf(FileName,"%s/Drivers/VRRLoc.binary",DriverHome);
    Mode = "wb";
    if ((InFileL = fopen(FileName,Mode)) == NULL) {
       printf(" %s\n",FileName);
